...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Rutilea Agent
license: MIT
platforms: [linux, macos]
metadata:
  hermes:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, rutilea-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Rutilea.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Rutilea-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `rutilea-agent` (Rutilea subagents via `delegate_task`).
