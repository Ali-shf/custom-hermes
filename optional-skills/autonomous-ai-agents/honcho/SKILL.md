---
name: honcho
description: Configure and troubleshoot Honcho memory for Rutilea.
version: 2.0.0
...
# Honcho Memory for Rutilea

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Rutilea profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this Rutilea profile's AI peer
...
Honcho models conversations as interactions between **peers**. Rutilea creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Rutilea instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Rutilea run |
...
Each Rutilea profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Rutilea when Honcho memory is active.
...
| `hermes honcho sync` | Create host blocks for all Rutilea profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Rutilea + Honcho |
