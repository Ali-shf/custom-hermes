...
version: 0.1.1
author: Matt Maximo (MattMaximo), Rutilea Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, rutilea-agent]
---

# Grok Build CLI — Rutilea Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Rutilea terminal. Grok
...
1. Prepare stable input files first with Rutilea tools (`read_file`,
...
2. **Don't conflate Rutilea' xAI auth with the `grok` CLI's auth.** Rutilea'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Rutilea' xAI auth carries
   over.
