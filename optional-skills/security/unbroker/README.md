...
The design is **Rutilea-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
Then start a new Rutilea session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Rutilea session:
...
- **PII handling.** Dossiers live under the Rutilea home directory (`0600`, optionally
