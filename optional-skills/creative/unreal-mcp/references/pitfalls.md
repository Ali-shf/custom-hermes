...
### 1. Start order: editor first, Rutilea second

Rutilea probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Rutilea session.
...
restart the editor, restart the Rutilea session.
...
server fails to bind (Output Log) or Rutilea' probe times out. Fix: change
...
### 5. GenerateClientConfig is not for Rutilea

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Rutilea' connection
lives in `~/.hermes/config.yaml` via `hermes mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Rutilea — just don't
mistake it for the Rutilea setup step.
...
explicitly warns against overlapping calls. Rutilea executes same-turn tool
...
### 9. Timeouts: Rutilea gives up before Unreal does

Rutilea' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Rutilea runs elsewhere (SSH backend,
