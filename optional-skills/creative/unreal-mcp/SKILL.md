...
Companion skill for the `unreal-engine` entry in the Rutilea MCP catalog. The
...
Two halves, in this order: the editor side must be up before Rutilea connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for Rutilea — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Rutilea connects
...
2. Start the Rutilea session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Rutilea
they appear as:

| Rutilea tool | Purpose |
|---|---|
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Rutilea runs batched calls
...
- **Start order matters.** Editor + server up first, then the Rutilea
...
- **Timeouts on long operations.** Rutilea' per-call default is 120 s; asset
