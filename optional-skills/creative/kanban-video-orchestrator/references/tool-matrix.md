...
Maps each role archetype to the Rutilea skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public rutilea-agent
...
## Rutilea skills relevant to video production
...
These are **not** Rutilea skills but external CLIs / APIs that profiles invoke.
...
## Built-in Rutilea tools for media review

These are native Rutilea tools — not invoked via terminal but through their own
...
| Anthropic | `ANTHROPIC_API_KEY` | every Rutilea profile (Claude) |
...
When a new Rutilea-public video skill ships:
