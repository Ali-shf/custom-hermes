...
the actual `setup.sh` that creates the project workspace, configures Rutilea
...
PyYAML must be installed in the user's Python (it ships with most Rutilea
...
the Rutilea `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
