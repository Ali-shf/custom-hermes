...
short to a music video to an ASCII loop — in a Rutilea Kanban pipeline that
...
3. **Generates** a setup script that creates Rutilea profiles, project workspace, and the initial kanban task
...
For mapping role → which Rutilea skills + toolsets it loads, see
...
3. Creates each Rutilea profile via `hermes profile create --clone`
