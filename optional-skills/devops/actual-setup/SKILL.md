---
name: actual-setup
description: Set up Actual Computer (actual.inc) inference in Rutilea.
version: 2.0.0
author: shl0ms + Rutilea Agent
license: MIT
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Rutilea inference
...
- User has an `ac_` key and wants Rutilea routed through their Actual cluster.
...
- Rutilea has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Rutilea — the
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Rutilea at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
...
1. **reasoning_effort trap (handled by Rutilea since the first-class provider).**
   ...
   request still 400s this way on an old Rutilea, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Rutilea' default toolset
   ...
   servers emit a bare `data: [DONE]` — Rutilea reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Rutilea the built-in provider wins the name; stale custom blocks
