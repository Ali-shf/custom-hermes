---
name: rutilea-s6-container-supervision
description: Modify or debug s6 services in the Rutilea Docker image.
version: 1.0.0
...
# Rutilea s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Rutilea Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the Rutilea Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
```sh
docker build -t rutilea-agent-harness:latest .
HERMES_TEST_IMAGE=rutilea-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
- `rutilea-agent-dev`: General rutilea-agent codebase navigation
- `hermes-tool-quirks`: Specific Rutilea-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
