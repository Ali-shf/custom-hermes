...
Defenses 1 and 2 (the ``_RUTILEA_GATEWAY`` guard on ``rutilea gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.rutilea.gateway")``,
...
State lives in ``<RUTILEA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_rutilea_home() / "gateway" / "restart_loop.json"
