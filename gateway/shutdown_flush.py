...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<rutilea_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active RUTILEA_HOME."""
    from rutilea_constants import get_rutilea_home

    flush_dir = get_rutilea_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<rutilea_home>/pending_messages/``), so a runtime cap rotation no
