...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from rutilea_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (rutilea_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Rutilea is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_rutilea_home() / "sessions")
    ...
    # gateway behaves exactly as before — single RUTILEA_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``rutilea config set gateway.multiplex_profiles true``.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.rutilea/config.yaml (primary user-facing config)
    3. ~/.rutilea/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_rutilea_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through rutilea_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `rutilea config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `rutilea config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``rutilea config set gateway.streaming.*``
