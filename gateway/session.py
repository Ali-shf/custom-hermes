...
# ``RUTILEA_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``RUTILEA_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("RUTILEA_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in rutilea_state, request-dump
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `rutilea tools` (opt-in, default
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `rutilea tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `rutilea tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_rutilea_home()}/cron/output/)"
    )
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier Rutilea session "
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # ``get_rutilea_home()`` for the turn (its docstring lists "sessions"
    ...
    def _open_session_db_for_active_scope(self):
        ...
        time, and that helper follows the context-local RUTILEA_HOME override
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.rutilea/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.rutilea/state.db "
                "and are shown by `rutilea sessions list` and `/sessions`. "
    ...
    def _append_to_transcript_serialized(
        ...
        while True:
            try:
                ...
                from rutilea_state import CompressionSessionClosedError
