...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.rutilea to the active profile home."""
    ...
    if raw == "~/.rutilea":
        return rutilea_home
    if raw.startswith("~/.rutilea/"):
        return rutilea_home / raw.removeprefix("~/.rutilea/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return rutilea_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under RUTILEA_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from rutilea_constants import get_rutilea_home

    scripts_root = (get_rutilea_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.rutilea" or raw_text.startswith("~/.rutilea/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__rutilea_ignore__") is True
        ):
            return False, None
