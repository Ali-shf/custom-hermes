...
There are two ways to add a platform to the Rutilea gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.rutilea/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Rutilea code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `rutilea gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `rutilea_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Rutilea core.
...
And add it to the `rutilea-gateway` composite:

```python
"rutilea-gateway": {
    "includes": [..., "rutilea-your-platform"]
}
```
...
## 12. Status Display (`rutilea_cli/status.py`)
...
## 13. Gateway Setup Wizard (`rutilea_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ rutilea_cli/ toolsets.py \
  --include="*.py" -l | sort -u
