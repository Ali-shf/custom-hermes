...
# Audio file extensions Rutilea recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Rutilea but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Rutilea' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Rutilea-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_rutilea_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``RUTILEA_SESSION_PLATFORM`` contextvar — that contextvar is
...
from rutilea_constants import get_default_rutilea_root, get_rutilea_dir, get_rutilea_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.rutilea/.env manually."
)
...
IMAGE_CACHE_DIR = get_rutilea_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_rutilea_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_rutilea_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from rutilea_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; RutileaAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_rutilea_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_rutilea_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_rutilea_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_rutilea_dir("cache/screenshots", "browser_screenshots")
...
_RUTILEA_ROOT = get_default_rutilea_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "RUTILEA_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "RUTILEA_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "RUTILEA_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "RUTILEA_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _RUTILEA_HOME / "image_cache",
    _RUTILEA_HOME / "audio_cache",
    _RUTILEA_HOME / "video_cache",
    _RUTILEA_HOME / "document_cache",
    _RUTILEA_HOME / "browser_screenshots",
    ...
    _RUTILEA_HOME / "cache" / "images",
    _RUTILEA_HOME / "cache" / "audio",
    _RUTILEA_HOME / "cache" / "videos",
    _RUTILEA_HOME / "cache" / "documents",
    _RUTILEA_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Rutilea root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* RUTILEA_HOME's cache, so a gateway running at
    the root (e.g. ``RUTILEA_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when RUTILEA_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _RUTILEA_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("RUTILEA_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _RUTILEA_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Rutilea profile and shared Rutilea root both contain control
    ...
    # RUTILEA_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.rutilea
    ...
    for rutilea_root in (_RUTILEA_HOME, _RUTILEA_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(rutilea_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Rutilea
    secrets (``~/.rutilea/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Rutilea cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.rutilea/...``
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Rutilea cache dirs (``/root/.rutilea/...``), the default
    ...
    # In-process gateways (Desktop backend, `rutilea serve`) may not have
    ...
    # are longer prefixes, so /root/.rutilea/... still translates to the host
    ...
    if default_home is not None and not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.rutilea/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.rutilea"):
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``RUTILEA_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Rutilea-managed cache, under an operator-allowlisted root
    (``RUTILEA_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Rutilea root (~/.rutilea/.env,
    ...
    # ``MEDIA:~/.rutilea/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?rutilea\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+rutilea[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``RUTILEA_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Rutilea commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-RUTILEA_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("RUTILEA_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "RUTILEA_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "RUTILEA_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Rutilea sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-RUTILEA_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(rutilea --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Rutilea home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Rutilea home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.rutilea/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from rutilea_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          RUTILEA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          RUTILEA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          RUTILEA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("RUTILEA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("RUTILEA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("RUTILEA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_rutilea_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # RUTILEA_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _rutilea_run_generation is set on
    ...
    def build_source(
        ...
        per-profile RUTILEA_HOME isolation.
        """
