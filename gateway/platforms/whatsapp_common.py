...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Rutilea Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to RUTILEA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/rutilea), this function
    mirrors the bridge source to a writable RUTILEA_HOME location and returns that
    ...
    # Try RUTILEA_HOME location first
    rutilea_home = get_rutilea_home()
    rutilea_home_bridge = rutilea_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to RUTILEA_HOME if needed
    if rutilea_home_bridge.exists():
        return rutilea_home_bridge

    # Mirror the bridge source to RUTILEA_HOME
    try:
        rutilea_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            rutilea_home_bridge,
            dirs_exist_ok=False,
