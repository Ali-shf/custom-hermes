...
The PID file lives at ``{RUTILEA_HOME}/gateway.pid``.  RUTILEA_HOME defaults to
``~/.rutilea`` but can be overridden via the environment variable.  This means
separate RUTILEA_HOME directories naturally get separate PID files — a property
...
from rutilea_constants import get_rutilea_home, _get_platform_default_rutilea_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_rutilea_home() / "gateway-starts.log"
...
def _get_process_rutilea_home() -> Path:
    """Return the process-level RUTILEA_HOME, skipping context-local overrides.
    ...
    ``get_rutilea_home()`` honors ``_RUTILEA_HOME_OVERRIDE`` contextvar used for
...
def _canonical_rutilea_home(path: Path | str) -> Path:
    """Return a stable absolute RUTILEA_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_rutilea_home(left: Path | str, right: Path | str) -> bool:
    """Compare RUTILEA_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_rutilea_home(left))) == os.path.normcase(
        str(_canonical_rutilea_home(right))
    )


# Mirrors rutilea_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (rutilea_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a RUTILEA_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.rutilea/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_rutilea_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_rutilea_home(canonical, get_default_rutilea_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_rutilea_home(canonical, _get_platform_default_rutilea_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    RUTILEA_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``rutilea_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting RUTILEA_HOME."""
    home = _get_process_rutilea_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("RUTILEA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "rutilea" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Rutilea' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Rutilea gateway lifecycle subcommand from a command line.
    ...
    test also matched ``rutilea_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\rutilea-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Rutilea's
    ...
    for token in tokens:
        ...
        if basename in ("rutilea-gateway", "rutilea-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "rutilea_cli.main" in joined
        or "rutilea_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("rutilea", "rutilea.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `rutilea gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Rutilea-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Rutilea gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a RUTILEA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.rutilea`` or ``$RUTILEA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``rutilea_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``RUTILEA_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a RUTILEA_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"rutilea_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit RUTILEA_HOME= on the argv. RUTILEA_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "rutilea_home=" in command_lc and f"rutilea_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # RUTILEA_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "rutilea_home": str(_canonical_rutilea_home(_get_process_rutilea_home())),
    }
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `rutilea status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``RUTILEA_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``rutilea_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    RUTILEA_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different RUTILEA_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from rutilea_home.
    profile = _profile_label_for_home(_get_process_rutilea_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``rutilea.service`` + ``rutilea-
# gateway.service``).
#
...
def _get_takeover_marker_path(rutilea_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``rutilea_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = rutilea_home or _get_process_rutilea_home()
    return _canonical_rutilea_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-RUTILEA_HOME --replace while
    ...
    our_home = _get_process_rutilea_home()
    target_home = record.get("target_rutilea_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_rutilea_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_rutilea_home")
        if replacer_home is not None and not _same_rutilea_home(
    ...
    # ``rutilea gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's RUTILEA_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_rutilea_home(
            target_home or _get_process_rutilea_home()
        )
        ...
        record = {
            ...
            "target_rutilea_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_rutilea_home": str(
                _canonical_rutilea_home(_get_process_rutilea_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target RUTILEA_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("rutilea_home")
    if not isinstance(pid_record_home, str) or not _same_rutilea_home(
