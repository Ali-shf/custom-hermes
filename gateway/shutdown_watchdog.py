...
2. An event-loop heartbeat file at ``<RUTILEA_HOME>/state/gateway.heartbeat`` so
...
def _process_rutilea_home() -> Path:
    """RUTILEA_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<RUTILEA_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_rutilea_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from rutilea_logging import drain_log_queue
