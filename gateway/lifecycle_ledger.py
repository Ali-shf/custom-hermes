...
``<RUTILEA_HOME>/state/gateway.lifecycle.json``:
...
def _process_rutilea_home() -> Path:
    """RUTILEA_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<RUTILEA_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``rutilea_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
