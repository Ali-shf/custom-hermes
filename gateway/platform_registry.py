...
class PlatformEntry:
    ...
    # status displays (``rutilea setup``, ``rutilea status``, the dashboard
    ...
    # Env vars this platform needs (for ``rutilea setup`` display).
    ...
    # built-ins).  Used by ``rutilea gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved RUTILEA_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `rutilea`
        # invocation -- including plain `rutilea chat`, which never touches any
        ...
        # `rutilea setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``rutilea setup``/``gateway status``, channel
