...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.rutilea/.env`` into the gateway process via
    ``rutilea_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m rutilea_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "rutilea_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m rutilea_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=rutilea_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from rutilea_cli.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "loop":
        ...
        if not session:
            return _err(rid, 4001, "no active session")
        try:
            from rutilea_cli.loops import LoopManager, dispatch_loop_command
        except Exception as exc:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind RUTILEA_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_rutilea_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("RUTILEA_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("RUTILEA_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_rutilea_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off RUTILEA_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from rutilea_constants import set_rutilea_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from rutilea_constants import reset_rutilea_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``rutilea journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from rutilea_cli.skills_hub import do_install
        ...
        if action == "browse":
            from rutilea_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from rutilea_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `rutilea mcp` offers, so
    ...
    try:
        ...
        from rutilea_cli import mcp_catalog
...
# (rutilea_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes RUTILEA_HOME via set_rutilea_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses rutilea_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from rutilea_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from rutilea_cli.config import load_config, save_config, save_env_value
        from rutilea_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from rutilea_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.rutileaDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``rutilea mcp login`` uses
    ...
    try:
        ...
        rutilea_home = str(get_rutilea_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(rutilea_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``rutilea plugins`` / the dashboard, so the three surfaces
    ...
    plugins live under each profile's RUTILEA_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Rutilea plugin.
