...
def _default_registry_path() -> Path:
    return get_rutilea_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_rutilea_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_rutilea_home = expected_rutilea_home if expected_rutilea_home is not None else str(get_rutilea_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["RUTILEA_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("rutilea_home") or "")
        if got_home and got_home != self.expected_rutilea_home:
            raise RuntimeError(f"compute host RUTILEA_HOME mismatch: {got_home} != {self.expected_rutilea_home}")
