...
handlers delegate to the same `rutilea_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Rutilea profiles (name, path, model, description, skill count).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's rutilea-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``rutilea setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (rutilea_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local RUTILEA_HOME override — the same
        ...
        try:
            ...
            token = set_rutilea_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from rutilea_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from rutilea_cli.config import load_config_readonly, read_user_config_raw
            from rutilea_cli.web_routers.profiles import _write_profile_model
            from rutilea_constants import (
                reset_rutilea_home_override,
                set_rutilea_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the RUTILEA_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from rutilea_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `rutilea tools`
            ...
            # composites (rutilea-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from rutilea_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `rutilea tools` / Settings is where
            ...
            description = ""
            try:
                from rutilea_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from rutilea_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from rutilea_cli.skills_config import save_disabled_skills
