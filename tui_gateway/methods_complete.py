...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'rutilea' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "rutilea".startswith(prefix.lower()) and "rutilea" not in seen:
                out.append(
                    {
                        "text": "@rutilea",
                        "display": "@rutilea",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from rutilea_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from rutilea_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from rutilea_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `rutilea model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.rutilea/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from rutilea_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
