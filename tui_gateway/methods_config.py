...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from rutilea_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_rutilea_home), "display": display_rutilea_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _rutilea_home / "config.yaml"
        try:
...
def _(rid, params: dict) -> dict:
    try:
        from rutilea_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Rutilea provider is configured.",
                },
            )
