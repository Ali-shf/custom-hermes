"""ACP auth helpers — detect and advertise Rutilea authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "rutilea-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Rutilea runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Rutilea can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Rutilea.
    ...
    not have Rutilea provider credentials configured yet, so Rutilea always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Rutilea using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Rutilea provider",
            description=(
                "Open Rutilea' interactive model/provider setup in a terminal. "
                "Use this when Rutilea has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
