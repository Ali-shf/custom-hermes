"""CLI entry point for the rutilea-agent ACP adapter.

Loads environment variables from ``~/.rutilea/.env``, configures logging
...
try:
    ...
    # shadowing Rutilea's own modules — ``rutilea acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from RUTILEA_HOME (default ``~/.rutilea``)."""
    from rutilea_cli.env_loader import load_rutilea_dotenv

    rutilea_home = get_rutilea_home()
    loaded = load_rutilea_dotenv(rutilea_home=rutilea_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", rutilea_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="rutilea-acp",
        description="Run Rutilea Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Rutilea version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Rutilea provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.rutilea/node/ "
...
def _print_version() -> None:
    ...
    print(rutilea_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import RutileaACPAgent  # noqa: F401

    print("Rutilea ACP check OK")


def _run_setup() -> None:
    from rutilea_cli.main import main as rutilea_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "rutilea", "model"]
        rutilea_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting rutilea-agent ACP adapter")
    ...
    import acp
    from .server import RutileaACPAgent
    ...
    if os.environ.get("RUTILEA_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
