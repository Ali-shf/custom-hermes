...
_RUTILEA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (RUTILEA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `rutilea tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``rutilea tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (RUTILEA_KANBAN_TASK env "
    ...
    # Coding posture (base Rutilea — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in rutilea_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Rutilea toolsets (CLI + messaging platforms)
    #
    ...
    # the `rutilea send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "rutilea-cli": {
        ...
        "tools": _RUTILEA_CORE_TOOLS,
        "includes": []
    },

    "rutilea-cron": {
        # Mirrors rutilea-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `rutilea tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as rutilea-cli; gated by `rutilea tools`",
        "tools": _RUTILEA_CORE_TOOLS,
        "includes": []
    },
    ...
    "rutilea-email": {
        "description": "Email bot toolset - interact with Rutilea via email (IMAP/SMTP)",
        "tools": _RUTILEA_CORE_TOOLS,
        "includes": []
    },
    ...
    "rutilea-sms": {
        "description": "SMS bot toolset - interact with Rutilea via SMS (Twilio)",
        "tools": _RUTILEA_CORE_TOOLS,
        "includes": []
    },
    ...
    "rutilea-webhook": {
        ...
        "tools": _RUTILEA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "rutilea-gateway": {
        ...
        "tools": [],
        "includes": ["rutilea-telegram", "rutilea-discord", "rutilea-whatsapp", "rutilea-slack", "rutilea-signal", "rutilea-bluebubbles", "rutilea-homeassistant", "rutilea-email", "rutilea-sms", "rutilea-mattermost", "rutilea-matrix", "rutilea-dingtalk", "rutilea-feishu", "rutilea-wecom", "rutilea-wecom-callback", "rutilea-weixin", "rutilea-qqbot", "rutilea-webhook", "rutilea-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``rutilea-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_RUTILEA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``rutilea-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (rutilea-<name>).
        # Gives them _RUTILEA_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("rutilea-"):
            platform_name = name[len("rutilea-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_RUTILEA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
