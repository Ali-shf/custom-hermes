...
three virtual model IDs so the ``rutilea tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `rutilea tools` → Image "
                    "Generation → OpenAI to configure, or `rutilea setup` "
                    "to add the key."
                ),
