...
``<rutilea home>/plugins/<name>/``. That tree is the plugin *install* dir —
``rutilea plugins remove`` deletes it and ``rutilea plugins update`` git-pulls
...
``<rutilea home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Rutilea.
...
# Mirrors the plugin-name shape `rutilea plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<rutilea home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`rutilea_constants.get_rutilea_home` on every call. Don't cache the
    result across profile switches.
    """
    from rutilea_constants import get_rutilea_home

    root = get_rutilea_home() / "plugin-data" / _validate_name(name)
