"""`rutilea meet node ...` subcommand tree.

Wired into the existing ``rutilea meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``rutilea meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="rutilea-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``rutilea meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             rutilea meet node approve <name> ws://<host>:{args.port} {token}")
        try:
