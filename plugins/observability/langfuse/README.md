...
This plugin ships bundled with Rutilea but is **opt-in** — it only loads when
...
rutilea tools  # → Langfuse Observability

# Manual
pip install langfuse
rutilea plugins enable observability/langfuse
```

## Required credentials

Set these in `~/.rutilea/.env` (or via `rutilea tools`):

```bash
RUTILEA_LANGFUSE_PUBLIC_KEY=pk-lf-...
RUTILEA_LANGFUSE_SECRET_KEY=sk-lf-...
RUTILEA_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
```bash
rutilea plugins list                 # observability/langfuse should show "enabled"
rutilea chat -q "hello"              # then check Langfuse for a "Rutilea turn" trace
```

Generation observations include the Rutilea system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `RUTILEA_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
RUTILEA_LANGFUSE_ENV=production       # environment tag
RUTILEA_LANGFUSE_RELEASE=v1.0.0       # release tag
RUTILEA_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
RUTILEA_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
RUTILEA_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
RUTILEA_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`RUTILEA_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
rutilea plugins disable observability/langfuse
```
