...
Optional Rutilea observability plugin that configures exporters and maps
Rutilea-specific observer hooks to NeMo Relay marks and ATIF state. Rutilea core
...
not replace Rutilea Agent's planner, tools, memory, model provider routing, or
CLI UX. Rutilea core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Rutilea Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Rutilea core.
- Add Rutilea session, turn, approval, and subagent mark events.
...
```bash
rutilea plugins enable observability/nemo_relay
```

The `RUTILEA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `RUTILEA_HOME` that the
agent run will use:

```bash
env RUTILEA_HOME=/tmp/rutilea-nemo-relay-test \
  rutilea plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`RUTILEA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`RUTILEA_HOME` is the Rutilea profile/config home used by both
`rutilea plugins enable ...` and the later `rutilea chat ...` run. If unset,
Rutilea uses the user's default home, usually `~/.rutilea`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export RUTILEA_HOME=/tmp/rutilea-nemo-relay-test
rutilea plugins enable observability/nemo_relay
rutilea chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `rutilea` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run rutilea plugins enable observability/nemo_relay
uv run rutilea chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Rutilea installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/rutilea_agent-*.whl
rutilea plugins enable observability/nemo_relay
```
...
behavior; it does not control Rutilea shared client metrics.
...
`rutilea.session.segment` / `rutilea.session.segment_reason`
...
The plugin can configure exporters directly from `RUTILEA_NEMO_RELAY_*`
...
```bash
export RUTILEA_NEMO_RELAY_ATOF_ENABLED=1
export RUTILEA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export RUTILEA_NEMO_RELAY_ATIF_ENABLED=1
export RUTILEA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `RUTILEA_NEMO_RELAY_ATOF_FILENAME`
- `RUTILEA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `RUTILEA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `RUTILEA_NEMO_RELAY_ATIF_AGENT_NAME`
- `RUTILEA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `RUTILEA_NEMO_RELAY_ATIF_MODEL_NAME`
- `RUTILEA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Rutilea at it:

```bash
export RUTILEA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Rutilea Agent"
agent_version = "local"
```

When `RUTILEA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`RUTILEA_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`RUTILEA_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Rutilea does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Rutilea keeps the direct env-var fallbacks active for that run.

Rutilea core routes provider and tool execution through NeMo Relay managed APIs
...
Rutilea identifies managed LLM operations by provider wire protocol, not by the
...
| Rutilea API mode | Relay operation |
| --- | --- |
| `chat_completions` | `openai.chat_completions` |
| `codex_responses` | `openai.responses` |
| `anthropic_messages` | `anthropic.messages` |

Middleware written against earlier Rutilea builds might match physical provider
names such as `openrouter`, `nous`, or `anthropic` as the operation. Match the
canonical operation instead. Rutilea retains the original provider in the
managed invocation metadata field `rutilea.provider`. Unknown or custom API
modes continue to use the physical provider name as their operation.

### Dynamic Plugins

Rutilea uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Rutilea-owned
...
with this Rutilea-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Rutilea rejects
...
Rutilea activates these plugins before registering its managed LLM and tool
...
For the full generic Rutilea middleware contract, see
...
with Rutilea and a local Ollama model served through the OpenAI-compatible API.

```bash
export RUTILEA_HOME=/tmp/rutilea-nemo-relay-docs/rutilea-home
mkdir -p "$RUTILEA_HOME"

cat > "$RUTILEA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Rutilea session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export RUTILEA_NEMO_RELAY_ATOF_ENABLED=1
export RUTILEA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/rutilea-nemo-relay-docs/subagent/atof
export RUTILEA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export RUTILEA_NEMO_RELAY_ATOF_MODE=overwrite
export RUTILEA_NEMO_RELAY_ATIF_ENABLED=1
export RUTILEA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/rutilea-nemo-relay-docs/subagent/atif
export RUTILEA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export RUTILEA_NEMO_RELAY_ATIF_AGENT_NAME='Rutilea Agent E2E'
export RUTILEA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export RUTILEA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"rutilea.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Rutilea Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Rutilea dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/rutilea-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/rutilea-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/rutilea-nemo-relay-docs/workdir/beta.txt
cd /tmp/rutilea-nemo-relay-docs/workdir

export RUTILEA_NEMO_RELAY_ATOF_ENABLED=1
export RUTILEA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/rutilea-nemo-relay-docs/parallel/atof
export RUTILEA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export RUTILEA_NEMO_RELAY_ATOF_MODE=overwrite
export RUTILEA_NEMO_RELAY_ATIF_ENABLED=1
export RUTILEA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/rutilea-nemo-relay-docs/parallel/atif
export RUTILEA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export RUTILEA_NEMO_RELAY_ATIF_AGENT_NAME='Rutilea Agent E2E'
export RUTILEA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Rutilea sessions map to `agent` scopes.
- Rutilea core managed provider calls map to `llm` scope start/end events.
- Rutilea core managed tool calls map to `tool` scope start/end events.
...
Rutilea core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Rutilea-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Rutilea calls the
...
Enable it for Rutilea:

```bash
export RUTILEA_NEMO_RELAY_PLUGINS_TOML=/tmp/rutilea-middleware-test/plugins.toml
```

Execution follows these boundaries with or without an adaptive component:

```text
Rutilea provider call
  -> nemo_relay.llm.execute(...)
    -> Rutilea provider adapter callback(...)

Rutilea tool call
  -> nemo_relay.tools.execute(...)
    -> Rutilea authorization and dispatch callback(...)
```
...
middleware for a local Rutilea run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export RUTILEA_HOME=/tmp/rutilea-middleware-test/rutilea-home
mkdir -p "$RUTILEA_HOME" /tmp/rutilea-middleware-test/nemo-relay
...
cat > /tmp/rutilea-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/rutilea-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/rutilea-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Rutilea Middleware E2E"
agent_version = "local"
...
export RUTILEA_NEMO_RELAY_PLUGINS_TOML=/tmp/rutilea-middleware-test/nemo-relay/plugins.toml
