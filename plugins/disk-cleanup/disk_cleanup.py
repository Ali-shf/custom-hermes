"""disk_cleanup — ephemeral file cleanup for Rutilea Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under RUTILEA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly RUTILEA_HOME and /tmp/rutilea-*
Never touches: ~/.rutilea/logs/ or any system directory.
"""
...
try:
    ...
    def get_rutilea_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("RUTILEA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".rutilea").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$RUTILEA_HOME/logs/``."""
    return get_rutilea_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$RUTILEA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under RUTILEA_HOME or ``/tmp/rutilea-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    rutilea_home = get_rutilea_home()
    try:
        path.resolve().relative_to(rutilea_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/rutilea-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("rutilea-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "rutilea-agent", "backups", "profiles", ".worktrees",
...
# Paths under $RUTILEA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so RUTILEA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        rutilea_home = get_rutilea_home()
        for parent in ("cron", "cronjobs"):
            base = rutilea_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside RUTILEA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under RUTILEA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Rutilea checkout, venv,
    # and desktop build under RUTILEA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    rutilea_home = get_rutilea_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in rutilea_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    rutilea_home = get_rutilea_home()
    try:
        rel = path.resolve().relative_to(rutilea_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "rutilea-agent",
        ...
        # Path isn't under RUTILEA_HOME (e.g. /tmp/rutilea-*) — fall through.
        pass
