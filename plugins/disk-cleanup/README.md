...
Auto-tracks and cleans up ephemeral files created during Rutilea Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$RUTILEA_HOME` and `/tmp/rutilea-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `RUTILEA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under RUTILEA_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `RUTILEA_HOME` or `/tmp/rutilea-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$RUTILEA_HOME/disk-cleanup/` is itself excluded
- `$RUTILEA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
