...
``{title, url, description, position}`` rows every other Rutilea web
provider produces.
...
prefers Rutilea-managed xAI Grok OAuth (via ``rutilea auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.rutilea/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``rutilea tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `rutilea auth` to sign in with "
