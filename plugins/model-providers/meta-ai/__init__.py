"""Meta Model API (Muse Spark) provider plugin for Rutilea Agent.
...
Bundled from https://github.com/albertodepaola/rutilea-meta-provider. Rutilea'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to rutilea' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Rutilea' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in rutilea_cli/providers.py and the retention hint in
