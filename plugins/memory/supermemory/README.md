...
- Hosted: API key from [app.supermemory.ai/integrations?connect=rutilea](http://app.supermemory.ai/integrations?connect=rutilea)
...
```bash
rutilea memory setup    # select "supermemory"
```

Or manually:

```bash
rutilea config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.rutilea/.env
```
...
Before running `rutilea memory setup`, add the local endpoint to
`$RUTILEA_HOME/supermemory.json`:
...
Then run `rutilea memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$RUTILEA_HOME/supermemory.json`
...
| `container_tag` | `rutilea` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `rutilea-{identity}` → `rutilea-coder`). |
...
`https://api.supermemory.ai`. Rutilea resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: rutilea`, and document writes stamp
`metadata.sm_source: rutilea`. This is a **functional routing key, not telemetry**:
it groups Rutilea-written memories into a dedicated "Rutilea" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Rutilea profile:

```json
{
  "container_tag": "rutilea-{identity}"
}
```

For a profile named `coder`, this resolves to `rutilea-coder`. The default profile resolves to `rutilea-default`. Without `{identity}`, all profiles share the same container.
