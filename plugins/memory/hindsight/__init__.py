...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: rutilea)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: rutilea)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $RUTILEA_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "rutilea").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Rutilea can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``rutilea memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    selected local_embedded without going through ``rutilea memory setup`` — a
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'rutilea memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $RUTILEA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_rutilea_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "rutilea": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "rutilea"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Rutilea config."""
    profile = config.get("profile", "rutilea")
    return str(profile or "rutilea")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Rutilea .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Rutilea profile name (from agent_identity)
      {workspace} — Rutilea workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``rutilea-{user}`` with no user becomes ``rutilea``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Rutilea Agent and the User"
    ...
    def save_config(self, values, rutilea_home):
        """Write config to $RUTILEA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(rutilea_home) / "hindsight"
    ...
    def post_setup(self, rutilea_home: str, config: dict) -> None:
        ...
        from rutilea_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(rutilea_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "rutilea")
        ...
        self.save_config(provider_config, rutilea_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(rutilea_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(rutilea_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Rutilea starter template (best-effort)."""
        from rutilea_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "rutilea")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "rutilea"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: rutilea-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Rutilea Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "rutilea"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "rutilea"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/rutilea (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "rutilea", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "rutilea")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Rutilea Agent and the User")
        ...
        # Rutilea emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Rutilea as a non-root user, or switch "
                    "to cloud / local_external mode via 'rutilea memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Rutilea get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See rutilea-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Rutilea created/used that client on the shared async loop,
