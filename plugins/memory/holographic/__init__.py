"""rutilea-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $RUTILEA_HOME/config.yaml (profile-scoped):
  plugins:
    rutilea-memory-store:
      db_path: $RUTILEA_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "rutilea-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, rutilea_home):
        """Write config to config.yaml under plugins.rutilea-memory-store."""
        from pathlib import Path
        config_path = Path(rutilea_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["rutilea-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_rutilea_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _rutilea_home = str(get_rutilea_home())
        _default_db = _rutilea_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $RUTILEA_HOME in user-supplied paths so config values like
        # "$RUTILEA_HOME/memory_store.db" or "~/.rutilea/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$RUTILEA_HOME", _rutilea_home)
            db_path = db_path.replace("${RUTILEA_HOME}", _rutilea_home)
