...
```bash
rutilea memory setup    # select "holographic"
```

Or manually:
```bash
rutilea config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.rutilea-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$RUTILEA_HOME/memory_store.db` | SQLite database path |
