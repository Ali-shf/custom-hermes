...
```bash
rutilea memory setup    # select "mem0"
```

Or manually:
```bash
rutilea config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.rutilea/.env
```

## Config

Behavioral settings live in `$RUTILEA_HOME/mem0.json` (set them via `rutilea memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.rutilea/.env`.
...
| `user_id` | `rutilea-user` | User identifier on Mem0 |
| `agent_id` | `rutilea` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   rutilea memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   rutilea memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.rutilea/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.rutilea/.env
   ```
   or in `$RUTILEA_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Rutilea session and call `mem0_search` — it connects to your server.
...
```bash
rutilea memory setup mem0 --mode oss \
...
```bash
rutilea memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$RUTILEA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.rutilea/mem0_qdrant"}}
  }
}
```
...
```bash
rutilea memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
rutilea memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$RUTILEA_HOME/mem0.json`).
