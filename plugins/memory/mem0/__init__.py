...
Configuration
-------------
Secret (lives in $RUTILEA_HOME/.env or the environment):
...
Behavioral settings (live in $RUTILEA_HOME/mem0.json, set via `rutilea memory
setup`):
  ...
  agent_id           — Agent identifier (default: rutilea)
...
def _load_config() -> dict:
    """Load config from env vars, with $RUTILEA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "rutilea"),
        "oss": {},
    }
    ...
    config_path = get_rutilea_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, rutilea_home):
        """Write config to $RUTILEA_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, rutilea_home: str, config: dict) -> None:
        ...
        post_setup(rutilea_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $RUTILEA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "rutilea")
