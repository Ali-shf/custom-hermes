...
- OpenViking server running and reachable from Rutilea

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Rutilea can identify older servers that expose the legacy status-only health
...
Then configure Rutilea:

```bash
rutilea memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Rutilea, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
rutilea config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.rutilea/.env`; for a named profile use
`~/.rutilea/profiles/<profile>/.env`.
...
# OPENVIKING_AGENT=rutilea
```

## Config

OpenViking's server config is separate from Rutilea:
...
Rutilea-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `rutilea` | Rutilea peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Rutilea lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Rutilea sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Rutilea built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Rutilea action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Rutilea
...
file URIs, such as
`viking://user/peers/rutilea/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/rutilea/memories/preferences/mem_abc123.md`. Files
