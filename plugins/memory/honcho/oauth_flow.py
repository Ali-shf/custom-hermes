...
``rutilea://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Rutilea OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "rutilea-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``rutilea-desktop`` / ``rutilea-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Rutilea.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local RUTILEA_HOME override can't reach.
