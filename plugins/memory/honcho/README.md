...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/rutilea>
...
```bash
rutilea memory setup honcho   # configure Honcho directly (works on a fresh install)
rutilea memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
rutilea config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.rutilea/.env
```

> `rutilea honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `rutilea memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `rutilea model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$RUTILEA_HOME/honcho.json` | Profile-local (isolated Rutilea instances) |
| 2 | `~/.rutilea/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Rutilea profile: `rutilea` (default) or `rutilea_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `rutilea honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Rutilea session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`rutilea-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`rutilea`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-rutilea-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening rutilea in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Rutilea session ID (timestamp + hex). Every `rutilea` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Rutilea profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`rutilea`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Rutilea profile: `rutilea` (default) or `rutilea_<profile>` (e.g. `rutilea -p coder` -> host key `rutilea_coder`). Older `rutilea.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `RUTILEA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `rutilea-agent`) |
...
|---------|-------------|
| `rutilea memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `rutilea honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `rutilea memory setup`) |
| `rutilea honcho status` | Show resolved config for active profile |
| `rutilea honcho enable` / `disable` | Toggle Honcho for active profile |
| `rutilea honcho mode <mode>` | Change recall or observation mode |
| `rutilea honcho peer --user <name>` | Update user peer name |
| `rutilea honcho peer --ai <name>` | Update AI peer name |
| `rutilea honcho tokens --context <N>` | Set context token budget |
| `rutilea honcho tokens --dialectic <N>` | Set dialectic max chars |
| `rutilea honcho map <name>` | Map current directory to a session name |
| `rutilea honcho sync` | Create host blocks for all Rutilea profiles |
