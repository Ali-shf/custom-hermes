...
```bash
rutilea memory setup    # select "byterover"
```

Or manually:
```bash
rutilea config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.rutilea/.env
```
...
Working directory: `$RUTILEA_HOME/byterover/` (profile-scoped).
