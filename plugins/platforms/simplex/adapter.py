"""SimpleX Chat platform adapter (Rutilea plugin).
...
This adapter ships as a Rutilea platform plugin under
``plugins/platforms/simplex/``. The Rutilea plugin loader scans the
...
Optional environment variables:
    ...
    RUTILEA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``rutilea setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Rutilea — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("RUTILEA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rutilea setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.rutilea/.env`` via
    ``rutilea_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "rutilea_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.rutilea/.env"
        )
        return
