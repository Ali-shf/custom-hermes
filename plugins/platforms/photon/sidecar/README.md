...
Small Node helper that bridges Rutilea Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Rutilea is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Rutilea-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Rutilea can receive inbound messages
...
The Rutilea plugin's `rutilea photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Rutilea is Python, so replies go through this sidecar
