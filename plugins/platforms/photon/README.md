...
This plugin connects Rutilea Agent to iMessage (and other Spectrum
...
TypeScript-only, Rutilea runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Rutilea channel,
...
Run `rutilea photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.rutilea/.env` (the same place every other
...
Management metadata lives in `~/.rutilea/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `rutilea photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Rutilea
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Rutilea coalesces
...
- **Native polls are supported.** Rutilea posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `rutilea send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<rutilea-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `rutilea photon status`, a DM and a group roundtrip,
