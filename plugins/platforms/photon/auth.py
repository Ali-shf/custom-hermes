...
Credential storage mirrors every other Rutilea channel:

    * runtime SDK creds  -> ``~/.rutilea/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.rutilea/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a rutilea dependency
...
# registers Rutilea as its own client_id.
...
# Default name of the project Rutilea provisions for the operator.
DEFAULT_PROJECT_NAME = "Rutilea Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of rutilea-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.rutilea/auth.json`` honouring the active Rutilea profile."""
    try:
        from rutilea_constants import get_rutilea_home
        return Path(get_rutilea_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.rutilea")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # rutilea_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from rutilea_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.rutilea/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.rutilea/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.rutilea/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: rutilea_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `rutilea photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `rutilea photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `rutilea photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `rutilea photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `rutilea photon setup`)"
