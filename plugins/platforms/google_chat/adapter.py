...
# ``model_tools`` import time, so ``rutilea status``, ``rutilea chat``, etc.
...
# ``rutilea_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Rutilea is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a rutilea MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "rutilea-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${RUTILEA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _rutilea_home = _get_rutilea_home()
        except (ModuleNotFoundError, ImportError):
            _rutilea_home = _Path.home() / ".rutilea"
        self._thread_count_store = _ThreadCountStore(
            _rutilea_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Rutilea is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("RUTILEA_HOME", str(_Path.home() / ".rutilea"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${RUTILEA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a rutilea MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Rutilea is thinking…' marker message.
        ...
        "Rutilea is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Rutilea is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Rutilea is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Rutilea is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``rutilea setup``.

    The setup wizard at ``rutilea_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.rutilea/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. rutilea-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.rutilea/.env")
    print_info("Restart the gateway: rutilea gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``rutilea cron`` running as a
    separate process from ``rutilea gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Rutilea plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `rutilea setup` to install Google Chat support.",
        ...
        # the "Rutilea is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Rutilea is thinking…' marker message "
