"""
LINE Messaging API platform adapter for Rutilea Agent.
...
to Rutilea Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``RUTILEA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            rutilea_home = Path(get_rutilea_home()).resolve()
        except Exception:
            rutilea_home = Path.home().joinpath(".rutilea").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``rutilea status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``rutilea status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rutilea setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.rutilea/.env`` via ``rutilea_cli.config``.
    """
    print()
    ...
    try:
        from rutilea_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("rutilea_cli.config not available; set LINE_* vars manually in ~/.rutilea/.env")
        return
