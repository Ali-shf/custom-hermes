"""
Email platform adapter for the Rutilea gateway.

Allows users to interact with Rutilea by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "rutilea-agent" "version" "{_rutilea_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `rutilea config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `rutilea gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Rutilea Agent")
        ...
        msg_id = f"<rutilea-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# rutilea_cli/gateway.py, and the _send_email dispatch in
