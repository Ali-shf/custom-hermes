"""ntfy platform adapter (Rutilea plugin).
...
a Rutilea dependency.

This adapter ships as a Rutilea platform plugin under
``plugins/platforms/ntfy/``. The Rutilea plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "rutilea-in"              # subscribe topic (incoming)
          publish_topic: "rutilea-out"     # optional — defaults to topic
...
_ECHO_TAG = "rutilea-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``rutilea cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Rutilea dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `rutilea gateway status` without
