# A2A — Agent-to-Agent protocol for Rutilea
...
peer (another Rutilea, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
rutilea gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Rutilea serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.rutilea/a2a_audit.jsonl`.
- Conversations persist to `~/.rutilea/a2a_conversations/` — they survive context
