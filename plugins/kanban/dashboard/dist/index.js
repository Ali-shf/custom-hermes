/**
 * Rutilea Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.rutilea/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__RUTILEA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__RUTILEA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "rutilea-kanban-dot-triage",
    todo: "rutilea-kanban-dot-todo",
    ready: "rutilea-kanban-dot-ready",
    running: "rutilea-kanban-dot-running",
    blocked: "rutilea-kanban-dot-blocked",
    review: "rutilea-kanban-dot-review",
    done: "rutilea-kanban-dot-done",
    archived: "rutilea-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-rutilea-task";
  ...
  const LS_BOARD_KEY = "rutilea.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="rutilea-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])rutilea-kanban-md-code\1/i.test(attrs)) {
      return ' class="rutilea-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "rutilea-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "rutilea-kanban-md",
  ...
  // standard `drop` event and our `rutilea-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("rutilea-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("rutilea-kanban-column--drop");
          if (target) target.classList.add("rutilea-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("rutilea-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("rutilea-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("rutilea-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__RUTILEA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "rutilea-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "rutilea-kanban-attention",
        "rutilea-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "rutilea-kanban-attention-bar" },
        h("span", { className: "rutilea-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "rutilea-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "rutilea-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "rutilea-kanban-attention-row",
                  "rutilea-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "rutilea-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "rutilea-kanban-attention-row-id" }, task.id),
                h("span", { className: "rutilea-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "rutilea-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "rutilea-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "rutilea-kanban-diag-action-btn",
      action.suggested ? "rutilea-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " rutilea-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".rutilea-kanban-drawer-comment-row input, .rutilea-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "rutilea-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("rutilea-kanban-diag", sevClass) },
      h("div", { className: "rutilea-kanban-diag-header" },
        h("span", { className: "rutilea-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "rutilea-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "rutilea-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "rutilea-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "rutilea-kanban-diag-data-row" },
                  h("span", { className: "rutilea-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "rutilea-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "rutilea-kanban-diag-data-row" },
                h("span", { className: "rutilea-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "rutilea-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "rutilea-kanban-diag-reassign-row" },
            h("span", { className: "rutilea-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "rutilea-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "rutilea-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "rutilea-kanban-diag-msg",
              msg.ok ? "rutilea-kanban-diag-msg--ok" : "rutilea-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "rutilea-kanban-section" },
      h("div", { className: "rutilea-kanban-section-head-row" },
        h("span", { className: "rutilea-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "rutilea-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "rutilea-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "rutilea-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "rutilea-kanban-docs-link",
      title: "Open Rutilea Kanban docs in a new tab",
      "aria-label": "Rutilea Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `rutilea kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "rutilea-kanban-msg-ok" : "rutilea-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "rutilea-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "rutilea-kanban-boardswitcher" },
      h("div", { className: "rutilea-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "rutilea-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "rutilea-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "rutilea-kanban-dialog-title" },
        ...
        h("div", { className: "rutilea-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Rutilea profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "rutilea-kanban-bulk" },
      h("span", { className: "rutilea-kanban-bulk-count" },
      ...
      h("div", { className: "rutilea-kanban-bulk-priority",
      ...
      h("div", { className: "rutilea-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Rutilea profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "rutilea-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("rutilea-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("rutilea-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "rutilea-kanban-trash",
        dragOver ? "rutilea-kanban-trash--drop" : "",
        props.draggingTaskId ? "rutilea-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "rutilea-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "rutilea-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".rutilea-kanban-card")) return true;
      if (target.closest && target.closest(".rutilea-kanban-column-add")) return true;
      if (target.closest && target.closest(".rutilea-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("rutilea-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("rutilea-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".rutilea-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "rutilea-kanban-columns",
        isScrollable ? "rutilea-kanban-columns--scrollable" : "",
        isPanning ? "rutilea-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("rutilea-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("rutilea-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "rutilea-kanban-column",
        dragOver ? "rutilea-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "rutilea-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "rutilea-kanban-col-check",
        ...
        h("span", { className: cn("rutilea-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "rutilea-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "rutilea-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "rutilea-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "rutilea-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "rutilea-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "rutilea-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "rutilea-kanban-lane" },
                  h("div", { className: "rutilea-kanban-lane-head" },
                    h("span", { className: "rutilea-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "rutilea-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "rutilea-kanban-card--stale-red";
    if (age >= tier.amber) return "rutilea-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".rutilea-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "rutilea-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "rutilea-kanban-card",
        props.selected ? "rutilea-kanban-card--selected" : "",
        props.failed ? "rutilea-kanban-card--failed" : "",
        props.draggingSource ? "rutilea-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "rutilea-kanban-card-content" },
          h("div", { className: "rutilea-kanban-card-row" },
            h("label", {
              className: "rutilea-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "rutilea-kanban-card-check",
            ...
            h("span", { className: "rutilea-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or rutilea kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "rutilea-kanban-warning-badge",
                    "rutilea-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "rutilea-kanban-needs-assignee",
          ...
          h("div", { className: "rutilea-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "rutilea-kanban-card-row rutilea-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "rutilea-kanban-assignee",
                            title: `Assigned to Rutilea profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "rutilea-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "rutilea-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "rutilea-kanban-ago",
  ...
  // centered modal reusing the rutilea-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "rutilea-kanban-dialog rutilea-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Rutilea profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Rutilea profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "rutilea-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "rutilea-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "rutilea-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "rutilea-kanban-drawer-close",
        ...
        data ? h("div", { className: "rutilea-kanban-drawer-comment-foot" },
          h("div", {
            className: "rutilea-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "rutilea-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "rutilea-kanban-section" },
      h("div", { className: "rutilea-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "rutilea-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "rutilea-kanban-drawer-body" },
      h("div", { className: "rutilea-kanban-drawer-title" },
        h("span", { className: cn("rutilea-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "rutilea-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "rutilea-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "rutilea-kanban-section" },
            h("div", { className: "rutilea-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "rutilea-kanban-section" },
            h("div", { className: "rutilea-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "rutilea-kanban-done-no-result rutilea-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "rutilea-kanban-section" },
            h("div", { className: "rutilea-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "rutilea-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "rutilea-kanban-section" },
        h("div", { className: "rutilea-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "rutilea-kanban-comment" },
            h("div", { className: "rutilea-kanban-comment-head" },
              h("span", { className: "rutilea-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "rutilea-kanban-diag-action-btn",
      ...
      h("div", { className: "rutilea-kanban-section" },
        h("div", { className: "rutilea-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "rutilea-kanban-comment" },
                h("div", { className: "rutilea-kanban-comment-head" },
                  h("span", { className: "rutilea-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "rutilea-kanban-comment-ago" },
      ...
      h("div", { className: "rutilea-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "rutilea-kanban-event",
              isDiag ? "rutilea-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "rutilea-kanban-event-header" },
                  h("span", { className: "rutilea-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "rutilea-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "rutilea-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "rutilea-kanban-event-header-plain" },
                  h("span", { className: "rutilea-kanban-event-kind" }, e.kind),
                  h("span", { className: "rutilea-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "rutilea-kanban-event-phantom-row" },
                  h("span", { className: "rutilea-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "rutilea-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "rutilea-kanban-event-payload" },
    ...
    return h("div", { className: "rutilea-kanban-section" },
      h("div", { className: "rutilea-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "rutilea-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `rutilea-kanban-run--${r.outcome || r.status || "ended"}`
          : "rutilea-kanban-run--active";
        return h("div", { key: r.id, className: cn("rutilea-kanban-run", outcomeClass) },
          h("div", { className: "rutilea-kanban-run-head" },
            h("span", { className: "rutilea-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "rutilea-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "rutilea-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "rutilea-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "rutilea-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "rutilea-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "rutilea-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "rutilea-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "rutilea-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "rutilea-kanban-pre rutilea-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "rutilea-kanban-meta-row" },
      h("span", { className: "rutilea-kanban-meta-label" }, props.label),
      h("span", { className: "rutilea-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "rutilea-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "rutilea-kanban-meta-row" },
        h("span", { className: "rutilea-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "rutilea-kanban-meta-value rutilea-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "rutilea-kanban-meta-row" },
        h("span", { className: "rutilea-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "rutilea-kanban-meta-value rutilea-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "rutilea-kanban-meta-row" },
        h("span", { className: "rutilea-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "rutilea-kanban-meta-value rutilea-kanban-editable",
    ...
    return h("div", { className: "rutilea-kanban-meta-row" },
      h("span", { className: "rutilea-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "rutilea-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "rutilea-kanban-section" },
      h("div", { className: "rutilea-kanban-section-head-row" },
        h("span", { className: "rutilea-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "rutilea-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "rutilea-kanban-section" },
      h("div", { className: "rutilea-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "rutilea-kanban-deps-row" },
        h("span", { className: "rutilea-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "rutilea-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "rutilea-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "rutilea-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "rutilea-kanban-dep-chip-x",
      ...
      h("div", { className: "rutilea-kanban-deps-row" },
        h("span", { className: "rutilea-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "rutilea-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "rutilea-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "rutilea-kanban-msg-ok"
          : "rutilea-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "rutilea-kanban-section" },
      ...
      h("div", { className: "rutilea-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "rutilea-kanban-home-sub rutilea-kanban-home-sub--on"
              : "rutilea-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__RUTILEA_PLUGINS__ && typeof window.__RUTILEA_PLUGINS__.register === "function") {
    window.__RUTILEA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
