...
# 5. Symlinks the 'rutilea' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Rutilea Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/rutilea-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink rutilea into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up rutilea command..."

RUTILEA_BIN="$SCRIPT_DIR/venv/bin/rutilea"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$RUTILEA_BIN" "$COMMAND_LINK_DIR/rutilea"
echo -e "${GREEN}✓${NC} Symlinked rutilea → $COMMAND_LINK_DISPLAY_DIR/rutilea"
...
# ============================================================================
# Seed bundled skills into ~/.rutilea/skills/
# ============================================================================

RUTILEA_SKILLS_DIR="${RUTILEA_HOME:-$HOME/.rutilea}/skills"
mkdir -p "$RUTILEA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.rutilea/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$RUTILEA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  rutilea status        # Check configuration"
if is_termux; then
    echo "  rutilea gateway       # Run gateway in foreground"
else
    echo "  rutilea gateway install # Install gateway service (messaging + cron)"
fi
echo "  rutilea cron list     # View scheduled jobs"
echo "  rutilea doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m rutilea_cli.main setup
fi
