...
const SIDEBAR_COLLAPSED_KEY = "rutilea-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="rutilea-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="rutilea-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateRutilea = status?.can_update_rutilea === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'rutilea update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "rutilea update";
    return (
      t.status.updateRutileaConfirmMessage ??
      `This will run 'rutilea update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateRutileaConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateRutilea) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateRutilea,
      runningLabel: t.status.updatingRutilea,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Rutilea gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateRutileaConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateRutileaConfirmTitle ?? `${t.status.updateRutilea}?`}
    />
    </>
  );
}
