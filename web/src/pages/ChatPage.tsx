/**
 * ChatPage — embeds `rutilea --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (rutilea_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "rutilea.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__RUTILEA_SESSION_TOKEN__ &&
    !window.__RUTILEA_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `rutilea dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("rutilea-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("rutilea-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__RUTILEA_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Rutilea TUI has enabled xterm mouse-events
    ...
    //      payloads upload to RUTILEA_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to RUTILEA_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[rutilea-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__RUTILEA_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets RUTILEA_HOME pointed at the
    ...
    // forwarding them into Rutilea. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="rutilea-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
