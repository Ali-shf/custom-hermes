import { buildRutileaWebSocketUrl } from "@rutilea/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/rutilea/). The Python backend
// injects ``window.__RUTILEA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__RUTILEA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const RUTILEA_BASE_PATH = readBasePath();
const BASE = RUTILEA_BASE_PATH;
...
declare global {
  interface Window {
    __RUTILEA_SESSION_TOKEN__?: string;
    __RUTILEA_BASE_PATH__?: string;
    ...
    __RUTILEA_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Rutilea-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__RUTILEA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``rutilea update``, ``rutilea gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__RUTILEA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__RUTILEA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__RUTILEA_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``rutilea_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__RUTILEA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Rutilea-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``rutilea_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__RUTILEA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildRutileaWebSocketUrl({
...
export const api = {
  ...
  updateRutilea: () =>
    fetchJSON<ActionResponse>("/api/rutilea/update", { method: "POST" }),
  checkRutileaUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/rutilea/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** rutilea-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``rutilea update``. */
  can_update_rutilea?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the RUTILEA_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the RUTILEA_HOME volume — no staleness dimension, so no
 * sampled_at. */
