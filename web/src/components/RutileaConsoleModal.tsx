...
interface RutileaConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function RutileaConsoleModal({ open, onClose }: RutileaConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("rutilea> ");
  const inputPromptRef = useRef("rutilea> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "rutilea> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Rutilea Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="rutilea-console-title"
    >
      <div
        className={cn(
          themedBody,
