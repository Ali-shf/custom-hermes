...
import { api, RUTILEA_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "rutilea:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${RUTILEA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${RUTILEA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?rutilea_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-rutilea-plugin", manifest.name);
