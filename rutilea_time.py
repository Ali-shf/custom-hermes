"""
Timezone-aware clock for Rutilea.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``RUTILEA_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.rutilea/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Rutilea never
...
from rutilea_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Rutilea requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("RUTILEA_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``RUTILEA_TIMEZONE`` update) to force ``get_timezone()`` /
