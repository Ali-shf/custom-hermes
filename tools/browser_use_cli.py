...
_PRIVATE_BROWSER_SENTINEL = "_RUTILEA_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# rutilea: pin this named session to its own tab (once per daemon process)
def _rutilea_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "rutilea-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_rutilea_ensure_own_tab()
del _rutilea_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Rutilea's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Rutilea's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_rutilea_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `rutilea tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Rutilea' own bin dir ($RUTILEA_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from rutilea_constants import get_rutilea_home

        return str(Path(get_rutilea_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Rutilea' own ``$RUTILEA_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Rutilea-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Rutilea' managed uv (bootstrapped on demand via
    ``rutilea_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$RUTILEA_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Rutilea-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through rutilea tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_rutilea_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `rutilea tools` → Browser Automation."
        )
