...
This module provides the glue:
    - ``RutileaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Rutilea Agent"
"""
...
from rutilea_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__rutilea_user_skipped__"
...
def _get_token_dir(rutilea_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses RUTILEA_HOME so each profile gets its own OAuth tokens.
    Layout: ``RUTILEA_HOME/mcp-tokens/``
    """
    from rutilea_constants import get_rutilea_home

    base = Path(rutilea_home) if rutilea_home is not None else Path(get_rutilea_home())
...
def _cached_redirect_port(storage: "RutileaTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Rutilea restarts and chooses a
...
def _cached_redirect_uri(storage: "RutileaTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``rutilea mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `rutilea mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# RutileaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class RutileaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        RUTILEA_HOME/mcp-tokens/<server_name>.json         -- tokens
        RUTILEA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        RUTILEA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, rutilea_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._rutilea_home = Path(rutilea_home) if rutilea_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._rutilea_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._rutilea_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._rutilea_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Rutilea records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._rutilea_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Rutilea.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `rutilea mcp login <server>` later to "
...
RutileaOAuthClientProvider: Any = None


def _get_rutilea_oauth_provider_class() -> type | None:
    global RutileaOAuthClientProvider
    if RutileaOAuthClientProvider is not None:
        return RutileaOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _RutileaOAuthClientProvider(OAuthClientProvider):
    ...
    _RutileaOAuthClientProvider.__name__ = "RutileaOAuthClientProvider"
    _RutileaOAuthClientProvider.__qualname__ = "RutileaOAuthClientProvider"
    RutileaOAuthClientProvider = _RutileaOAuthClientProvider
    return RutileaOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = RutileaTokenStorage(server_name, rutilea_home=rutilea_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "RutileaTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Rutilea Agent" / "Rutilea" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Rutilea Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Rutilea is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: rutilea mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Rutilea auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Rutilea is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = RutileaTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`rutilea mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_rutilea_oauth_provider_class()
