...
_FORCE_SYNC_ENV = "RUTILEA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.rutilea") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.rutilea to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``RUTILEA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, rutilea_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.rutilea/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(rutilea_home=rutilea_home)

    def _sync_back_transaction(self, rutilea_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="rutilea-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.rutilea/skills/a.md`` →
        ``~/.rutilea/skills/a.md``, a new remote file at
        ``/root/.rutilea/skills/b.md`` maps to ``~/.rutilea/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
