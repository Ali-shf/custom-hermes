...
wrapper, while preserving Rutilea' persistent snapshot behavior across sessions.
"""
...
_SNAPSHOT_STORE = get_rutilea_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.rutilea"),
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        ...
        Modal sandboxes always run as root, so /root/.rutilea is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.rutilea"
            )
