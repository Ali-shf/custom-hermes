...
Lets the LLM write a Python script that calls Rutilea tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `rutilea_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `rutilea_tools.py` with file-based RPC stubs
...
# available on every platform Rutilea itself runs on.
...
# match a safe prefix, the operational RUTILEA_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Rutilea
# code can lose the DB-layer Kanban mutation guard while still inheriting
# RUTILEA_HOME.
#
# NB: the broad "RUTILEA_" prefix was deliberately removed (#27303) — it leaked
# RUTILEA_*-named config that lacks a secret substring (e.g. RUTILEA_BASE_URL,
# RUTILEA_KANBAN_DB, RUTILEA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _RUTILEA_CHILD_ALLOWED below; RUTILEA_RPC_SOCKET /
# RUTILEA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational RUTILEA_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set rutilea_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_RUTILEA_CHILD_ALLOWED = frozenset({
    "RUTILEA_HOME",
    "RUTILEA_PROFILE",
    "RUTILEA_CONFIG",
    "RUTILEA_ENV",
    "RUTILEA_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational RUTILEA_* vars (_RUTILEA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret RUTILEA_* vars dropped by the tightened allowlist (#27303). The
    # broad "RUTILEA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like RUTILEA_KANBAN_DB / RUTILEA_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _RUTILEA_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("RUTILEA_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped RUTILEA_* var.
            _dropped_rutilea.append(k)
    if _dropped_rutilea:
        logger.debug(
            "execute_code: dropped %d non-allowlisted RUTILEA_* var(s) from the "
            ...
            len(_dropped_rutilea),
            ", ".join(sorted(_dropped_rutilea)),
        )
...
# ---------------------------------------------------------------------------
# rutilea_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Rutilea-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Rutilea-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    rutilea_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'rutilea_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_rutilea_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the rutilea_tools.py stub module.
...
"""Auto-generated Rutilea tools RPC stubs."""
...
def _connect():
    ...
    RUTILEA_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["RUTILEA_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("RUTILEA_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Rutilea tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("RUTILEA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "rutilea_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated rutilea_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/rutilea_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_rutilea_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/rutilea_tools.py", tools_src)
        ...
        env_prefix = (
            f"RUTILEA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"RUTILEA_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("RUTILEA_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with rutilea_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="rutilea_sandbox_")
    ...
    # selector from RUTILEA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"rutilea_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated rutilea_tools module.
        ...
        tools_src = generate_rutilea_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "rutilea_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  RUTILEA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["RUTILEA_RPC_SOCKET"] = rpc_endpoint
        child_env["RUTILEA_RPC_TOKEN"] = rpc_token
        ...
        # RUTILEA_TIMEZONE is an internal Rutilea setting and must not leak
        # into child processes.
        _tz_name = os.getenv("RUTILEA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("RUTILEA_TIMEZONE", None)
        ...
        # ``rutilea_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # Rutilea's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Rutilea's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Rutilea-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-Rutilea-
        ...
        from tools.environments.local import _strip_rutilea_owned_pythonpath
        _strip_rutilea_owned_pythonpath(child_env)
        _rutilea_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_rutilea_python_environment(_child_python):
            _pp_parts.append(_rutilea_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import rutilea_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the Rutilea "
                "environment; rutilea root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.rutilea/.env')).
...
def _load_config() -> dict:
    ...
    ``rutilea --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (rutilea-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the Rutilea environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the rutilea root from every
...
def _uses_rutilea_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Rutilea's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``rutilea tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and rutilea-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.rutilea/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Rutilea tools programmatically. "
        ...
        f"Available via `from rutilea_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from rutilea_tools import {import_str}` "
