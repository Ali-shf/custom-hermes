...
``ui_meta['rutilea-bots']`` block in its profile.yaml), a bot's canonical
...
def _rutilea_root(home: Path) -> Path:
    """Root ~/.rutilea for both the default profile and named profiles."""
...
def _is_bot_managed(profile_dir: Path) -> bool:
    """True when profile.yaml carries a ui_meta['rutilea-bots'] block.
    ...
    try:
        ...
        if "rutilea-bots" not in raw:
            return False
        import yaml

        data = yaml.safe_load(raw)
        ui_meta = data.get("ui_meta") if isinstance(data, dict) else None
        return isinstance(ui_meta, dict) and isinstance(ui_meta.get("rutilea-bots"), dict)
    except Exception:
        return False
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @rutilea.
    return "rutilea" if name == "default" else name


def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``rutilea peer``), for the protocol text.
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "`rutilea peer dm` (same terminal-tool pattern: background=true, "
        "notify_on_complete=true; the reply prints on stdout when it completes):\n"
        "```\n"
        'rutilea peer dm <peer>/<agent-name> "Message from 🤖 <you> (@<you>): your message"\n'
        "```\n"
        "Use `<peer>` alone for the peer's main agent. Run `rutilea peer list` "
        "for the live peer list."
    )


def _build_section(home: Path) -> str:
    root = _rutilea_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Rutilea profile is an agent teammate with "
        ...
        "```\n"
        f'rutilea -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from 🤖 {handle} (@{handle}): your message"\n'
        "```\n"
        ...
        "which agent it came from. Run `rutilea profile list` for the LIVE teammate "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient RUTILEA_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    resolved = str(home) if home else (os.getenv("RUTILEA_HOME") or os.path.expanduser("~/.rutilea"))
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    resolved = Path(str(home) if home else (os.getenv("RUTILEA_HOME") or os.path.expanduser("~/.rutilea")))
    surface: dict = {}
    try:
        ...
        token = set_rutilea_home_override(str(resolved))
        try:
    ...
    try:
        ...
        surface["peers"] = _peers(_rutilea_root(resolved))
    except Exception:
        surface["peers"] = []
    try:
