...
Design contract (matches the Rutilea "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    rutilea_meta = meta.get("rutilea") if isinstance(meta, dict) else None
    if not isinstance(rutilea_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.rutilea.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in rutilea_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.rutilea.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("rutilea", "agent", "rutilea agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Rutilea Agent' (proper caps) "
