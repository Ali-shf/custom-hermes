...
`rutilea skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.rutilea.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.rutilea.blueprint`` is a mapping containing
    ...
    rutilea = meta.get("rutilea") if isinstance(meta, dict) else None
    blueprint = rutilea.get("blueprint") if isinstance(rutilea, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.rutilea.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.rutilea.blueprint`` block) they can hand
    to ``rutilea skills publish`` to share. ``body`` is the plain-language
