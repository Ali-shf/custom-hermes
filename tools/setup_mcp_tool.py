...
``rutilea mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Rutilea desktop app. Use the "
            "terminal instead: `rutilea mcp install <name>` for catalog entries, "
            "`rutilea mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Rutilea desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `rutilea mcp catalog` "
