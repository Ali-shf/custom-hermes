...
etc.) surface as specific blocked checks via `rutilea computer-use doctor`
...
# fetches the latest release, so a `RUTILEA_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `RUTILEA_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "RUTILEA_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Rutilea should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Rutilea should disable cua-driver telemetry for this user.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session Rutilea YOLO toggle so a stale config line can never
...
def _cua_grant_existing_profile() -> bool:
    ...
    cua-driver's trusted-launcher grant. Rutilea passes
...
def _standard_runtime_launch_args(
    ...
    *,
    ...
    private_socket = socket_path or os.path.join(
        tempfile.gettempdir(), f"rutilea-cua-standard-{uuid.uuid4().hex[:12]}.sock"
    )
...
def _empty_discovery_reason() -> str:
    ...
    if sys.platform == "darwin":
        ...
        return (
            ...
            "`rutilea computer-use doctor`"
        )
    return (
        "window discovery returned no windows; run `rutilea computer-use "
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Rutilea runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Rutilea process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Rutilea session's YOLO choice
    ...
    * ``unrestricted`` — explicit Rutilea YOLO; launch-time risk
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\rutilea-cua-{token}"
        else:
    ...
    def start(self) -> None:
        ...
        self._stderr_thread = threading.Thread(
            ...
            args=(self._process,),
            name="rutilea-cua-daemon-stderr",
            daemon=True,
        )
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Rutilea an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``RUTILEA_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Rutilea Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `rutilea computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Rutilea process is restarted.
            os.path.join(
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``RUTILEA_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Rutilea' 0.20 integration."""
    ...
    if version < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return {
            ...
            "binary": resolved,
            "version": raw_version,
            "reason": "Rutilea computer use requires cua-driver 0.20.0 or newer",
        }
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `rutilea computer-use install --upgrade`."
    )
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``RUTILEA_CUA_DRIVER_CMD`` override is authoritative even when broken, and
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  rutilea computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `rutilea tools` and enable the Computer Use toolset to install it automatically."
    )
...
class _CuaDriverSession:
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Rutilea-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `rutilea computer-use doctor` and check "
                f"{display_rutilea_home()}/logs/agent.log for the phase timings."
            )
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # Rutilea talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so Rutilea did not replay it. Take fresh state before "
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        #   - Gets a distinct agent-cursor color per Rutilea run, with
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a Rutilea
        ...
        if not contract.get("ready"):
            ...
            if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip():
                repair = (
                    "Update the binary selected by RUTILEA_CUA_DRIVER_CMD or "
                    "remove that override."
                )
            else:
                repair = "Run `rutilea computer-use install` to repair it."
            raise RuntimeError(f"cua-driver is not ready: {reason}. {repair}")
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Rutilea' minimal core.
    ...
    def capture(
        ...
        Maps rutilea `capture(mode, app)` → cua-driver `list_windows` +
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Rutilea adapter, and uses cua-driver's
