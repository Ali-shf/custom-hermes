...
`health_report` (surfaced by `rutilea computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Rutilea runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Rutilea-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Rutilea's explicit approval bypass onto Cua's immutable mode.

    Rutilea has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "RUTILEA_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown RUTILEA_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Rutilea YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Rutilea sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Rutilea process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `rutilea computer-use install`. "
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Rutilea sessions.
...
# "foreground" (it doesn't know Rutilea has a typed page route), so the model
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$RUTILEA_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_rutilea_dir("cache/vision", "temp_vision_images")
...
def _spill_elements_to_file(cap: CaptureResult) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_rutilea_dir("cache/computer_use", "computer_use_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
...
def check_computer_use_requirements() -> bool:
    ...
    `rutilea computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
