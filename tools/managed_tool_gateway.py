...
def auth_json_path():
    """Return the Rutilea auth store path, respecting RUTILEA_HOME overrides."""
    return get_rutilea_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`rutilea tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Rutilea release, and
