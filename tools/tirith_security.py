...
it is automatically downloaded from GitHub releases to $RUTILEA_HOME/bin/tirith.
...
def _get_rutilea_home() -> str:
    """Return the Rutilea home directory, respecting RUTILEA_HOME env var."""
    return str(get_rutilea_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_rutilea_home(), ".tirith-install-failed")
...
def _rutilea_bin_dir() -> str:
    """Return $RUTILEA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_rutilea_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $RUTILEA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_rutilea_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $RUTILEA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $RUTILEA_HOME/bin/tirith
    ...
    rutilea_bin = os.path.join(_rutilea_bin_dir(), "tirith")
    if os.path.isfile(rutilea_bin) and os.access(rutilea_bin, os.X_OK):
        _resolved_path = rutilea_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return rutilea_bin
