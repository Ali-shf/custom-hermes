...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Rutilea's live source checkout "
        ...
        "checkout, stop Rutilea, run the command externally, then restart "
        "Rutilea."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(rutilea_home).expanduser() if rutilea_home else Path.home() / ".rutilea"
