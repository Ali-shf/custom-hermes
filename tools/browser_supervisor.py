...
One ``CDPSupervisor`` runs per Rutilea ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Rutilea logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "rutilea-dialog-bridge.invalid"
...
(() => {
  if (window.__rutileaDialogBridgeInstalled) return;
  window.__rutileaDialogBridgeInstalled = true;
  const ENDPOINT = "http://rutilea-dialog-bridge.invalid/";
