...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``rutilea tools`` repaint, tool-registration scans,
    ...
    2. ``~/.rutilea/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``rutilea auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_rutilea_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.rutilea/.env`` first, then ``os.environ``.

    Wraps :func:`rutilea_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from rutilea_cli.config import get_env_value as _rutilea_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _rutilea_get_env_value(name)
    return value if value is not None else default


def rutilea_xai_user_agent() -> str:
    """Return a stable Rutilea-specific User-Agent for xAI HTTP calls."""
    try:
        from rutilea_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Rutilea-Agent/{__version__}"


def rutilea_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Rutilea Agent,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": rutilea_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Rutilea config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Rutilea home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_rutilea_home() / "state"
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``RUTILEA_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`rutilea_cli.auth._xai_validate_inference_base_url` so a tampered
    env override can't exfiltrate the bearer; on rejection it falls back to
    the default rather than raising.
    """
    override = str(
        get_env_value("RUTILEA_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Rutilea-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``rutilea_cli.config.get_env_value`` so keys
    stored in ``~/.rutilea/.env`` (the standard Rutilea location) are honored —
    ...
    honors ``RUTILEA_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
