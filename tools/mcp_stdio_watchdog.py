...
<url>``) is spawned as a direct child of the Rutilea process. Rutilea's own
...
Rutilea process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Rutilea startup's opt-in
