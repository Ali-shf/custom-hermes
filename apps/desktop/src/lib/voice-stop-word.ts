...
//
// When someone is in a hands-free "Hey Rutilea" voice chat, the natural way to
...
// phrase (optionally addressed to Rutilea), so a real turn that merely contains
...
// Optional address prefixes so "rutilea stop" / "ok stop" / "hey rutilea, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey rutilea', 'hey rutilea,', 'rutilea', 'rutilea,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("rutilea") is not a stop command on its own.
      continue
    }
 ...
 * to Rutilea). Returns false for anything that merely contains "stop" as part of
