...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>Rutilea Rutilea</p>')
    performScopedFind(surface, 'rutilea', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'rutilea', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>rutilea</p>')
    performScopedFind(surface, 'rutilea', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">Rutilea</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>rutilea</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'rutilea')).toBe(true)
  })
