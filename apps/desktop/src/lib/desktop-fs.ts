import type {
  RutileaConnection,
  RutileaReadDirResult,
  RutileaReadFileTextResult,
  RutileaSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: RutileaConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: RutileaConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<RutileaReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<RutileaReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<RutileaReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<RutileaReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.rutileaDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: RutileaSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
