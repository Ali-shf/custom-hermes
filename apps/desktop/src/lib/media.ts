...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.rutileaDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.rutileaDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.rutileaDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.rutileaDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `rutilea-media://remote/${file}${profile}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `rutilea-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.rutilea/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.rutileaDesktop?.saveGatewayFile) {
    throw new Error('Desktop file download bridge is unavailable')
  }

  return window.rutileaDesktop.saveGatewayFile({
    path: file,
