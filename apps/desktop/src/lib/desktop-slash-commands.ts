...
/** Why a known Rutilea command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Rutilea profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Rutilea Agent version', surface: exec() },
...
function isKnownRutileaSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Rutilea' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownRutileaSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Rutilea and were never asked for. It is
