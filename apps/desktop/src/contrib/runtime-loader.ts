 ...
 *   (`@rutilea/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default RutileaPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<rutilea home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<rutilea home>/plugins/<name>/desktop/
...
import { createPluginContext, type RutileaPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.rutilea/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@rutilea/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @rutilea/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: RutileaPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default RutileaPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of rutilea-bots predating its adoption in-tree) must
...
//  - `<rutilea home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<rutilea home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's rutilea_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.rutilea/plugins` is installed-but-inert until the
