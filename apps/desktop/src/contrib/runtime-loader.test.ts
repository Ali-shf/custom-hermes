...
import type { RutileaReadDirResult } from '@/global'
import type * as RutileaModule from '@/rutilea'
...
// getStatus would supply the connected backend's rutilea_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ rutilea_home: '/remote/box/.rutilea' }))

vi.mock('@/rutilea', async importActual => ({
  ...(await importActual<typeof RutileaModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const agentPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<RutileaReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { rutileaDesktop: unknown }).rutileaDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { rutileaDesktop?: unknown }).rutileaDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend rutilea_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.rutilea/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.rutilea/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.rutilea/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.rutilea/plugins')
    // The remote backend's rutilea_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.rutilea/desktop-plugins')
  })
  ...
  it('probes desktop/plugin.js inside agent-plugin packages (unified packaging)', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.rutilea/plugins'
        ? { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.rutilea/plugins/my-feature' }] }
        : { entries: [] }
    )
    ...
    expect(readFileText).toHaveBeenCalledWith('/local/.rutilea/plugins/my-feature/desktop/plugin.js')
    // The Python half's files must never be probed as a desktop entry.
    expect(readFileText).not.toHaveBeenCalledWith('/local/.rutilea/plugins/my-feature/plugin.js')
  })

  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.rutileaDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.rutilea/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.rutilea/plugins'
        ? { entries: [{ isDirectory: true, name: 'uni', path: '/local/.rutilea/plugins/uni' }] }
        : { entries: [] }
    )
    ...
    try {
      ...
      // ~/.rutilea/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend rutilea_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.rutilea/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.rutilea/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.rutilea/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.rutilea/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.rutilea/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
