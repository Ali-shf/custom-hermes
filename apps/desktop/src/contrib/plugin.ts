 ...
 * `RutileaPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/rutilea'
...
export type { PluginRestOptions } from '@/rutilea'
 ...
 *  under `rutilea.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.rutileaDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Rutilea — use `host.notify` for the in-app toast.
...
export interface RutileaPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `rutilea.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.rutileaDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.rutileaDesktop
