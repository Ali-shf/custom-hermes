...
const SKIN_KEY = 'rutilea-desktop-theme-v2'
const MODE_KEY = 'rutilea-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'rutilea-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'rutilea-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'rutilea-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.rutileaTheme = skinName
  root.dataset.rutileaMode = rendered
  ...
  window.rutileaDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('rutilea-boot-background', chromeBg)
    window.localStorage.setItem('rutilea-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.rutileaThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.rutileaDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Rutilea authoring/activating a skin from a
