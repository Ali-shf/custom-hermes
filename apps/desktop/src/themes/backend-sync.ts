/**
 * Live skin sync from the Rutilea backend.
 *
 * The backend resolves the active skin (built-in or `$RUTILEA_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Rutilea
 ...
 */
export function ingestBackendSkin(skin: RutileaSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as RutileaSkin)
