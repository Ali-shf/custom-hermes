...
import { getRutileaConfigRecord, type RutileaConfigRecord, saveRutileaConfig } from '@/rutilea'
...
export interface I18nConfigClient {
  getConfig: () => Promise<RutileaConfigRecord>
  saveConfig: (config: RutileaConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.rutileaDesktop?.api) {
      return Promise.resolve({})
    }

    return getRutileaConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.rutileaDesktop?.api) {
    ...
    return saveRutileaConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: RutileaConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: RutileaConfigRecord, locale: Locale): RutileaConfigRecord {
