...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'rutilea:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.rutileaDesktop?.saveImageFromUrl) {
        if (await window.rutileaDesktop.saveImageFromUrl(src)) {
