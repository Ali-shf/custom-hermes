import type { ConnectionState } from '@rutilea/shared'
...
import type { SessionInfo, UsageStats } from '@/types/rutilea'
...
const COMPOSER_MODEL_KEY = 'rutilea.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'rutilea.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'rutilea.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'rutilea.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'rutilea.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'rutilea.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'rutilea.desktop.lastRoute'
...
function workspaceCwdKey(connection: RutileaConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.rutileaDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.rutileaDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<RutileaConnection | null>(null)
...
export const setConnection = (next: Updater<RutileaConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useRutileaConfig.
