...
const localConn = {
  ...
  mode: 'local',
  profile: 'default'
} as unknown as RutileaConnection
...
describe('connection-scoped sidebar lists (#77318)', () => {
  ...
  it('writes remote pins to a connection-scoped key, leaving the legacy key intact', () => {
    ...
    expect(readKey('rutilea.desktop.pinnedSessions')).toBe(JSON.stringify(['local-1']))

    // The remote pin landed under its own scope, not the shared key.
    const scoped = readKey(
      `rutilea.desktop.pinnedSessions.remote.${encodeURIComponent('https://vps-a.example:8443')}.default`
    )
