...
// reaches the gateway); everything else routes through window.rutileaDesktop.git,
...
function file(path: string, over: Partial<RutileaReviewFile> = {}): RutileaReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as RutileaReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.rutileaDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as RutileaReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.rutileaDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: RutileaReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as RutileaReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { rutileaDesktop?: unknown }).rutileaDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as RutileaReviewShipInfo)
