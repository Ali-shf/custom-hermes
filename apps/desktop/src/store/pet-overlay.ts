 ...
 * app's bounds and stays visible while Rutilea is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'rutilea.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'rutilea.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.rutileaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.rutileaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.rutileaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.rutileaDesktop?.petOverlay?.close()
}
