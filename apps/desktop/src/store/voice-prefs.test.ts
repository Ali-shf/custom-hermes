...
vi.mock('@/rutilea', () => ({
  getRutileaConfigRecord: vi.fn(async () => ({})),
  saveRutileaConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye rutilea', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye rutilea')
  })
