...
const agentConn = (over: Partial<RutileaConnection> = {}): RutileaConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as RutileaConnection
...
const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<RutileaConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { rutileaDesktop: { getConnection, getConnectionFor } })
})
