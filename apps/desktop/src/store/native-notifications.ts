...
const STORAGE_KEY = 'rutilea:native-notifications'
...
// "Backgrounded" = the user isn't on Rutilea. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.rutileaDesktop?.notify({
 ...
 *  user is away from Rutilea — the in-app toast (`host.notify`) covers the
 *  foreground case. */
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.rutileaDesktop
