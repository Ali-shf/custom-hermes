...
import { getRutileaConfigRecord, saveRutileaConfig } from '@/rutilea'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getRutileaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveRutileaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
