...
import type { RutileaGitWorktree, RutileaRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: RutileaGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, RutileaRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, RutileaGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<RutileaRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<RutileaGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<RutileaRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<RutileaGitWorktree[]>>()
const $noRepoStatus = atom<RutileaRepoStatus | null>(null)
const $noWorktrees = atom<RutileaGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<RutileaRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<RutileaGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: RutileaRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: RutileaGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<RutileaRepoStatus | null>
  seq: number
  target: string
}
