 ...
 * composer with the reply scrolling above it, so Rutilea can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.rutileaDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.rutileaDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.rutileaDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.rutileaDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
