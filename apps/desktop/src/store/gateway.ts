import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@rutilea/shared'
...
const isOpen = (gateway: RutileaGateway | null): boolean => gateway?.connectionState === 'open'

interface RegistryConfig {
  onEvent: (event: GatewayEvent) => void
  onActiveConnectionInvalidated?: (fallbackProfile: string, activationEpoch: number) => void
  onActiveConnectionChanged?: (connection: RutileaConnection) => void
}
...
interface Secondary {
  ...
  connection: RutileaConnection | null
  gateway: RutileaGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: RutileaGateway | null
  primaryProfile: string
  activeKey: string
  activationEpoch: number
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<RutileaGateway | null>>
}

const STATE_KEY = Symbol.for('rutilea.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<RutileaGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: RutileaGateway | null, profile = 'default'): void {
...
export function activeGateway(): RutileaGateway | null {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the rutilea module (null
...
function publishActiveConnection(connection: RutileaConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new RutileaGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: RutileaGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Rutilea gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.rutileaDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Rutilea Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<RutileaGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
