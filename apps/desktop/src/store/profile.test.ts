...
const { getProfiles } = await import('@/rutilea')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/rutilea/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<RutileaConnection> = {}): RutileaConnection =>
  ({ baseUrl: 'https://rutilea-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as RutileaConnection

const localConn = (over: Partial<RutileaConnection> = {}): RutileaConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as RutileaConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<RutileaConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { rutileaDesktop: { getConnection } })
