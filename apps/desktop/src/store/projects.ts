...
import type { RutileaGitBaseBranch, RutileaGitBranch } from '@/global'
...
import { getRutileaConfig, type RutileaGateway } from '@/rutilea'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/rutilea'
...
const PROJECT_SCOPE_KEY = 'rutilea.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Rutilea gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Rutilea profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: RutileaGateway): Promise<void> {
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await window.rutileaDesktop.api<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<RutileaGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<RutileaGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getRutileaConfig(context.profile))
...
// repo, returning where Rutilea should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<RutileaGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<RutileaGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.rutileaDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.rutileaDesktop?.writeClipboard?.(path)
  }
}
