 ...
 *
 * The archive is the CLI's own `rutilea profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/rutilea'
...
import type { ProfileDesktopOverlay } from '@/types/rutilea'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Rutilea profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.rutileaDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.rutileaDesktop?.selectPaths?.({
