...
/**
 * Feature store for backend (agent) plugins — the native Rutilea plugins plus
 ...
 * primitives `rutilea plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Rutilea. */
  portable?: boolean
}
 ...
 *  RUTILEA_HOME. Always refetches (it's a cheap local disk scan on the
