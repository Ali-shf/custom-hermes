...
export function setZoomPercent(percent: number): void {
  window.rutileaDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.rutileaDesktop?.zoom) {
  void window.rutileaDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.rutileaDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
