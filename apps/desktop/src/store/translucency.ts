 ...
 * the desktop shows through — see `@rutilea/shared/translucency`, which owns the
 ...
 * web contents to read (see the `[data-rutilea-glass]` block in styles.css).
 */
...
} from '@rutilea/shared/translucency'
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-rutilea-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-rutilea-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-rutilea-glass', glassOn)
  root.toggleAttribute('data-rutilea-clear', clearOn)

  if (glassOn) {
    root.setAttribute('data-rutilea-glass-scope', scope)
    root.style.setProperty('--translucency-glass-keep', `${glassSurfaceKeep(intensity)}%`)
  } else {
    root.removeAttribute('data-rutilea-glass-scope')
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.rutileaDesktop?.setTranslucency?.(state)
