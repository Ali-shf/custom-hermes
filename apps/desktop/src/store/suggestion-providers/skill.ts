 ...
 *  Working in `~/www/rutilea-agent`, the draft says "rutilea-agent" constantly
 *  — a "Use skill: rutilea-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`rutilea-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/rutilea-agent-dev`, `plugin:skill`). */
