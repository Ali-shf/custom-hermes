...
import type { RutileaReviewFile, RutileaReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Rutilea' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'rutilea.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'rutilea.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'rutilea.desktop.reviewTreeMode'
const SELECTED_KEY = 'rutilea.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<RutileaReviewFile[]>([])
...
export const $reviewShipInfo = atom<RutileaReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['rutileaDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: RutileaReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly RutileaReviewFile[], path: string): RutileaReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.rutileaDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.rutileaDesktop?.openExternal?.(url)
    }
