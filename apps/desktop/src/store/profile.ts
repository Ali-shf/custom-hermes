...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/rutilea'
...
import type { ProfileInfo } from '@/types/rutilea'
...
// /api/profiles/active `current`). "default" is the root ~/.rutilea. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'rutilea.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'rutilea.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await window.rutileaDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new RUTILEA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.rutileaDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.rutileaDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'rutilea.desktop.showAllProfiles'
...
// Switch to the default (root ~/.rutilea) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.rutileaDesktop?.touchBackend?.(target).catch(() => undefined)
}
