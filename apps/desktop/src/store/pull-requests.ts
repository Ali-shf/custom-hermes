...
import type { RutileaBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/rutilea'
...
export const $pullRequestsByBranch = atom<Record<string, RutileaBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'rutilea.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('rutilea.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: RutileaBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
