import type { GatewayWsUrlResult } from '@rutilea/shared'
import type { TranslucencyState } from '@rutilea/shared/translucency'
...
declare global {
  interface Window {
    rutileaDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<RutileaConnection>
      ...
      // Resume this session in the user's own terminal emulator (`rutilea --tui
      ...
      // Rutilea Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new RUTILEA_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: RutileaApiRequest) => Promise<T>
      notify: (payload: RutileaNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<RutileaReadFileTextResult>
      selectPaths: (options?: RutileaSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<RutileaPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<RutileaPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<RutileaPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: RutileaActiveWork) => void
      setTitleBarTheme?: (payload: RutileaTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<RutileaReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<RUTILEA_HOME>/desktop-plugins),
      ...
      // Local AGENT-plugin root (<RUTILEA_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<RutileaGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<RutileaGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<RutileaGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<RutileaRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: RutileaReviewScope, baseRef?: null | string) => Promise<RutileaReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: RutileaReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<RutileaReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<RutileaRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<RutileaPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: RutileaTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<RutileaTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: RutileaWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: RutileaPreviewFileChanged) => void) => () => void
...
export interface RutileaTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface RutileaTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `rutilea update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  rutileaRoot?: string
...
export interface RutileaConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface RutileaTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface RutileaActiveWork {
  count: number
  titles: string[]
}

export interface RutileaWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Rutilea Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Rutilea Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Rutilea Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the rutilea gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Rutilea Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface RutileaApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface RutileaNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface RutileaPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface RutileaReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface RutileaPreviewWatch {
  id: string
  path: string
}
...
export interface RutileaGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface RutileaGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface RutileaGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface RutileaRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface RutileaRepoStatus {
  branch: null | string
...
// turn began.
export type RutileaReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface RutileaReviewFile {
  path: string
  added: number
  removed: number
...
export interface RutileaReviewList {
  files: RutileaReviewFile[]
...
export interface RutileaReviewPr {
  url: string
  state: string
  number: number
}
...
export interface RutileaBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface RutileaRepoPullRequests {
  ghReady: boolean
  prs: RutileaBranchPullRequest[]
}
...
export interface RutileaPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface RutileaReviewShipInfo {
  ghReady: boolean
  pr: RutileaReviewPr | null
}

export interface RutileaReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface RutileaReadDirResult {
  entries: RutileaReadDirEntry[]
  error?: string
}

export interface RutileaPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface RutileaSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
