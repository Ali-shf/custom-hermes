  ...
  useI18n: () => ({
    t: {
      ...
      profiles: {
        ...
        connectGateway: 'Connect another Rutilea gateway…',
...
describe('ProfileRail multi-gateway entry point', () => {
  it('deep-links to the unified Settings → Gateways page from the rail', () => {
    ...
    const pill = screen.getByRole('button', { name: 'Connect another Rutilea gateway…' })
  ...
  it('keeps the entry point visible for single-profile users', () => {
    ...
    expect(screen.getByRole('button', { name: 'Connect another Rutilea gateway…' })).toBeTruthy()
