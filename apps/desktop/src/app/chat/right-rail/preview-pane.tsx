        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.rutileaDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Rutilea to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.rutileaDesktop?.watchPreviewFile ||
      !window.rutileaDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.rutileaDesktop.onPreviewFileChanged(payload => {
    ...
    void window.rutileaDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.rutileaDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.rutileaDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:rutilea-preview')
