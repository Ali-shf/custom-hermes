...
// bug that drives the REAL useGatewayBoot hook + REAL RutileaGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshRutileaConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { rutileaDesktop?: unknown }).rutileaDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { rutileaDesktop?: unknown }).rutileaDesktop
  window.localStorage.removeItem('rutilea.desktop.workspace-cwd')
  $currentCwd.set('')
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForRutilea) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startRutilea()'s remote branch awaits waitForRutilea() for 45s before it
    ...
    ;(window as { rutileaDesktop?: unknown }).rutileaDesktop = desktop
    ...
    // After ~45s waitForRutilea gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Rutilea backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Rutilea couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Rutilea')
    expect($currentCwd.get()).toBe('C:\\Rutilea')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startRutilea() rejects with a
