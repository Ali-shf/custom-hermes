...
import { RutileaGateway } from '@/rutilea'
import {
  $desktopBoot,
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.rutileaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: RutileaGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: RutileaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Rutilea...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Rutilea…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Rutilea gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshRutileaConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshRutileaConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startRutilea re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new RutileaGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshRutileaConfig(),
          ...
          // retry it), not brick boot behind the "Rutilea couldn't start"
