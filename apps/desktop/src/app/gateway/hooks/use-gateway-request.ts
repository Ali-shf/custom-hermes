...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as RutileaGateway | null
  const gatewayRef = useRef<RutileaGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.rutileaDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<RutileaGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as RutileaGateway | null
      }),
    []
  )
