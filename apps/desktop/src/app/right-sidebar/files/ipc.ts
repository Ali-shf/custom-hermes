...
import type { RutileaReadDirEntry, RutileaReadDirResult } from '@/global'
...
export type ProjectTreeEntry = RutileaReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: RutileaReadDirEntry) {
...
async function filterIgnored(entries: RutileaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<RutileaReadDirResult> {
  if (!window.rutileaDesktop) {
