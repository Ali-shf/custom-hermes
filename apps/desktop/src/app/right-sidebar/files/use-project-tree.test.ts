...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { rutileaDesktop: { readDir: typeof readDir } }).rutileaDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): RutileaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { rutileaDesktop: unknown }).rutileaDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: RutileaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<RutileaReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { rutileaDesktop: unknown }).rutileaDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.rutileaDesktop is missing', async () => {
    delete (window as unknown as { rutileaDesktop?: unknown }).rutileaDesktop
