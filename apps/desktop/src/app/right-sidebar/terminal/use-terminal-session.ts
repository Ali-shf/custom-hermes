...
const RUTILEA_PATHS_MIME = 'application/x-rutilea-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(RUTILEA_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(RUTILEA_PATHS_MIME)
  ...
  const getPath = window.rutileaDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.rutileaDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (rutilea --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.rutileaDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[rutilea-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.rutileaDesktop?.terminal?.write(sessionId, `${command}\r`)
