...
import { getRutileaConfigRecord, saveMcpServers } from '@/rutilea'
import { $mcpInstallRequest } from '@/store/mcp-deeplink-install'
import { notify, readableError } from '@/store/notifications'

import { setRutileaConfigCache } from '../hooks/use-config-record'
...
/**
 * Explicit-confirm gate for `rutilea://mcp/install` deep links. The payload is
 ...
 * confirming lets Rutilea spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getRutileaConfigRecord())
      ...
      setRutileaConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
