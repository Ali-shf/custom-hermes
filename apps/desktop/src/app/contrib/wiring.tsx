...
import { getLatestSessionMessages } from '@/rutilea'
...
import { useRutileaConfig } from '../session/hooks/use-rutilea-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshRutileaConfig, sttEnabled, voiceMaxRecordingSeconds } = useRutileaConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('rutilea:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('rutilea:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshRutileaConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshRutileaConfig])
