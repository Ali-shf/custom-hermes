...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.rutileaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.rutileaDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.rutileaDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.rutileaDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // rutilea:// deep links -> a reviewable /blueprint command in the composer,
  // or (rutilea://mcp/install) a pending MCP install awaiting explicit
  // confirmation in McpInstallDeepLinkDialog. Never auto-installs.
  useEffect(() => {
    const unsubscribe = window.rutileaDesktop?.onDeepLink?.(payload => {
      if (!payload) {
        return
      }
    ...
    void window.rutileaDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.rutileaDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.rutileaDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
