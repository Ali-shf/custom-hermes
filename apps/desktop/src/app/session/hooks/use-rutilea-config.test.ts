...
import { getRutileaConfig } from '@/rutilea'
...
import { useRutileaConfig } from './use-rutilea-config'

vi.mock('@/rutilea', () => ({
  getRutileaConfig: vi.fn(),
  getRutileaConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'rutilea.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getRutileaConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getRutileaConfig>>)

describe('useRutileaConfig refreshRutileaConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Rutilea' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useRutileaConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshRutileaConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useRutileaConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getRutileaConfig>>>()
    vi.mocked(getRutileaConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useRutileaConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshRutileaConfig(true)
    })
    expect(getRutileaConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getRutileaConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getRutileaConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getRutileaConfig>>>()
    vi.mocked(getRutileaConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshRutileaConfig(true)
      refreshC = result.current.refreshRutileaConfig(true)
    })
