import type { BillingBlock } from '@rutilea/shared'
import { registryBackendScopeKey } from '@rutilea/shared'
import type { RutileaSkin } from '@rutilea/shared/skin'
...
import type { RpcEvent } from '@/types/rutilea'
 ...
 * billing-specific toast — never the generic "Rutilea error" — with a smart CTA
...
interface GatewayEventDeps {
  ...
  refreshRutileaConfig: () => Promise<void>
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshRutileaConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: RutileaSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (Rutilea activating an authored skin, or `/skin`
        // on another surface). Only the active source+profile's change repaints.
        if (fromActiveSource()) {
          ingestBackendSkin(payload as RutileaSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshRutileaConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Rutilea reported an error',
        ...
        if (requestId) {
          const read = window.rutileaDesktop?.readWindowBelow
        ...
        // native OS notification when Rutilea is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Rutilea reported an error'
