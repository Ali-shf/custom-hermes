...
import { getRutileaConfig, getRutileaConfigDefaults } from '@/rutilea'
...
interface RutileaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useRutileaConfig({ activeSessionIdRef }: RutileaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshRutileaConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getRutileaConfig(), getRutileaConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Rutilea' built-in medium over the user's config.
  ...
  return { refreshRutileaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
