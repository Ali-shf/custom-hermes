...
import type { ConfigFieldSchema, RutileaConfigRecord, ToolsetInfo } from '@/types/rutilea'
...
export function getNested(obj: RutileaConfigRecord, path: string): unknown {
  let cur: unknown = obj
 ...
 */
export function clearsEnabledToolsets(prev: RutileaConfigRecord, next: RutileaConfigRecord): boolean {
...
export function setNested(obj: RutileaConfigRecord, path: string, value: unknown): RutileaConfigRecord {
...
function personalityOptions(config: RutileaConfigRecord): string[] {
...
function commandProviderNames(config: RutileaConfigRecord, section: 'tts' | 'stt'): string[] {
