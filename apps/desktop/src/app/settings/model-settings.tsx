...
import { rutileaConfigCacheWriter, invalidateRutileaConfig, useRutileaConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `rutilea model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in rutilea_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile = null }: ModelSettingsProps) {
  ...
  const { data: config } = useRutileaConfigRecord(scopeProfile)
  const setConfig = useMemo(() => rutileaConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateRutileaConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveRutileaConfig(next, scopeProfile ?? undefined)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `rutilea model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Rutilea runs the flow for you.`}
          </p>
        )}
