...
import { disconnectOAuthProvider, listOAuthProviders } from '@/rutilea'
import { notify, notifyError } from '@/store/notifications'
import { $desktopOnboarding, startManualLocalEndpoint, startManualProviderOAuth } from '@/store/onboarding'
import type { EnvVarInfo, OAuthProvider } from '@/types/rutilea'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.rutileaDesktop?.terminal)
...
//      in rutilea_cli/provider_catalog.py) — the SAME provider identity
//      `rutilea model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Rutilea can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Rutilea can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Rutilea never deletes creds another tool owns behind a silent API call.
