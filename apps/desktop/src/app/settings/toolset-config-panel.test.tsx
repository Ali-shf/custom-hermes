...
import type { ToolsetConfig } from '@/types/rutilea'
...
const getRutileaConfigSchema = vi.fn()
...
vi.mock('@/rutilea', () => ({
  ...
  getRutileaConfigSchema: () => getRutileaConfigSchema(),
...
beforeEach(() => {
  ...
  getRutileaConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getRutileaConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveRutileaConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveRutileaConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
