...
import { getElevenLabsVoices, getRutileaConfigSchema, saveRutileaConfig } from '@/rutilea'
import {
  $dataUrlReadMaxMb,
...
import type { ConfigFieldSchema, RutileaConfigRecord } from '@/types/rutilea'

import { rutileaConfigCacheWriter, useRutileaConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: RutileaConfigRecord): boolean {
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<RutileaConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useRutileaConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => rutileaConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['rutilea-config-schema'] : ['rutilea-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getRutileaConfigSchema(scopeProfile ?? undefined),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveRutileaConfig(config, scopeProfile ?? undefined)
  ...
  const updateConfig = (next: RutileaConfigRecord) => {
