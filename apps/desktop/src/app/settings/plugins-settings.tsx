...
import { getProfiles } from '@/rutilea'
...
// User-installed plugins first — mirrors `rutilea plugins list --user`.
...
// (~/.rutilea/plugins, git, project, pip entry points, portable packages).
...
// configured from their own surfaces (Settings → Models, Messaging, `rutilea
...
function reveal(file: string) {
  void window.rutileaDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // rutilea_home breaks against a remote backend (#66899).
    const dir = await window.rutileaDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.rutileaDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's rutilea home (profile-aware), so the
// path comes from the gateway — not from the renderer's local RUTILEA_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.rutileaDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Rutilea (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
