...
// Rutilea Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // --- Rutilea Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.rutileaDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.rutileaDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.rutileaDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteRutileaPath: state.sshRemoteRutileaPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.rutileaDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.rutileaDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'rutilea-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.rutileaDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.rutileaDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.rutileaDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.rutileaDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.rutileaDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.rutileaDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.rutileaDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.rutileaDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.rutileaDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.rutileaDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Rutilea Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/rutilea"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteRutileaPath: event.target.value }))}
                placeholder={g.sshRutileaPathPlaceholder}
                value={state.sshRemoteRutileaPath}
              />
            }
            description={g.sshRutileaPathDesc}
            title={g.sshRutileaPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.rutileaDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
