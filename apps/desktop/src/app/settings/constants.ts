...
// lib/personalities (mirrors rutilea_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__rutilea_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Rutilea Portal',
    description: 'Hosted Rutilea & Rutilea-trained models',
  ...
  { prefix: 'RUTILEA_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // rutilea_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Rutilea stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Rutilea can read from one file request.',
  approvals: {
    mode: 'How Rutilea handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Rutilea updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
