...
import type { RutileaConfigRecord, SessionInfo } from '@/types/rutilea'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveRutileaConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Rutilea wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.rutileaDesktop` is
    ...
    const settings = window.rutileaDesktop?.settings
