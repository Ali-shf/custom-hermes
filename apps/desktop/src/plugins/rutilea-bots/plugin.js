/**
 * Rutilea Bot Mode — a "one chat per agent" roster for the Rutilea desktop.
 *
 * Left pane "Bots": one row per Rutilea profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Rutilea cron jobs) scoped to the
 ...
 * bot-initiated sends use `rutilea -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@rutilea/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
async function saveBotMeta(name, patch) {
  ...
  try {
    ...
    serverRequest = Promise.resolve(host.request('profiles.configure', { name, ui_meta: { 'rutilea-bots': rest } }))
  } catch {
...
// handoffs (`rutilea -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      host
        ...
        .then(() => queryClient.invalidateQueries({ queryKey: ['rutilea-bots', 'roster'] }))
...
function mergeServerMeta(roster) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['rutilea-bots']
...
/** Permanently delete a bot's Rutilea profile, then remove plugin-local state
 ...
 * directory (rutilea-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('rutilea-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'rutilea-bots-roster-css'
  style.textContent =
    '.rutilea-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.rutilea-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes rutilea-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.rutilea-bots-pulse { animation: rutilea-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run rutilea mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.rutileaDesktop && window.rutileaDesktop.openExternal) {
        window.rutileaDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Rutilea), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `rutilea pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (rutilea-agent bot_mode_probe).
 ...
 *  click opens (rutilea-agent#88200). Unknown params are ignored by older
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (rutilea-agent #86875) also expose the union
 ...
 *  callable alias is 'rutilea' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'rutilea' : name
}
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'rutilea') {
      token = byForm.has('rutilea') ? 'rutilea' : token
    }
...
async function ensureRemoteCanonicalChat(route, profile) {
  ...
  try {
    ...
    pinned = owner?.ui_meta?.['rutilea-bots']?.chat || null
  } catch {
 ...
 *
 *  Identity rules (rutilea-agent#88200 — the row must open the session its
 *  preview describes):
...
async function prepareBotSource(bot, pinnedChat) {
  ...
  if (typeof host.ensureAgent !== 'function') {
    throw new Error('Update Rutilea Desktop to chat with agents on other connections.')
  }
  ...
  try {
    ...
    return owner?.ui_meta?.['rutilea-bots']?.chat || null
  } catch {
...
function displayName(bot, meta) {
  ...
  // "Rutilea". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `rutilea profile rename
  ...
  // that reads like nobody bothered. Present it as Rutilea (the agent it is)
...
// turn in its OWN persistent per-group Rutilea session and is fed only the
 ...
 *  literally named "default" — render it as Rutilea (matching displayName and
 *  the @rutilea handle) so the main agent never loses its name in rooms. */
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'rutilea -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `rutilea profile list` for the LIVE',
        ...
        children: [
          jsxs('div', {
            ...
            children: [
              ...
              activeNow
                ? jsx('span', {
                    className: 'rutilea-bots-pulse size-1.5 shrink-0 rounded-full bg-(--ui-accent,#4f9cf9)',
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Rutilea).'
    })
  }
  ...
  // (rutilea-agent#87317): Skills (installed list + one-click hub installs +
...
// {type: 'rutilea-skill-pick', ...} to us (rutilea-agent#86243). We validate
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'rutilea-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'rutilea-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Rutilea).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// rutilea -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `rutilea -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Rutilea-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Rutilea schedule string from picker state. */
...
async function openProfileSession(botName, storedId, gatewayGeneration) {
  ...
  if (typeof host.openSession !== 'function') {
    throw new Error('This Rutilea Desktop version cannot open stored sessions')
  }
...
function GroupChatWorkspace({ group, members, onBack }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Rutilea, remote dupes
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['rutilea-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Rutilea and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : filteredRoster.length === 0 && rosterRows.length === 0
              ...
              : jsx(ScrollArea, {
                  className: 'rutilea-bots-roster min-h-0 flex-1',
                  children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Rutilea profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @rutilea.
    ctx.register({
    ...
    if (!document.getElementById('rutilea-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'rutilea-bots-keyframes'
      style.textContent = '@keyframes rutilea-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
        ...
        handler: async draft => {
          ...
          if (!roster) {
            ...
            for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
              ...
              if (name === 'rutilea' && !names.includes('rutilea') && names.includes('default')) {
                name = 'default'
              }
          ...
          if (localMentions.length) {
            note +=
              ...
              localMentions.map(bot => '`rutilea -p ' + shellQuote(bot.name) + ' chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from 🤖 ' + shellDoubleQuote(senderName) + ' (@' + shellDoubleQuote(botHandle(live.name)) + '): <your composed message>"`').join('\n') +
          ...
          if (remoteMentions.length) {
            ...
            note +=
              ...
              ' over Connections in the background. Do not run rutilea -p for them and do not switch Gateway. Tell the user they were messaged here; when a reply lands, relay it attributed to that agent.]'
          }
