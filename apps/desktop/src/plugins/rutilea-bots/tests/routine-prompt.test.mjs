...
test('integration: a different active profile retains the delegated routine wrapper', () => {
  ...
  assert.match(prompt, /rutilea -p 'research' chat/)
...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('rutilea '), prompt.lastIndexOf('\n\nIf the command'))
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  const titleSentinel = `/tmp/rutilea-bot-mode-title-${process.pid}`
  const taskSentinel = `/tmp/rutilea-bot-mode-task-${process.pid}`
  ...
  const oldPrompt =
    ...
    `rutilea -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('rutilea '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `rutilea() { printf '%s\\037' "$@"; }\n${command}`
...
test('regression: Create Cronjob passes the active profile to routinePrompt', () => {
  ...
  assert.doesNotMatch(__routines.routinePrompt('ops', 'Check', instruction, 'ops'), /rutilea -p/)
})
