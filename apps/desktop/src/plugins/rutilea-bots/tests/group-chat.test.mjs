...
test('default profile speaks as Rutilea in room transcripts, not @default', () => {
  const gc = load(() => '(pass)')
  const line = gc.formatGroupChatLine({ from: { kind: 'member', name: 'default' }, text: 'hello room' }, 'builder')
  assert.equal(line, 'Rutilea: hello room')
...
test('turn prompt addresses the default profile as @rutilea', () => {
  ...
  assert.match(prompt, /You are @rutilea,/)
  ...
  assert.match(peerView, /group chat with @rutilea/)
})

test('mention routing: @rutilea resolves to the default member', () => {
  const gc = load(() => '(pass)')
  const members = [{ name: 'default', title: '' }, { name: 'builder', title: '' }]
  const parsed = gc.parseGroupChatMentions('@rutilea take a look', members)
...
test('source contract: workspace speaker labels use displayName with a click-to-reveal handle', () => {
  // Speaker labels come from the roster displayName (default → Rutilea)…
