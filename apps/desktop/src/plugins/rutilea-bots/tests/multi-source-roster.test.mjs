...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@rutilea\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Rutilea". Regression:
  ...
  assert.equal(name(active, undefined), 'Rutilea')
})

test('botHandle: precomputed multi-source handle wins; default stays rutilea', () => {
  ...
  assert.equal(botHandle('default'), 'rutilea')
})
...
test('displayName: local default stays Rutilea; remote default uses the device label', () => {
...
test('resolveRosterMentions: @rutilea in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'rutilea' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @rutilea and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
