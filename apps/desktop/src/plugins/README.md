...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `RutileaPlugin` and
...
in the companion
[`rutilea-example-plugins`](https://github.com/NousResearch/rutilea-example-plugins)
repo instead.

User- and agent-authored plugins load at runtime from
`$RUTILEA_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`rutilea-desktop-plugins` skill.
