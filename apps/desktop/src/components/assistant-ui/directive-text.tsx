...
const RUTILEA_REF_TYPES = WIRE_REFERENCE_KINDS
type RutileaRefType = (typeof RUTILEA_REF_TYPES)[number]
...
const RUTILEA_DIRECTIVE_RE = referenceRe()
...
export const rutileaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(RUTILEA_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...rutileaDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Rutilea directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.rutileaDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.rutileaDesktop?.readFileDataUrl(id)
