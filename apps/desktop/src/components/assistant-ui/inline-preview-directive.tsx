 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.rutilea.send(prompt)` (or declarative
 * `data-rutilea-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'rutilea-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'rutilea-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `rutilea.send(prompt)`.
 ...
 *  wires `data-rutilea-send` so declarative HTML works with zero script:
 *  `<button data-rutilea-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.rutilea={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-rutilea-send]"):null;' +
    'if(el)send(el.getAttribute("data-rutilea-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.rutileaDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
