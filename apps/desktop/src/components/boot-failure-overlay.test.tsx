...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Rutilea gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.rutileaDesktop
  Object.defineProperty(window, 'rutileaDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'rutileaDesktop', { configurable: true, value: original })
}
