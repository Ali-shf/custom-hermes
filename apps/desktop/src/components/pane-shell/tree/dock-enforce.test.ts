...
const TREE_KEY = 'rutilea.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'rutilea.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'rutilea.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['rutilea-bots:pane'], active: 'rutilea-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'rutilea-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'rutilea-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","rutilea-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['rutilea-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['rutilea-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'rutilea-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['rutilea-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'rutilea-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'rutilea-bots:pane'],
          active: 'rutilea-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
