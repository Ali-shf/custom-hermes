...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Rutilea doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in rutilea_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `rutilea model` uses — instead of
...
export interface RutileaConfig {
  agent?: {
...
export type RutileaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `rutilea model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Rutilea simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (rutilea.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors rutilea_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Rutilea, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `rutilea tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Rutilea); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/rutilea/update/check` — the backend's own update state.
