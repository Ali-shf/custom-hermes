/**
 * @rutilea/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__RUTILEA_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, type RutileaGateway } from '@/rutilea'
import {
  $gateway,
...
import type { UsageStats } from '@/types/rutilea'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Rutilea profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.rutileaDesktop?.getAgentRoster
...
export const host = {
  ...
  connections: async () => {
    const bridge = window.rutileaDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Rutilea Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.rutileaDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Rutilea Desktop.')
    }
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Rutilea Desktop connection routing unavailable')
    }
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Rutilea gateway unavailable')
    }
...
export type { RutileaGateway } from '@/rutilea'
...
/** Rutilea' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/rutilea'
