import { JsonRpcGatewayClient } from '@rutilea/shared'
...
// "Timed out connecting to Rutilea backend" that hangs the UI (#48504).
//
...
export class RutileaGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Rutilea gateway connection closed',
      connectErrorMessage: 'Could not connect to Rutilea gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Rutilea gateway is not connected',
...
// process to route them to that host (rutilea:api's registry branch). Null /
...
async function activeConnection(): Promise<RutileaConnection> {
  const getConnectionFor = window.rutileaDesktop.getConnectionFor

  if (_apiConnectionId && getConnectionFor) {
    return getConnectionFor({ connectionId: _apiConnectionId, profile: _apiProfile })
  }

  return window.rutileaDesktop.getConnection(_apiProfile)
}

/** Options for a plugin REST call — mirrors the app's own `rutileaDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see RutileaApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.rutileaDesktop?.api) {
    throw new Error('Rutilea desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return window.rutileaDesktop.api<T>({
...
export async function listSessions(
  ...
  const result = await window.rutileaDesktop.api<PaginatedSessions>({
    path:
...
// "Error invoking remote method 'rutilea:api': Error: 404: ..." through the
...
export function scanSessionPullRequests(
  ...
  return window.rutileaDesktop.api<{
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.rutileaDesktop.api<SidebarSessionsResponse>({
...
export function setSessionArchived(id: string, archived: boolean, profile?: string | null): Promise<{ ok: boolean }> {
  return window.rutileaDesktop.api<{ ok: boolean }>({
    ...(profile ? { profile } : {}),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.rutileaDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.rutileaDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(
  ...
  return window.rutileaDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.rutileaDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(profile?: null | string): Promise<ModelInfoResponse> {
  return window.rutileaDesktop.api<ModelInfoResponse>({
...
export function getStatus(): Promise<StatusResponse> {
  return window.rutileaDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.rutileaDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getRutileaConfig(profile?: string): Promise<RutileaConfig> {
  return window.rutileaDesktop.api<RutileaConfig>({
    ...profileScoped(profile),
    path: '/api/config',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
...
export function getRutileaConfigRecord(profile?: null | string): Promise<RutileaConfigRecord> {
  return window.rutileaDesktop.api<RutileaConfigRecord>({
    ...profileScoped(profile),
    path: '/api/config'
  })
}

export function getRutileaConfigDefaults(): Promise<RutileaConfigRecord> {
  return window.rutileaDesktop.api<RutileaConfigRecord>({
    ...profileScoped(),
...
export function getRutileaConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return window.rutileaDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveRutileaConfig(config: RutileaConfigRecord, profile?: null | string): Promise<{ ok: boolean }> {
  return window.rutileaDesktop.api<{ ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string, profile?: null | string): Promise<MemoryProviderConfig> {
  return window.rutileaDesktop.api<MemoryProviderConfig>({
    ...profileScoped(profile),
...
export function getEnvVars(profile?: null | string): Promise<Record<string, EnvVarInfo>> {
  return window.rutileaDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(profile),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.rutileaDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.rutileaDesktop.api<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.rutileaDesktop.api<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return window.rutileaDesktop.api<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function revealEnvVar(key: string, profile?: null | string): Promise<{ key: string; value: string }> {
  return window.rutileaDesktop.api<{ key: string; value: string }>({
    ...profileScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.rutileaDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.rutileaDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string, profile?: null | string): Promise<OAuthStartResponse> {
  return window.rutileaDesktop.api<OAuthStartResponse>({
    ...profileScoped(profile),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.rutileaDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(
  ...
  return window.rutileaDesktop.api<OAuthPollResponse>({
    ...profileScoped(profile),
...
export function startMemoryProviderOAuth(
  ...
  return window.rutileaDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(profile),
...
export function getSkills(profile?: null | string): Promise<SkillInfo[]> {
  return window.rutileaDesktop.api<SkillInfo[]>({
    ...profileScoped(profile),
    path: '/api/skills'
  })
}
...
export function getSkillContent(
  ...
  return window.rutileaDesktop.api<{ content: string; name: string; path: string }>({
    ...profileScoped(profile),
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.rutileaDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function getLearningNode(id: string, profile?: null | string): Promise<LearningNodeDetail> {
  return window.rutileaDesktop.api<LearningNodeDetail>({
    ...profileScoped(profile),
...
export function deleteLearningNode(id: string, profile?: null | string): Promise<{ message: string; ok: boolean }> {
  return window.rutileaDesktop.api<{ message: string; ok: boolean }>({
    ...profileScoped(profile),
...
export function setSkillEnabled(
  ...
  return window.rutileaDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...profileScoped(profile),
...
export function testMcpServer(name: string, profile?: null | string): Promise<McpTestResult> {
  return window.rutileaDesktop.api<McpTestResult>({
    ...profileScoped(profile),
 ...
 *  `saveRutileaConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string, profile?: null | string): Promise<McpOAuthFlow> {
  return window.rutileaDesktop.api<McpOAuthFlow>({
    ...profileScoped(profile),
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return window.rutileaDesktop.api<{ ok: boolean; status: string }>({
    ...profileScoped(profile),
    path: `/api/mcp/oauth/flows/${encodeURIComponent(flowId)}`,
    method: 'DELETE'
  })
...
export function getToolsets(profile?: null | string): Promise<ToolsetInfo[]> {
  return window.rutileaDesktop.api<ToolsetInfo[]>({
    ...profileScoped(profile),
...
export function getToolsetConfig(name: string, profile?: null | string): Promise<ToolsetConfig> {
  return window.rutileaDesktop.api<ToolsetConfig>({
    ...profileScoped(profile),
...
export function getToolsetModels(
  ...
  return window.rutileaDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(profile),
...
export function selectToolsetModel(
  ...
  return window.rutileaDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(profile),
...
export function selectToolsetProvider(
  ...
  return window.rutileaDesktop.api<SelectToolsetProviderResponse>({
    ...profileScoped(profile),
...
export function runToolsetPostSetup(
  ...
  return window.rutileaDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(profile),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.rutileaDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.rutileaDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.rutileaDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function grantComputerUsePermissions(): Promise<ActionResponse> {
  return window.rutileaDesktop.api<ActionResponse>({
    ...profileScoped(),
...
export function getMessagingPlatforms(profile?: null | string): Promise<MessagingPlatformsResponse> {
  return window.rutileaDesktop.api<MessagingPlatformsResponse>({
    ...profileScoped(profile),
...
export function updateMessagingPlatform(
  ...
  return window.rutileaDesktop.api<{ ok: boolean; platform: string }>({
    ...profileScoped(profile),
...
export function testMessagingPlatform(
  ...
  return window.rutileaDesktop.api<MessagingPlatformTestResponse>({
    ...profileScoped(profile),
...
export function getPairing(profile?: null | string): Promise<PairingResponse> {
  return window.rutileaDesktop.api<PairingResponse>({
    ...profileScoped(profile),
    path: '/api/pairing'
  })
}
...
export function approvePairing(
  ...
  return window.rutileaDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(profile),
...
export function getWebhooks(): Promise<WebhooksResponse> {
  return window.rutileaDesktop.api<WebhooksResponse>({
    ...profileScoped(),
    path: '/api/webhooks'
  })
}
...
export function enableWebhooks(): Promise<WebhookEnableResponse> {
  return window.rutileaDesktop.api<WebhookEnableResponse>({
    ...profileScoped(),
...
export function createWebhook(body: WebhookCreatePayload): Promise<WebhookCreateResponse> {
  return window.rutileaDesktop.api<WebhookCreateResponse>({
    ...profileScoped(),
    path: '/api/webhooks',
    method: 'POST',
    body
...
export function setWebhookEnabled(
  ...
  return window.rutileaDesktop.api<{ enabled: boolean; name: string; ok: boolean }>({
    ...profileScoped(),
...
// Cron jobs are stored per-profile (<RUTILEA_HOME>/cron/jobs.json), and the
...
export function getCronJobs(profile?: string): Promise<CronJob[]> {
  ...
  return window.rutileaDesktop.api<CronJob[]>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.rutileaDesktop.api<{ runs: SessionInfo[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await window.rutileaDesktop.api<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return window.rutileaDesktop.api<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.rutileaDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.rutileaDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.rutileaDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.rutileaDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.rutileaDesktop.api<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return window.rutileaDesktop.api<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return window.rutileaDesktop.api<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
...
export function getUsageAnalytics(days = 30, profile?: null | string): Promise<AnalyticsResponse> {
  return window.rutileaDesktop.api<AnalyticsResponse>({
    ...profileScoped(profile),
...
export function getGlobalModelOptions(
  ...
  return window.rutileaDesktop.api<ModelOptionsResponse>({
    ...profileScoped(profile),
...
// curation `rutilea model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(
  provider: string,
  profile?: null | string
): Promise<RecommendedDefaultModel> {
  return window.rutileaDesktop.api<RecommendedDefaultModel>({
    ...profileScoped(profile),
...
export function getAuxiliaryModels(profile?: null | string): Promise<AuxiliaryModelsResponse> {
  return window.rutileaDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(profile),
...
export function getMoaModels(profile?: null | string): Promise<MoaConfigResponse> {
  return window.rutileaDesktop.api<MoaConfigResponse>({
    ...profileScoped(profile),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(
  ...
  return window.rutileaDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(
  ...
  return window.rutileaDesktop.api<ModelAssignmentResponse>({
    ...profileScoped(profile),
    path: '/api/model/set',
    method: 'POST',
    body
...
export function updateRutilea(): Promise<ActionResponse> {
  return window.rutileaDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/rutilea/update',
    method: 'POST'
  })
}
...
export function checkRutileaUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.rutileaDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/rutilea/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: null | string): Promise<ActionStatusResponse> {
  return window.rutileaDesktop.api<ActionStatusResponse>({
    ...profileScoped(profile),
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.rutileaDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.rutileaDesktop.api<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(profile?: null | string): Promise<ElevenLabsVoicesResponse> {
  return window.rutileaDesktop.api<ElevenLabsVoicesResponse>({
...
// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `rutilea skills`
...
export function getSkillHubSources(profile?: null | string): Promise<SkillHubSourcesResponse> {
  return window.rutileaDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(profile),
...
export function searchSkillsHub(
  ...
  return window.rutileaDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(profile),
...
export function previewSkillHub(identifier: string, profile?: null | string): Promise<SkillHubPreview> {
  return window.rutileaDesktop.api<SkillHubPreview>({
    ...profileScoped(profile),
...
export function scanSkillHub(identifier: string, profile?: null | string): Promise<SkillHubScanResult> {
  return window.rutileaDesktop.api<SkillHubScanResult>({
    ...profileScoped(profile),
...
// `rutilea mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveRutileaConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.rutileaDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(body: {
  ...
  return window.rutileaDesktop.api<McpServerSummary>({
    ...profileScoped(),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: null | string): Promise<McpCatalogResponse> {
  return window.rutileaDesktop.api<McpCatalogResponse>({
    ...profileScoped(profile),
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return window.rutileaDesktop.api<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `rutilea memory` / `rutilea curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.rutileaDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.rutileaDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.rutileaDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.rutileaDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `rutilea doctor` / `rutilea security
// audit` / `rutilea backup` / `rutilea debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.rutileaDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.rutileaDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.rutileaDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.rutileaDesktop.api<DebugShareResponse>({
