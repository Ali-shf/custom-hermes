  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Rutilea is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Rutilea is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Rutilea's\\python.exe" }, 'inspect', [
    'C:\\x y\\rutilea.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'rutilea_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Rutilea''s/)
  assert.match(script, /C:\\x y\\rutilea\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { rutileaPath: lock.rutileaPath, rutileaHome: lock.rutileaHome }
