// Launching the Rutilea TUI in the user's OWN terminal emulator.
//
...
// `rutilea --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Rutilea runtime is often a venv Python invoked
//     as `python -m rutilea_cli.main`, not a `rutilea` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Rutilea
