...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rutilea-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'rutilea-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Rutilea Test'], { cwd: dir })
