...
const READY_RE = /^RUTILEA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitRutileaPath = '') {
  const explicit = psLiteral(explicitRutileaPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$rutileaHome=$env:RUTILEA_HOME',
    'if(-not $rutileaHome){$rutileaHome=Join-Path $env:LOCALAPPDATA "rutilea"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command rutilea.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $rutileaHome "rutilea-agent\\venv\\Scripts\\rutilea.exe")',
    '$candidates+=(Join-Path $HOME "rutilea-agent\\.venv\\Scripts\\rutilea.exe")',
    '$rutilea=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $rutilea){throw "Rutilea is not installed on the remote Windows host."}',
    'if($explicit -and $rutilea -ne $explicit){throw "The configured Rutilea path is not an executable file."}',
    '$python=Join-Path (Split-Path $rutilea) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Rutilea Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;rutileaHome=$rutileaHome;rutileaPath=$rutilea;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitRutileaPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitRutileaPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'rutilea_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.rutileaPath === 'string' &&
    typeof lock.rutileaHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.rutileaPath === runtime.rutileaPath &&
    lock.rutileaHome === runtime.rutileaHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteRutileaPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.rutileaPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Rutilea on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.rutileaPath = inspection.path
  const rutileaVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located rutilea at ${runtime.rutileaPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, rutileaPath: runtime.rutileaPath })
    )
  } catch (error) {
  ...
  const owned = {
    ...
    profile,
    rutileaPath: runtime.rutileaPath,
    rutileaHome: runtime.rutileaHome,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForRutilea(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Rutilea SSH"', 'powershell.exe -NoLogo')
