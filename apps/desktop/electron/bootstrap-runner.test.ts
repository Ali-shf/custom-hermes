...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'rutilea-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/rutilea-runner-test',
    sourceRepoRoot: null,
    rutileaHome: '/tmp/rutilea-runner-test',
    logRoot: '/tmp/rutilea-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'rutilea-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'rutilea-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/rutilea-agent', '--rutilea-home', '/tmp/rutilea', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/rutilea',
      rutileaHome: '/tmp/home'
    }),
    ['--dir', '/tmp/rutilea', '--rutilea-home', '/tmp/home', '--branch', 'main']
  )
})
