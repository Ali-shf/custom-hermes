...
function routeIdentity(route: StoredRoute): null | string {
  if (route.kind === 'ssh') {
    ...
    return stableValue({
      ...
      kind: 'ssh',
      port: ssh.port || 22,
      remoteRutileaPath: ssh.remoteRutileaPath || '',
...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'RUTILEA_DESKTOP_REMOTE_URL is set but RUTILEA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Rutilea backend.'
      )
    }
