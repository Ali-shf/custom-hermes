 ...
 * runtime, remote gateways, Rutilea Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@rutilea/shared`) — tsconfig project
...
/** Turn `ls ~/.rutilea/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Rutilea Cloud
 * instances are platform-managed — we never run `rutilea update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteRutileaPath: input.remoteRutileaPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Rutilea-managed names; an empty result stores nothing.
 ...
 * `remoteRutileaPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Rutilea Cloud' : 'Remote gateway'),
