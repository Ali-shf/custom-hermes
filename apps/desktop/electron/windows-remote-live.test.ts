...
//   RUTILEA_WIN_SSH_HOST   ssh alias/host of the Windows box
//   RUTILEA_WIN_SSH_USER   remote user
//   RUTILEA_WIN_SSH_RUTILEA absolute path to the remote rutilea.exe under test
const liveHost = process.env.RUTILEA_WIN_SSH_HOST || ''
const liveUser = process.env.RUTILEA_WIN_SSH_USER || ''
const configuredRutilea = process.env.RUTILEA_WIN_SSH_RUTILEA || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Rutilea-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredRutilea)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredRutilea.replace('rutilea.exe', 'python.exe')}' -m rutilea_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredRutilea.replace('rutilea.exe', 'python.exe')
        const terminate = `& '${python}' -m rutilea_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.rutileaPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m rutilea_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m rutilea_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
