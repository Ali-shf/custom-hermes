// window-below.ts — which OS window sits directly underneath a Rutilea window.
//
...
export function enumerationFailureNote(platform: string, env: NodeJS.ProcessEnv): string {
  ...
  if (env.HYPRLAND_INSTANCE_SIGNATURE) {
    return (
      ...
      'Check that `hyprctl clients` works from the same session Rutilea is ' +
      'running in.'
    )
  }
  ...
  if (wayland) {
    return (
      ...
      'X11/Xorg session, or run Rutilea under XWayland with DISPLAY set.'
    )
  }
 ...
 *
 * Walks past every window owned by our own process (all Rutilea windows share
...
export async function readWindowBelow(
  ...
  if (process.platform === 'darwin' && !titlesAvailable) {
    result.note =
      ...
      'Screen Recording permission, which Rutilea does not request for this.'
  }
