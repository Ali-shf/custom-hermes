 ...
 *     `X-Rutilea-Session-Token` header; WS uses `?token=`.
...
// rutilea_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - rutilea_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - rutilea_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / rutilea #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-rutilea_session_at', '__Secure-rutilea_session_at', 'rutilea_session_at']
const RT_COOKIE_VARIANTS = ['__Host-rutilea_session_rt', '__Secure-rutilea_session_rt', 'rutilea_session_rt']

// The Nous portal (NAS) does NOT use Rutilea gateway session cookies — it is a
...
// Keep this aligned with rutilea_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['rutilea', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Rutilea profile the remote SSH dashboard should actually run as.
...
// Rutilea Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteRutileaPath = String(entry.remoteRutileaPath || '').trim()

  if (remoteRutileaPath) {
    out.remoteRutileaPath = remoteRutileaPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Rutilea installation. Preserve an explicit mapping
  // when it is a valid Rutilea profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `RUTILEA_HOME` scopes it.
 *
 ...
 * and by the registry SSH branch of the `rutilea:api` handler — both routes
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://rutilea.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `rutilea:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a rutilea session ACCESS-token cookie
 ...
 * the Rutilea gateway session cookies, so cloud sign-in / discovery liveness
