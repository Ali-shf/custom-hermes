 ...
 *
 * The staged Tauri `rutilea-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `rutilea update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `rutilea update`, swaps/relaunches
 * the app, and writes .rutilea-update-result.json. With the app gone before
 * the update starts, the RUTILEA_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `rutilea update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into RUTILEA_HOME on *every* platform
 * (`rutilea-setup.exe` on Windows, `rutilea-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_rutilea_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `rutilea update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(rutileaHome, 'rutilea-setup.exe')
 ...
 *
 * `copy_self_to_rutilea_home` deliberately no-ops during `--update`
 ...
 * "Another Rutilea update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `rutilea update`, which
 ...
 * the installer is written to RUTILEA_HOME at install/repair time, making mtime
