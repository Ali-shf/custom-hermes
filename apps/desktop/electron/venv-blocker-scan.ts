...
const SCAN_MODULE = 'rutilea_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Rutilea processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'rutilea_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Rutilea process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Rutilea installation is free.\n' +
    '\n' +
    'Close other Rutilea windows and terminals, then retry.  If the problem\n' +
    'persists, run `rutilea update` in a terminal for detailed diagnostics.'
  )
}
