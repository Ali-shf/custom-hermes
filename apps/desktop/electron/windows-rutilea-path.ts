/**
 * windows-rutilea-path.ts
 *
 * Pure, dependency-injected pieces of Windows `rutilea` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvRutileaCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `rutilea` shim shadowed the real
 *      rutilea.cmd/rutilea.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\rutilea.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvRutileaCommand() — unwrapWindowsVenvRutileaCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `rutilea`) must
 * not shadow `rutilea.cmd`/`rutilea.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv rutilea shim, bootstrap-complete
 * marker) — gating on just the rutilea.exe console-script shim alone is the
...
export interface ResolveVenvRutileaCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportRutileaCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `rutilea`/`rutilea.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/rutilea(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m rutilea_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportRutileaCli(). A
 * venv whose update died mid-`pip install` still has python.exe + rutilea.exe
 ...
 *
 * Returns null when `command` is not a venv rutilea shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvRutileaCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvRutileaCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^rutilea(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Rutilea at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Rutilea Python at ${python}`,
    command: python,
