...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Rutilea Desktop cannot save remote gateway tokens. ' +
        ...
        'or set RUTILEA_DESKTOP_REMOTE_URL and RUTILEA_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set RUTILEA_DESKTOP_REMOTE_URL and RUTILEA_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
...
// RUTILEA_DESKTOP_REMOTE_* env vars.
