...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', RUTILEA_HOME: '/home/b/.rutilea' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'rutilea_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.rutilea/rutilea-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.rutilea/rutilea-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.rutilea/rutilea-agent'`,
      `exec '/home/b/.rutilea/rutilea-agent/venv/bin/python' '-m' 'rutilea_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\rutilea\\venv\\Scripts\\rutilea.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\rutilea\\venv\\Scripts\\rutilea.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
