export type ParentWatchdogEnv = {
  RUTILEA_PARENT_PID: string
  RUTILEA_PARENT_START_MARKER?: string
  RUTILEA_PARENT_NONCE?: string
}
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { RUTILEA_PARENT_PID: String(pid) }
  ...
  env.RUTILEA_PARENT_START_MARKER = startMarker
  env.RUTILEA_PARENT_NONCE = nonce
