 ...
 * console. It writes RUTILEA_HOME/.rutilea-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Rutilea within 30 minutes.
...
export function handoffResultPath(rutileaHome: string): string {
  return path.join(rutileaHome, '.rutilea-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(rutileaHome)
  let raw: string
