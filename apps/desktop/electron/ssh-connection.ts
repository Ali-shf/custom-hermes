...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(RUTILEA_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Rutilea-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.rutilea/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'rutilea-desktop-ssh')
  }

  return path.join(os.homedir(), '.rutilea', 'desktop-ssh')
}
