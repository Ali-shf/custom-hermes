...
export interface RutileaReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForRutileaReady`).
   */
...
export async function waitForRutileaReady(baseUrl: string, options: RutileaReadyOptions): Promise<void> {
  ...
  throw new Error(`Rutilea backend did not become ready: ${detail}`)
}
