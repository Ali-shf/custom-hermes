 ...
 *   1. RUTILEA_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\rutilea\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect RUTILEA_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.RUTILEA_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'rutilea', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'rutilea', 'git', 'usr', 'bin', 'bash.exe'))
  }
