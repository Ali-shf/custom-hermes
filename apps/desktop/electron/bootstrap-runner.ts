 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Rutilea Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.rutilea-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(rutileaHome) {
  return path.join(rutileaHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.rutilea/rutilea-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(rutileaHome) {
  if (!rutileaHome) {
    return null
  }

  const candidate = path.join(rutileaHome, 'rutilea-agent', 'scripts', installScriptName())
...
function cachedScriptPath(rutileaHome, commit) {
  return path.join(bootstrapCacheDir(rutileaHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Ali-shf/custom-hermes/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(rutileaHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(rutileaHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, rutileaHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass RUTILEA_HOME through so install.ps1 respects the caller's
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, rutileaHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, rutileaHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--rutilea-home', rutileaHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, rutileaHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, rutileaHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, rutileaHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(rutileaHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, rutileaHome, emit })
