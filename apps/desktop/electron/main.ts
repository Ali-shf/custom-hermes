...
const USER_DATA_OVERRIDE = process.env.RUTILEA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.RUTILEA_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.RUTILEA_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with RUTILEA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[rutilea] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[rutilea] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. RUTILEA_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[rutilea] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[rutilea] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via RUTILEA_DESKTOP_PASSWORD_STORE, bridged by the
// `rutilea desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[rutilea] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[rutilea] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `rutilea desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Rutilea sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[rutilea] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[rutilea] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[rutilea] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[rutilea] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[rutilea] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('rutilea:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Rutilea burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[rutilea] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[rutilea] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[rutilea] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[rutilea] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// RUTILEA_HOME — the user-facing root for everything Rutilea-related. Mirrors
// scripts/install.ps1's $RutileaHome and scripts/install.sh's $RUTILEA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\rutilea (matches install.ps1)
//   macOS / Linux: ~/.rutilea (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.rutilea directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\rutilea yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// RUTILEA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// RUTILEA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.rutilea / %LOCALAPPDATA%\rutilea.
function resolveRutileaHome() {
  if (process.env.RUTILEA_HOME) {
    return normalizeRutileaHomeRoot(process.env.RUTILEA_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'rutilea-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a RUTILEA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\rutilea and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('RUTILEA_HOME')

    if (fromRegistry) {
      return normalizeRutileaHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'rutilea')
    const legacy = path.join(app.getPath('home'), '.rutilea')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.rutilea setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.rutilea')
}

const RUTILEA_HOME = resolveRutileaHome()

function pathWithRutileaManagedNode(...entries) {
  const managed = rutileaManagedNodePathEntries(RUTILEA_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_RUTILEA_ROOT — the canonical mutable Rutilea install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_RUTILEA_ROOT = path.join(RUTILEA_HOME, 'rutilea-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_RUTILEA_ROOT, 'venv')
...
// boots, going straight to `resolveRutileaBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_RUTILEA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_RUTILEA_ROOT, '.rutilea-bootstrap-complete')
...
// active-profile.json records which Rutilea profile the desktop launches its
// local backend as. When set, startRutilea() passes `rutilea --profile <name>
// dashboard …`, which deterministically pins RUTILEA_HOME (see
// _apply_profile_override in rutilea_cli/main.py) and bypasses the sticky
// ~/.rutilea/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors rutilea_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// rutileaDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under RUTILEA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by rutilea_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(RUTILEA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (rutilea_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.RUTILEA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.RUTILEA_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.RUTILEA_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.RUTILEA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.RUTILEA_DESKTOP_APP_NAME || 'Rutilea'
...
// 'rutilea:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.rutilea) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.rutilea')
}

// Seed the native About panel with the live Rutilea version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `rutilea update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveRutileaVersion(),
...
// startRutilea(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.RUTILEA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.RUTILEA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startRutilea() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning rutilea serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Rutilea backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  rutileaLog.push(...lines)

  if (rutileaLog.length > 300) {
    rutileaLog.splice(0, rutileaLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-rutilea-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('rutilea:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the rutilea:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('rutilea:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_RUTILEA_ROOT
  })
}
...
// The Tauri updater writes RUTILEA_HOME/.rutilea-update-in-progress for the whole
...
// (`force_kill_other_rutilea`, taskkill /IM rutilea.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(RUTILEA_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Rutilea will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(RUTILEA_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Rutilea update did not finish',
        `${result.message}\n\nDetails: ${path.join(RUTILEA_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `rutilea` — must not shadow `rutilea.cmd`/`rutilea.exe`.
...
function unwrapWindowsVenvRutileaCommand(command, backendArgs) {
  return resolveVenvRutileaCommand(command, backendArgs, {
...
// spawns `rutilea serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `rutilea` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'rutilea_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `rutilea --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, RUTILEA_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveRutileaBackend step 4); execFileSync of a .cmd without
...
function isRutileaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'rutilea_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.RUTILEA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Rutilea deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the rutilea-import
...
// findGitBash — locate bash.exe on Windows. Resolves RUTILEA_GIT_BASH_PATH
...
// normal RUTILEA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\rutilea\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'rutilea', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'rutilea', 'git', 'bin', 'git.exe'))
  }
...
function recentRutileaLog() {
  return rutileaLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's rutilea root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_RUTILEA_ROOT.
// RUTILEA_DESKTOP_RUTILEA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.RUTILEA_DESKTOP_RUTILEA_ROOT && path.resolve(process.env.RUTILEA_DESKTOP_RUTILEA_ROOT),
    !IS_PACKAGED && isRutileaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isRutileaSourceRoot(ACTIVE_RUTILEA_ROOT) ? ACTIVE_RUTILEA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_RUTILEA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('rutilea:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'rutilea-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `rutilea update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(RUTILEA_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `rutilea update` can write
// fresh entry points. On Windows this is the file the running backend
// `rutilea.exe` holds open; on POSIX it's never mandatory-locked.
function venvRutileaShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'rutilea.exe')
    : path.join(updateRoot, 'venv', 'bin', 'rutilea')
}
...
// only signals the direct child, so on Windows a backend `rutilea.exe` that
// spawned its own grandchildren (a `rutilea` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    child.rutileaBackendIdentity = identity

    return identity
  } catch (error) {
    stopBackendChild(child)
    await waitForBackendExit(child)
    throw new Error(`Could not persist ownership for the Rutilea backend: ${error.message}`)
  }
}

function releaseBackendChild(child) {
  const identity = child?.rutileaBackendIdentity
...
// `rutileaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `rutilea.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a rutilea
// REPL, a pty terminal, the gateway) keep `rutilea.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const rutileaProcess = backendConnectionState.getProcess()

  stopBackendTreesForUpdate(rutileaProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvRutileaShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentRutileaProcess = backendConnectionState.getProcess()

    if (currentRutileaProcess && Number.isInteger(currentRutileaProcess.pid)) {
      stragglers.push(currentRutileaProcess.pid)
    }
...
// `rutilea update`). Instead we spawn the staged Rutilea-Setup binary with
// --update and quit, so it can run `rutilea update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `rutilea update` → relaunch),
      ...
      // in: the RUTILEA_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `rutilea update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `rutilea desktop`, never the Tauri installer that self-copies
      // rutilea-setup.exe into RUTILEA_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `rutilea` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `rutilea update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `rutilea update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, rutileaRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(RUTILEA_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Rutilea — this window will close and the updater will open. Don’t reopen Rutilea yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(RUTILEA_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // rutilea.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running rutilea, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Rutilea install open ' +
        '(a second Rutilea window or a terminal running rutilea?). Close it and retry.'
    ...
    // Rutilea processes running from this venv.  The updater normally refuses
    ...
    // `rutilea update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `rutilea update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          RUTILEA_HOME,
          PATH: pathWithRutileaManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `rutilea update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(RUTILEA_HOME, child.pid)
      }
      ...
      // abort with "Another Rutilea update is already running (PID <itself>)" —
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Rutilea will keep running — try again, or run \`rutilea update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvRutilea = path.join(venvBin, IS_WINDOWS ? 'rutilea.exe' : 'rutilea')
  ...
  // not just the `rutilea.exe` console-script shim. That shim is generated at the
...
// Python-level pre-update snapshot inside `rutilea update`.
function preflightStateDb(rutileaHome, rememberLog) {
  const stateDbPath = path.join(rutileaHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(rutileaHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(rutileaHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(rutileaHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `rutilea update`, swaps/relaunches the app bundle, and writes
// .rutilea-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'rutilea update', percent: null })

    return { ok: true, manual: true, command: 'rutilea update', rutileaRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...process.env,
      RUTILEA_HOME,
      PATH: pathWithRutileaManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Rutilea — this window will close. Don’t reopen Rutilea yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_RUTILEA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isRutileaSourceRoot(ACTIVE_RUTILEA_ROOT) &&
    fileExists(venvPython) &&
    canImportRutileaCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_RUTILEA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `rutilea
...
function resolveWebDist() {
  const override = process.env.RUTILEA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set RUTILEA_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: rutilea desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: rutilea desktop --force-build`
  )
...
function resolveRutileaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Rutilea.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.RUTILEA_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveRutileaCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'rutilea_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_RUTILEA_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Rutilea at ${ACTIVE_RUTILEA_ROOT}`,
    command,
    args: ['-m', 'rutilea_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      rutileaHome: RUTILEA_HOME,
      pythonPathEntries: [ACTIVE_RUTILEA_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_RUTILEA_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolveRutileaBackend(backendArgs) {
  ...
  //    installed `rutilea` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isRutileaSourceRoot.)
  if (!IS_PACKAGED && isRutileaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Rutilea source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_RUTILEA_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\rutilea\\rutilea-agent (Windows) or ~/.rutilea/rutilea-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Rutilea runtime at ${ACTIVE_RUTILEA_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `rutilea` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    RUTILEA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.RUTILEA_DESKTOP_IGNORE_EXISTING !== '1') {
    let rutileaCommand = null
    const rutileaOverride = process.env.RUTILEA_DESKTOP_RUTILEA

    if (rutileaOverride) {
      const resolvedOverride = findOnPath(rutileaOverride)

      if (resolvedOverride) {
        rutileaCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(rutileaOverride, { isWsl: IS_WSL })) {
        rutileaCommand = rutileaOverride
      } else {
        rememberLog(`Ignoring Windows Rutilea override under WSL: ${rutileaOverride}`)
      }
    } else {
      rutileaCommand = findOnPath('rutilea')
    }

    if (rutileaCommand) {
      if (looksLikeDesktopAppBinary(rutileaCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Rutilea CLI: ${rutileaCommand}`)
        rutileaCommand = null
      }
    }

    if (rutileaCommand) {
      const unwrapped = unwrapWindowsVenvRutileaCommand(rutileaCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `rutilea` shim
      ...
      const shellForProbe = isCommandScript(rutileaCommand)

      // RUTILEA_DESKTOP_RUTILEA is an explicit deployment override (used by
      ...
      if (shouldTrustRutileaOverride(rutileaOverride) || verifyRutileaCli(rutileaCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvRutileaCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Rutilea CLI at ${rutileaCommand}`,
          command: rutileaCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Rutilea CLI at ${rutileaCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed rutilea_cli module via system Python.
  ...
  if (python) {
    ...
    // having rutilea_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\rutilea\rutilea-agent\venv.
    if (canImportRutileaCli(python)) {
      return {
        kind: 'python',
        label: `installed rutilea_cli module via ${python}`,
        command: python,
        args: ['-m', 'rutilea_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: rutilea_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveRutileaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Rutilea Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_RUTILEA_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveRutileaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Rutilea install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Rutilea recovery was handed off to Rutilea Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      rutileaHome: RUTILEA_HOME,
      logRoot: path.join(RUTILEA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Rutilea install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Rutilea bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(RUTILEA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startRutilea() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // rutilea:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveRutileaBackend(backend.args))
  }
  ...
  if (!isRutileaSourceRoot(ACTIVE_RUTILEA_ROOT)) {
    throw new Error(
      `Rutilea install at ${ACTIVE_RUTILEA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Rutilea' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\rutilea\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `rutilea` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Rutilea on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Rutilea.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isRutileaSourceRoot()
    // plus an importable rutilea_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Rutilea venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Rutilea at ${ACTIVE_RUTILEA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Rutilea runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----rutilea-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Rutilea backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Rutilea backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Rutilea backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Rutilea-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('rutilea:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveRutileaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('rutilea:preview-file-changed', payload)
}
...
// waitForRutileaReady how to read a 401 — rejected session vs gated route.
...
async function waitForRutilea(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForRutileaReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('rutilea:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('rutilea:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('rutilea:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Rutilea...".
function sendPowerResume() {
  ...
  webContents.send('rutilea:power-resume')
}
...
// 'rutilea:power-battery' push below.
ipcMain.handle('rutilea:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('rutilea:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('rutilea:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('rutilea:window-state-changed', state)
}
...
//
// Hosted Rutilea gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``rutilea_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / rutilea #37247).
//     Both are set as HttpOnly cookies (``rutilea_session_at`` ~15 min,
//     ``rutilea_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:rutilea-remote-oauth'
...
// "not signed in", which on the initial boot path (startRutilea → the renderer's
// single-shot boot() with no retry) surfaced as the "Rutilea couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Rutilea
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Rutilea Cloud agent…' : 'Sign in to Rutilea gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Rutilea backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in rutilea_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Rutilea Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:rutilea-remote-oauth`:
...
// convention (rutilea_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Rutilea surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.RUTILEA_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Rutilea gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for rutilea_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Rutilea Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Rutilea Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Rutilea Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Rutilea Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Rutilea Cloud. Open Settings → Gateway, choose Rutilea Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Rutilea Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Rutilea Cloud session has expired. Sign in to Rutilea Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Rutilea Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every rutilea:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteRutileaPath`/`remoteProfile` —
...
// a valid stored value (pins the root RUTILEA_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.RUTILEA_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.RUTILEA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Rutilea Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteRutileaPath: (ssh || savedSsh)?.remoteRutileaPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by RUTILEA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Rutilea Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Rutilea Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteRutileaPath: input.sshRemoteRutileaPath ?? existingBlock.remoteRutileaPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (rutilea #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Rutilea gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Rutilea gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Rutilea gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.RUTILEA_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteRutileaPath || '')
    ...
    result = await lifecycle({
      ...
      remoteRutileaPath: sshConfig.remoteRutileaPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForRutilea: (baseUrl, token) => waitForRutilea(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    rutileaVersion: result.rutileaVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.rutileaVersion || 'rutilea (version unknown)'} at ${result.rutileaPath || '?'}`
  )
  ...
  return { ...connection, remoteRutileaVersion: result.rutileaVersion || '' }
}
...
//   2. env override (RUTILEA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.RUTILEA_DESKTOP_REMOTE_TOKEN,
      url: process.env.RUTILEA_DESKTOP_REMOTE_URL
    },
    profile,
...
// env → global) so a startRutilea() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Rutilea
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteRutileaPath: input.sshRemoteRutileaPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteRutileaPath || '')
          let rutileaPath
          let rutileaVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            rutileaPath = runtime.rutileaPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.rutileaPath])
            rutileaVersion = inspection.version
            supported = inspection.supported
          } else {
            rutileaPath = await remoteLifecycle.locateRutilea(ssh, sshConfig.remoteRutileaPath || '')
            rutileaVersion = await remoteLifecycle.probeRutileaVersion(ssh, rutileaPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, rutileaPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Rutilea on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteRutileaPath: rutileaPath,
            remoteRutileaVersion: rutileaVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startRutilea())
  ...
  // connect to Rutilea gateway". Mirror the renderer's connect here so the test
...
function resetRutileaConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const rutileaProcess = backendConnectionState.invalidate()
  stopBackendChild(rutileaProcess)
...
// startRutilea() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetRutileaConnection() invalidates it.
  const rutileaProcess = backendConnectionState.getProcess()
  const dying = rutileaProcess && !rutileaProcess.killed ? rutileaProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('rutilea:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('rutilea:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startRutilea()
  ...
  // spawning, so two children never share one profile's RUTILEA_HOME.
...
async function connectRegistryBackend(source, profile, key, poolEntry) {
  ...
  if (source.kind === 'ssh') {
    ...
    const sshConfig = normalizeSshConfig({
      ...
      remoteRutileaPath: source.remoteRutileaPath,
  ...
  await waitForRutilea(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
// local-spawn portion of startRutilea() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForRutilea(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited RUTILEA_HOME env (see _apply_profile_override
  // step 3 in rutilea_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveRutileaBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const rutileaCwd = resolveRutileaCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Rutilea backend for profile "${profile}" via ${backend.label}`)

  const parentStartMarker = await desktopParentStartMarker()
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(RUTILEA_HOME, 'profiles', key))
  )
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: rutileaCwd,
        RUTILEA_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        RUTILEA_WEB_DIST: webDist,
        ...(readyFile ? { RUTILEA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Rutilea backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Rutilea backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)

    if (!ready) {
      rejectStart?.(
        new Error(`Rutilea backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForRutilea(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Rutilea backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Rutilea backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_rutilea_home() recreates the deleted profile directory.
//
...
async function startRutilea() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Rutilea Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startRutilea() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Rutilea backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Rutilea backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Rutilea backend at ${remote.baseUrl}`, 24)
      await waitForRutilea(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Rutilea backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, rutileaLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.rutilea/active_profile file) and
    // resolves RUTILEA_HOME the same way `rutilea -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Rutilea runtime', 28)

        return resolveRutileaBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Rutilea backend via ${backend.label}`, 84)
    rememberLog(`Starting Rutilea backend via ${backend.label}`)
    ...
    const rutileaProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin RUTILEA_HOME for the child so Python's get_rutilea_home()
          // resolves to the SAME location our resolveRutileaHome() picked. Without
          // this pin, Python falls back to ~/.rutilea on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\rutilea, which differs from C:\Users\<u>\.rutilea.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets RUTILEA_HOME via setx; the desktop
    ...
    await claimBackendChild(rutileaProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, rutileaProcess)

    if (!processOwner) {
      stopBackendChild(rutileaProcess)
      await waitForBackendExit(rutileaProcess)
      releaseBackendChild(rutileaProcess)
      throw new Error('Rutilea backend start was superseded by a newer connection attempt.')
    }

    rutileaProcess.stdout.on('data', rememberLog)
    rutileaProcess.stderr.on('data', rememberLog)
    ...
    rutileaProcess.once('error', error => {
      releaseBackendChild(rutileaProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Rutilea backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Rutilea backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Rutilea backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Rutilea backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    rutileaProcess.once('exit', (code, signal) => {
      releaseBackendChild(rutileaProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Rutilea backend exit (${signal || code})`)
      ...
      rememberLog(`Rutilea backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Rutilea backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Rutilea backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentRutileaLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Rutilea backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(rutileaProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Rutilea backend to become ready', 90)
    await Promise.race([waitForRutilea(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => rutileaProcess.exitCode === null && !rutileaProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Rutilea backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Rutilea backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Rutilea is
...
// pushes pet state over IPC (rutilea:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via rutilea:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Rutilea app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Rutilea icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see rutilea:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Rutilea in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('rutilea:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Rutilea can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('rutilea:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('rutilea:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `rutilea:hud:set-bounds`, which flips resizable on for the call — the
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('rutilea:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('rutilea:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('rutilea:quick-entry:shown')
}
...
function createWindow() {
  ...
  startRutilea().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('rutilea:connection', async (_event, profile) => ensureBackend(profile))
...
ipcMain.handle('rutilea:connection:for', async (_event, payload) => {
...
// "Starting Rutilea…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('rutilea:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetRutileaConnection,
...
ipcMain.handle('rutilea:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('rutilea:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('rutilea:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('rutilea:window:openInstance', async () => {
...
ipcMain.handle('rutilea:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveRutileaBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Rutilea is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `rutilea-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, RUTILEA_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('rutilea:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('rutilea:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('rutilea:zoom:get', event => {
...
ipcMain.on('rutilea:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('rutilea:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('rutilea:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('rutilea:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('rutilea:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('rutilea:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('rutilea:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('rutilea:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('rutilea:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('rutilea:pet-overlay:control', payload)
})

// --- HUD mode (chrome-free floating chat) -----------------------------------
ipcMain.handle('rutilea:hud:open', async (_event, request) => {
  openHudWindow(
...
ipcMain.handle('rutilea:hud:vibrancy', (_event, on) => {
...
// reaches the bar.
ipcMain.on('rutilea:hud:ignore-mouse', (_event, ignore) => {
  if (hudWindow && !hudWindow.isDestroyed()) {
    hudWindow.setIgnoreMouseEvents(Boolean(ignore), { forward: true })
  }
})

ipcMain.on('rutilea:hud:move-by', (event, delta) => {
...
ipcMain.on('rutilea:hud:set-bounds', (event, bounds) => {
...
ipcMain.on('rutilea:hud:session', (event, sessionId) => {
  if (hudWindow && !hudWindow.isDestroyed() && event.sender === hudWindow.webContents) {
    hudSessionId = typeof sessionId === 'string' && sessionId ? sessionId : null
  }
})

ipcMain.handle('rutilea:hud:close', async () => {
  closeHudWindow()

  return { ok: true }
})
ipcMain.handle('rutilea:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startRutilea() call restarts the
...
ipcMain.handle('rutilea:bootstrap:repair', async () => {
  // Forceful repair: force the next startRutilea() through the full installer
  ...
  // "bypass active runtime" path inside startRutilea() and fall through
  ...
  resetRutileaConnection()

  return { ok: true }
})
ipcMain.handle('rutilea:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('rutilea:bootstrap:cancel', async () => {
...
ipcMain.handle('rutilea:boot-progress:get', async () => bootProgressState)
ipcMain.handle('rutilea:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('rutilea:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('rutilea:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('rutilea:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('rutilea:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('rutilea:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('rutilea:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('rutilea:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('rutilea:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('rutilea:connections:set-primary', async (_event, id) => {
  const registry = setPrimaryConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('rutilea:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteRutileaPath: entry.remoteRutileaPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startRutilea()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteRutileaPath: connection.remoteRutileaPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteRutileaProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root RUTILEA_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('rutilea:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// rutilea:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
ipcMain.handle('rutilea:gateway:ws-url-for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
...
// Fan out `rutilea update` to every eligible registered connection at once.
...
// /api/rutilea/update endpoint (the dashboard updater), which runs
// `rutilea update` on THAT machine.
ipcMain.handle('rutilea:connections:update-all', async () => {
  const registry = readDesktopConnectionsRegistry()
  ...
  const results = await Promise.all(
    registry.connections.map(async connection => {
      ...
      try {
        ...
        const body: any = await postJsonForBackend(descriptor, '/api/rutilea/update', {}, { timeoutMs: 15_000 })
...
// analogue of the rutilea:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('rutilea:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('rutilea:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startRutilea() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('rutilea:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Rutilea Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('rutilea:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('rutilea:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('rutilea:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('rutilea:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('rutilea:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new RUTILEA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('rutilea:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('rutilea:requestMicrophoneAccess', async () => {
...
ipcMain.handle('rutilea:window:readBelow', async event => {
...
async function handleRutileaApiRequest(request) {
  ...
  // backend calls ensure_rutilea_home() which recreates the profile directory,
...
ipcMain.handle('rutilea:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its RUTILEA_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleRutileaApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleRutileaApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('rutilea:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('rutilea:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('rutilea:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('rutilea:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('rutilea:data-url-read-max:get', () => ({
...
ipcMain.handle('rutilea:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('rutilea:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('rutilea:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('rutilea:readFileText', async (_event, filePath) => {
...
ipcMain.handle('rutilea:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('rutilea:writeClipboard', (_event, text) => {
...
ipcMain.handle('rutilea:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('rutilea:readClipboard', () => clipboard.readText())

ipcMain.handle('rutilea:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('rutilea:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('rutilea:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('rutilea:saveClipboardImage', async () => {
...
ipcMain.handle('rutilea:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('rutilea:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('rutilea:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('rutilea:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('rutilea:active-work', (event, payload) => {
...
ipcMain.on('rutilea:titlebar-theme', (_event, payload) => {
...
ipcMain.on('rutilea:native-theme', (_event, mode) => {
...
ipcMain.on('rutilea:translucency', (_event, payload) => {
...
ipcMain.on('rutilea:keep-awake', (_event, on) => {
...
ipcMain.handle('rutilea:quick-entry:settings:get', async () => {
...
ipcMain.handle('rutilea:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('rutilea:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('rutilea:quick-entry:submit', {
...
ipcMain.on('rutilea:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('rutilea:quick-entry:state', payload)
  }
})

ipcMain.on('rutilea:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('rutilea:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('rutilea:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `rutilea:window:openSession`).
...
ipcMain.handle('rutilea:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('rutilea:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('rutilea:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveRutileaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('rutilea:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveRutileaCwd()
}))

ipcMain.handle('rutilea:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('rutilea:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('rutilea:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('rutilea:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('rutilea:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('rutilea:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: rutileaLog.slice(-200) }))
...
ipcMain.on('rutilea:logs:renderer-error', (_event, report) => {
...
function terminalShellCommand() {
  // RUTILEA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.RUTILEA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Rutilea' TUI into
  ...
  env.TERM_PROGRAM = 'Rutilea'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a rutilea/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from RUTILEA_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.RUTILEA_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `rutilea:terminal:${id}:${suffix}`
}
...
ipcMain.handle('rutilea:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('rutilea:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('rutilea:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('rutilea:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<RUTILEA_HOME>/desktop-plugins`,
// resolved from the main-process RUTILEA_HOME (see resolveRutileaHome) — NOT from
// the connected backend. A remote backend reports its own `rutilea_home` over
...
async function localPluginsRoot(dirName: string): Promise<string> {
  ...
  // profiles/<name>/, matching the profile-scoped rutilea_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(RUTILEA_HOME, 'profiles', profile) : RUTILEA_HOME
...
ipcMain.handle('rutilea:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

// The LOCAL agent-plugin root (`<RUTILEA_HOME>/plugins`), same Electron-local
...
ipcMain.handle('rutilea:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

// Rename a file/folder in place. The renderer passes the existing path + a new
// base name; the destination is resolved in the SAME parent dir so a rename can
// never move the item elsewhere or traverse out. Rejects on a name collision.
ipcMain.handle('rutilea:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('rutilea:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('rutilea:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('rutilea:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('rutilea:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('rutilea:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('rutilea:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('rutilea:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('rutilea:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('rutilea:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('rutilea:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('rutilea:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('rutilea:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('rutilea:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('rutilea:git:review:prList', async (_event, repoPath, branches, numbers) =>
  reviewPrList(repoPath, resolveGhBinary(), branches, numbers)
)
ipcMain.handle('rutilea:git:review:fetchPrComment', async (_event, repoPath, url) =>
  reviewFetchPrComment(repoPath, resolveGhBinary(), url)
)
ipcMain.handle('rutilea:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('rutilea:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('rutilea:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('rutilea:terminal:write', (_event, id, data) => {
...
ipcMain.handle('rutilea:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('rutilea:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('rutilea:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('rutilea:updates:check', async () =>
...
ipcMain.handle('rutilea:updates:apply', async (_event, payload) =>
...
ipcMain.handle('rutilea:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('rutilea:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Rutilea version (the one `release.py` bumps in
// rutilea_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Rutilea version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveRutileaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'rutilea_cli', '__init__.py')
...
// Renderer-bundle skew: `rutilea update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Rutilea version and push it into the native About panel
// just before showing it, so an in-place `rutilea update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveRutileaVersion()} — app build out of date, update the desktop app`
        : resolveRutileaVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('rutilea:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveRutileaVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    rutileaRoot: resolveUpdateRoot(),
...
// the actual removal via `rutilea uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (rutilea_cli/uninstall.py + rutilea_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_RUTILEA_ROOT
  ...
  const fallback = () => ({
    rutilea_home: RUTILEA_HOME,
    agent_installed: isRutileaSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'rutilea_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, RUTILEA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Rutilea agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import rutilea_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_RUTILEA_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_RUTILEA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `rutilea-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `rutilea-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('rutilea:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('rutilea:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('rutilea:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('rutilea:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// rutilea:// deep links (e.g. rutilea://blueprint/morning-brief?time=08:00, or
// rutilea://mcp/install?name=NAME&config=B64 — the vendor "Add to Rutilea"
...
// ---------------------------------------------------------------------------
const RUTILEA_PROTOCOL = 'rutilea'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${RUTILEA_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // rutilea://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('rutilea:deep-link', payload)
...
ipcMain.handle('rutilea:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${RUTILEA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(RUTILEA_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(RUTILEA_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `rutilea://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startRutilea), whose
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching rutilea:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Rutilea never keeps another app's chord hostage.
