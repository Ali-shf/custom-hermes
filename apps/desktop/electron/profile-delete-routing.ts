// Profile-delete routing logic for the `rutilea:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_rutilea_home() and
...
/** Parse a `rutilea:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its RUTILEA_HOME.
 *
...
/**
 * Route the next `rutilea:api` request away from the primary/window backend
 ...
 * ensure_rutilea_home() recreates the directory the delete just removed.
 */
