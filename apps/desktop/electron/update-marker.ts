...
export function markerPath(rutileaHome) {
  return path.join(rutileaHome, '.rutilea-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(rutileaHome)
  let raw
 ...
 *
 * The Tauri-based rutilea-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(rutileaHome, pid, { now = Date.now } = {}) {
  const file = markerPath(rutileaHome)
 ...
 * Rutilea to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(rutileaHome, opts)
