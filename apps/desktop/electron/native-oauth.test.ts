...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/rutilea', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/rutilea/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/rutilea'), 'https://gw.example.com/rutilea/auth/native/refresh')
})
