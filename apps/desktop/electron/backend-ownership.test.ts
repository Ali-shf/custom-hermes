...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'rutilea serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/rutilea serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m rutilea_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/rutilea --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Rutilea Runtime\\rutilea.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('rutilea chat --query serve'), false)
