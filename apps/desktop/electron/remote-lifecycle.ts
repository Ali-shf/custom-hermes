 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Rutilea dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Rutilea install on the remote (login-shell probe),
 ...
 *     `RUTILEA_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForRutilea in.
 *
 * The minted RUTILEA_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^RUTILEA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.rutilea/desktop-ssh'
...
// Resolve the remote rutilea executable. An EXPLICIT path is honored strictly
...
async function locateRutilea(ssh, remoteRutileaPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The rutilea binary or wrapper script
  ...
  if (remoteRutileaPath) {
    if (await isExecutable(remoteRutileaPath)) {
      return resolveLauncher(remoteRutileaPath)
    }

    const err: any = new Error(
      `The Rutilea path you set is not an executable on the remote host: "${remoteRutileaPath}". ` +
        'Check the path (it must be the full path to the `rutilea` binary on the remote, e.g. ' +
        '~/rutilea-agent/.venv/bin/rutilea), or clear it to auto-detect.'
    )

    err.kind = 'rutilea-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v rutilea')}`)).trim()
  ...
  candidates.push('~/.local/bin/rutilea')
  candidates.push('/usr/local/bin/rutilea')
  candidates.push('~/.rutilea/rutilea-agent/venv/bin/rutilea')
  ...
  const err: any = new Error(
    'Rutilea is not installed on the remote host (could not find a `rutilea` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Rutilea path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<rutilea> --version`,
// e.g. "Rutilea Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH rutilea a
// connection uses, so a stale/unexpected install is visible.
async function probeRutileaVersion(ssh, rutileaPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(rutileaPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Rutilea Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The RUTILEA_HOME the remote dashboard will use (explicit env wins, else
// ~/.rutilea). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteRutileaHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${RUTILEA_HOME:-$HOME/.rutilea}"')).trim().split('\n').pop()

    return out || '~/.rutilea'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Rutilea home.')
...
async function listRemoteRutileaProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteRutileaHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Rutilea profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Rutilea home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'rutileaPath', 'rutileaHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, rutileaPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !rutileaPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(rutileaPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.rutileaPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(rutileaPath, profile, opts: any = {}) {
  const rutilea = expandRemotePath(rutileaPath)
  ...
  const dashCmd =
    ...
    `exec env RUTILEA_DESKTOP=1 ${rutilea} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, rutileaPath) {
  const rutilea = expandRemotePath(rutileaPath)

  const out = await ssh.exec(
    `help="$(${rutilea} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { rutileaPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, rutileaPath))) {
    const err: any = new Error(
      'The remote Rutilea install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Rutilea on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(rutileaPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForRutilea, a token-gated
...
async function connect(deps) {
  ...
  const rutileaPath = await locateRutilea(ssh, remoteRutileaPath)
  log(`located rutilea at ${rutileaPath}`)
  const rutileaVersion = await probeRutileaVersion(ssh, rutileaPath)

  if (rutileaVersion) {
    log(`remote rutilea version: ${rutileaVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const rutileaHome = await probeRemoteRutileaHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.rutileaPath))
    ...
    const reusable =
      ...
      lock.rutileaPath === rutileaPath &&
      lock.rutileaHome === rutileaHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForRutilea(baseUrl, spawnToken)
    assertNotAborted(signal)
