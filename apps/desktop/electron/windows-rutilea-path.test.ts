// Unit tests for the pure Windows `rutilea` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvRutileaCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `rutilea` shim shadows
//      the real rutilea.cmd/rutilea.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the rutilea.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvRutileaCommand() — must probe the venv python via
//      canImportRutileaCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvRutileaCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportRutileaCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    rutileaHome: '/fake/rutilea-home',
...
test('resolveVenvRutileaCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvRutileaCommand('/root/venv/Scripts/rutilea.exe', [], deps), null)
})

test('resolveVenvRutileaCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvRutileaCommand('/root/venv/Scripts/rutilea.cmd', [], deps), null)
})

test('resolveVenvRutileaCommand: returns null when the basename is not rutilea/rutilea.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvRutileaCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvRutileaCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvRutileaCommand('/root/venv/bin/rutilea.exe', [], deps), null)
})

test('resolveVenvRutileaCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvRutileaCommand('/root/venv/Scripts/rutilea.exe', [], deps), null)
})

test('resolveVenvRutileaCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportRutileaCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvRutileaCommand('/root/venv/Scripts/rutilea.exe', ['serve'], deps)
...
test('resolveVenvRutileaCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvRutileaCommand('/root/venv/Scripts/rutilea.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'rutilea_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvRutileaCommand: is case-insensitive on rutilea.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvRutileaCommand('/root/venv/Scripts/RUTILEA.EXE', [], deps))
  assert.ok(resolveVenvRutileaCommand('/root/venv/SCRIPTS/rutilea.exe', [], deps))
})
