// Backend subcommand routing for the desktop-managed Rutilea process.
//
// The desktop app launches its own headless backend via `rutilea serve` — it
...
// app hasn't updated yet, or an older `rutilea` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Rutilea profile to pin via `--profile`.
 */
 ...
 * `-m rutilea_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `rutilea_cli/subcommands/dashboard.py` source registers
