 ...
 * the three user-facing uninstall modes to the `rutilea uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m rutilea_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `rutilea_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'rutilea_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Rutilea.app/Contents/MacOS/Rutilea  → …/Rutilea.app
 *   Windows: …\Rutilea\Rutilea.exe                 → …\Rutilea  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Rutilea.app/Contents/MacOS/Rutilea → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Rutilea.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Rutilea.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Rutilea$/i.test(dir) || /[\\/]rutilea-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/rutilea
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import rutilea_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, rutileaHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export RUTILEA_HOME=${q(rutileaHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import rutilea_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Rutilea install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "RUTILEA_HOME=${String(rutileaHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
