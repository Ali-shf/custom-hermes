...
/**
 * Rutilea-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\rutilea\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$RUTILEA_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_rutilea_node_dirs()` in rutilea_constants.py, which the Electron main
 * process cannot import.
 */
function rutileaManagedNodePathEntries(
  rutileaHome,
  ...
  const root = pathModule.join(rutileaHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const rutileaNodeDirs = rutileaManagedNodePathEntries(rutileaHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([rutileaNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeRutileaHomeRoot(rutileaHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!rutileaHome) {
    return rutileaHome
  }

  const resolved = pathModule.resolve(String(rutileaHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). rutilea_bootstrap sets this inside the child too, but only
