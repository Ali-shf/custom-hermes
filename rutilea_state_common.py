...
Extracted verbatim from rutilea_state.py so the SessionDB mixin modules
(rutilea_state_search / rutilea_state_schema / rutilea_state_portability) can
reference them without importing rutilea_state (which would be a cycle).
rutilea_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``rutilea sessions optimize-storage``. A legacy DB sits at
...
# `rutilea sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `rutilea db optimize`. This is
