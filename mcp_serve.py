"""
Rutilea MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Rutilea-specific extra)

Usage:
    rutilea mcp serve
    rutilea mcp serve --verbose
...
logger = logging.getLogger("rutilea.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using RUTILEA_HOME."""
    try:
        from rutilea_constants import get_rutilea_home
        return get_rutilea_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("RUTILEA_HOME", Path.home() / ".rutilea")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from rutilea_constants import get_rutilea_home
        directory_file = get_rutilea_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("RUTILEA_HOME", Path.home() / ".rutilea")
...
class EventBridge:
    ...
    This is the Rutilea equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from rutilea_constants import get_rutilea_home
            db_file = get_rutilea_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("RUTILEA_HOME", Path.home() / ".rutilea")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Rutilea MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "rutilea",
        instructions=(
            "Rutilea Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Rutilea MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
