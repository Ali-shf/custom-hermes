...
Uses a file-based lock (~/.rutilea/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `rutilea update` reloads
# the module) fail with ModuleNotFoundError for rutilea_time et al.
...
from rutilea_time import now as _rutilea_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `rutilea fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `rutilea cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Rutilea, pin it explicitly: "
                f"`rutilea cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy rutilea_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from rutilea_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``rutilea tools`` config for the ``cron`` platform.
    ...
    try:
        from rutilea_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``RUTILEA_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("RUTILEA_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid RUTILEA_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _rutilea_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_rutilea_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_rutilea_home())
...
# (RUTILEA_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse RUTILEA_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("RUTILEA_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid RUTILEA_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_rutilea_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_rutilea_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.rutilea/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``rutilea update`` / ``rutilea gateway stop`` / systemd restart, or an
...
_rutilea_home: Path | None = None


def _get_rutilea_home() -> Path:
    """Resolve Rutilea home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active RUTILEA_HOME at call time
    ...
    """
    return _rutilea_home or get_rutilea_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    rutilea_home = _get_rutilea_home()
    lock_dir = rutilea_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Rutilea — {task_name}"
    try:
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # RUTILEA_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `rutilea cron run` from the CLI,
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("RUTILEA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid RUTILEA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    RUTILEA_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("RUTILEA_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid RUTILEA_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    Scripts must reside within RUTILEA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Rutilea-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against RUTILEA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_rutilea_home() / "scripts"
    ...
    # escape — scripts MUST reside within RUTILEA_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("RUTILEA_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `rutilea setup`), or pin a "
            "working provider via `rutilea cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`rutilea setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        now_iso = _rutilea_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _rutilea_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    #
    # Bounded with its own timeout (separate from RUTILEA_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("RUTILEA_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid RUTILEA_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_rutilea_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_rutilea_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed RUTILEA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads RUTILEA_SESSION_PLATFORM
    #     and RUTILEA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via RUTILEA_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     RUTILEA_SESSION_PLATFORM == "telegram".
    ...
    #     RUTILEA_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read RUTILEA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the RUTILEA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing RUTILEA_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # RUTILEA_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ RUTILEA_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "RUTILEA_CRON_AUTO_DELIVER_PLATFORM",
        "RUTILEA_CRON_AUTO_DELIVER_CHAT_ID",
        "RUTILEA_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["RUTILEA_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `rutilea chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with RUTILEA_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $RUTILEA_KANBAN_TASK -- letting
        ...
        # load_rutilea_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this RUTILEA_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_rutilea_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from rutilea_cli.env_loader import (
            load_rutilea_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["RUTILEA_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["RUTILEA_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["RUTILEA_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > RUTILEA_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``rutilea cron edit --model``
        ...
        try:
            ...
            _cfg_path = str(_get_rutilea_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"RUTILEA_MODEL={os.getenv('RUTILEA_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`rutilea cron edit {job_id} --model <name>` or set a "
                "default with `rutilea model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from rutilea_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("RUTILEA_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_rutilea_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject RUTILEA_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Rutilea "
                        "pin it explicitly: "
                        f"`rutilea cron edit {job_id} --provider <provider> "
        ...
        agent = AIAgent(
            ...
            # RUTILEA_HOME. When a workdir is configured, also inject project
        ...
        # override via RUTILEA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from rutilea_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_rutilea_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_rutilea_now().strftime('%b %d %H:%M')}"
...
def run_one_job(
    ...
    *,
    ...
    profile_home = _get_rutilea_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_rutilea_home())
        )
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`rutilea pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `rutilea resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `rutilea cron run` that claimed a job
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _rutilea_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _rutilea_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set RUTILEA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("RUTILEA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid RUTILEA_CRON_MAX_PARALLEL value; defaulting to unbounded")
