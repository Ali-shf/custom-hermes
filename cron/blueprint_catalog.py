...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``rutilea://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``rutilea://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"rutilea://blueprint/{quote(blueprint.key)}{qs}"
