...
version: 2.0.0
author: Francesco Bonacci (f-trycua), Rutilea Agent
license: MIT
...
Rutilea drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the Rutilea `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Rutilea autodetection is a
planned cua-driver follow-up, so currently point Rutilea at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   Rutilea applies the same host-side floor in unrestricted mode. If it is not
...
Explicit Rutilea YOLO (`--yolo`, `/yolo`, or `approvals.mode: off`) launches an
...
Rutilea intentionally keeps THIS skill focused on the Rutilea-side
...
Rutilea autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Rutilea at that
