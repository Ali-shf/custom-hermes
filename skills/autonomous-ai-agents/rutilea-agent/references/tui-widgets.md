...
Author widget apps for the Rutilea TUI (`rutilea --tui`): glanceable ambient
...
- The TUI must be in use (`rutilea --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.rutilea/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.rutilea/logs/tui_gateway_crash.log` if a widget never appears.
