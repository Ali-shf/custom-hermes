...
Write plugins for the Rutilea desktop app: statusbar items, layout panes,
...
system (`~/.rutilea/plugins/`) is otherwise documented separately.
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Rutilea (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.rutilea/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
