...
Set via `rutilea model` (picker) or `rutilea setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `rutilea auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `rutilea auth add openai-codex` |
| qwen-oauth | OAuth | `rutilea auth add qwen-oauth` |
| minimax-oauth | OAuth | `rutilea auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`rutilea auth`).
Fallback chain when the primary fails: `rutilea fallback add|remove|list`.
...
#   rutilea config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
