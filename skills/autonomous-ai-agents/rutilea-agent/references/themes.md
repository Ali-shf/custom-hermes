# Themes / Skins — Author a Rutilea Color Theme

Author a Rutilea **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.rutilea/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Rutilea itself.
...
- Write access to the Rutilea home dir — `~/.rutilea` by default, or `$HERMES_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `rutilea config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   rutilea config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `rutilea config set display.skin default` (or they can `/skin default`).
...
```
rutilea skin set <key> <hex>      # e.g. rutilea skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `rutilea skin set background "#08201f"` changes
only the background; `rutilea skin use <name>` / `rutilea skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.rutilea`** when a profile is active — resolve the real home
  from `$HERMES_HOME` first, falling back to `~/.rutilea`.
...
- **Never hand-edit `config.yaml` to activate.** Use `rutilea config set
...
- **You apply it, not the user.** `rutilea config set display.skin <name>` is
...
- Run `rutilea config get display.skin` and confirm it reports `<name>`.
