...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Rutilea agent runs by POSTing events to a URL.

## Setup (Required First)

The webhook platform must be enabled before subscriptions can be created. Check with:
```bash
rutilea webhook list
```
...
```bash
rutilea gateway setup
```
Follow the prompts to enable webhooks, set the port, and set a global HMAC secret.

### Option 2: Manual config
Add to `~/.rutilea/config.yaml`:
```yaml
platforms:
  webhook:
    enabled: true
...
Add to `${HERMES_HOME:-~/.rutilea}/.env`:
```bash
...
All management is via the `rutilea webhook` CLI command:

### Create a subscription
```bash
rutilea webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.rutilea/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.rutilea/scripts/` (path traversal is blocked).

```bash
rutilea webhook subscribe todoist-hermes \
...
```bash
rutilea webhook remove <name>
```

### Test a subscription
```bash
rutilea webhook test <name>
rutilea webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: new issues
```bash
rutilea webhook subscribe github-issues \
  --events "issues" \
...
### GitHub: PR reviews
```bash
rutilea webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
rutilea webhook subscribe stripe-payments \
...
```bash
rutilea webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
rutilea webhook subscribe alerts \
...
```bash
rutilea webhook subscribe antenna-matches \
  --deliver telegram \
...
- Subscriptions persist to `~/.rutilea/webhook_subscriptions.json`

## How It Works

1. `rutilea webhook subscribe` writes to `~/.rutilea/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.rutilea/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `rutilea webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `rutilea webhook test <name>` to verify the route works.
