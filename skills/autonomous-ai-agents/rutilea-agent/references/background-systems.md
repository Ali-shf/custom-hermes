...
the `cronjob` tool, the `rutilea cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `rutilea curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `rutilea curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.rutilea/skills/.usage.json` holds
...
Users drive it via `rutilea kanban <verb>`; dispatcher-spawned workers
