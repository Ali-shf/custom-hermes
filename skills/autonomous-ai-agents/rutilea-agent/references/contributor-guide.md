...
Config: `~/.rutilea/config.yaml` (settings), `~/.rutilea/.env` (API keys) — both under `$HERMES_HOME` when it is set.
...
never hardcode `~/.rutilea`. For custom/local-only tools, write a plugin in
`~/.rutilea/plugins/` instead of editing core — see the developer docs.
...
- Tests auto-redirect `HERMES_HOME` to temp dirs — never touch real `~/.rutilea/`.
