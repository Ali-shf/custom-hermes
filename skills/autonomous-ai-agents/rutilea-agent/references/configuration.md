...
Edit with `rutilea config edit` or `rutilea config set section.key value`.
...
`rutilea config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `rutilea tools` (interactive) or `rutilea tools enable/disable NAME`.
