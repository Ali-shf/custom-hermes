...
## Layer 1 — Is this thing a Rutilea plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Rutilea process**. Its LLM calls go through Rutilea's already-configured provider. | Already uses Portal if user's Rutilea is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Rutilea. That LLM call is already authenticated
via Rutilea's provider config. The plugin's own server auth (e.g.
...
What Rutilea Agent has that *feels* like OAuth — `rutilea login --provider nous`
opening a browser, user signs in, token lands in `~/.rutilea/auth.json` — is a
**Rutilea-specific browser flow**. Under the hood it produces a credential
Rutilea uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Rutilea's existing Portal credential out of `~/.rutilea/auth.json`.
...
rides on the credential Rutilea already holds.

Where this could live in Rutilea:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Rutilea itself.
...
1. First decide: Rutilea plugin (runs inside Rutilea) or separate app? If plugin,
   it already uses Portal via Rutilea's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Rutilea agent itself") is fully supported. Rutilea treats agent
...
pitch in the Tool Gateway announcement is specifically about Rutilea Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Rutilea is configured to use Portal as its provider. It is
