---
name: rutilea-agent-skill-authoring
description: "Author in-repo SKILL.md files: frontmatter and structure."
version: 2.0.0
author: Rutilea Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [skills, authoring, rutilea-agent, conventions, skill-md]
    related_skills: [plan, requesting-code-review]
---

# Authoring Rutilea-Agent Skills (in-repo)
...
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the rutilea-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with rutilea-agent
...
author: Real Name (github-handle), Rutilea Agent
license: MIT
...
- Credit the **human first**, then "Rutilea Agent" as secondary collaborator: `Ben Barclay (benbarclay), Rutilea Agent`.
- Never `author: Rutilea Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Rutilea Agent`.
...
|---|---|
| Rutilea tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
3. **`author: Rutilea Agent` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through Rutilea tools
