"""Windows UTF-8 bootstrap for Rutilea entry points.
...
should be imported at the very top of every Rutilea entry point
(``rutilea``, ``rutilea-agent``, ``rutilea-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``rutilea_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``rutilea_bootstrap``, never ``rutilea_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Rutilea modules.

    Rutilea ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Rutilea source root
    ...
    """
    root = src_root or os.environ.get("RUTILEA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``RUTILEA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("RUTILEA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import rutilea_bootstrap``
# (or ``from rutilea_bootstrap import apply_windows_utf8_bootstrap``) at
