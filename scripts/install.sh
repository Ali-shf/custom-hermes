#!/bin/bash
# ============================================================================
# Rutilea Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://rutilea-agent.nousresearch.com/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Rutilea terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Ali-shf/custom-hermes.git"
REPO_URL_HTTPS="https://github.com/Ali-shf/custom-hermes.git"
RUTILEA_HOME="${RUTILEA_HOME:-$HOME/.rutilea}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${RUTILEA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$RUTILEA_INSTALL_DIR"
...
#   code at /usr/local/lib/rutilea-agent, command at /usr/local/bin/rutilea,
#   data still at /root/.rutilea (RUTILEA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$RUTILEA_HOME/.no-bundled-skills so future"
            echo "                   'rutilea update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Rutilea.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.rutilea/rutilea-agent"
            echo "                   default (root, Linux): /usr/local/lib/rutilea-agent"
            echo "  --rutilea-home PATH  Data directory (default: ~/.rutilea, or \$RUTILEA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Rutilea installs the code under"
            echo "  /usr/local/lib/rutilea-agent and links the command into"
            echo "  /usr/local/bin/rutilea (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$RUTILEA_HOME"
            echo "  (default /root/.rutilea).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$RUTILEA_HOME/rutilea-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Rutilea Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Rutilea Research.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `rutilea update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Rutilea-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Rutilea Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install rutilea command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `rutilea` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $RUTILEA_HOME/rutilea-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $RUTILEA_HOME/rutilea-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/rutilea-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $RUTILEA_HOME/rutilea-agent — then preserve it)
#
# Always no-op when the user set --dir or $RUTILEA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in RUTILEA_HOME.
    if is_termux; then
        INSTALL_DIR="$RUTILEA_HOME/rutilea-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$RUTILEA_HOME/rutilea-agent/.git" ]; then
            INSTALL_DIR="$RUTILEA_HOME/rutilea-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/rutilea-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/rutilea-agent"
        ...
        # /usr/local/bin/rutilea wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/rutilea"
        log_info "  Data:    $RUTILEA_HOME (unchanged)"
...
# Point a Rutilea-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $RUTILEA_HOME/node/bin — which is NOT on
...
# Rutilea's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Rutilea-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$RUTILEA_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$RUTILEA_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$RUTILEA_HOME/node/etc/npmrc"
}

get_rutilea_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/rutilea" ]; then
        echo "$link_dir/rutilea"
    else
        echo "rutilea"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://rutilea-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Rutilea owns its own uv at $RUTILEA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (rutilea_cli/managed_uv.py) looks in the same
    # place, so install.sh and `rutilea update` stay in sync.
    local _managed_uv="$RUTILEA_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $RUTILEA_HOME/bin ..."
    mkdir -p "$RUTILEA_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/rutilea-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/rutilea-uv-installer.$$.sh")"
    ...
    # directly into $RUTILEA_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$RUTILEA_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Rutilea-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Rutilea-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Rutilea-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Rutilea-managed Node from a previous run over a too-old system one.
    if [ -x "$RUTILEA_HOME/node/bin/node" ] && [ -x "$RUTILEA_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$RUTILEA_HOME/node/bin/node" --version)"; then
        export PATH="$RUTILEA_HOME/node/bin:$PATH"
        log_success "Node.js $("$RUTILEA_HOME/node/bin/node" --version) found (Rutilea-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Rutilea-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Rutilea requires Node >=26) — installing Rutilea-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.rutilea/node/..."
    ...
    # Place into ~/.rutilea/node/ and symlink binaries into the same bin dir
    # the rutilea command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$RUTILEA_HOME/node"
    mkdir -p "$RUTILEA_HOME"
    mv "$extracted_dir" "$RUTILEA_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$RUTILEA_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$RUTILEA_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$RUTILEA_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$RUTILEA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.rutilea/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Rutilea install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Rutilea Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `rutilea update` path
                # (#4735).
                ...
                local stash_name
                stash_name="rutilea-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``rutilea update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Rutilea behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"rutilea-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up rutilea command..."

    if [ "$USE_VENV" = true ]; then
        RUTILEA_BIN="$INSTALL_DIR/venv/bin/python"
        RUTILEA_ENTRYPOINT="$INSTALL_DIR/rutilea"
    else
        RUTILEA_BIN="$(which rutilea 2>/dev/null || echo "")"
        if [ -z "$RUTILEA_BIN" ]; then
            log_warn "rutilea not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$RUTILEA_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$RUTILEA_ENTRYPOINT" ]; }; then
        log_warn "Rutilea launcher prerequisites not found"
    ...
    # Create a user-facing shim for the rutilea command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $RUTILEA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$RUTILEA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/rutilea"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/rutilea" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$RUTILEA_BIN" "$RUTILEA_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/rutilea" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/rutilea"
    log_success "Installed rutilea launcher → $command_link_display_dir/rutilea"

    # Also expose `rutilea-agent`. The `rutilea-agent` console script declared in
    ...
    rm -f "$command_link_dir/rutilea-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/rutilea-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$RUTILEA_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/rutilea-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$RUTILEA_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/rutilea-agent"
    log_success "Installed rutilea-agent launcher → $command_link_display_dir/rutilea-agent"

    # Also expose `rutilea-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `rutilea-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Rutilea as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/rutilea-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/rutilea-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$RUTILEA_BIN" "$RUTILEA_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/rutilea-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$RUTILEA_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/rutilea-acp"
    log_success "Installed rutilea-acp launcher → $command_link_display_dir/rutilea-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "rutilea command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v rutilea' \
                >/dev/null 2>&1; then
        ...
        log_info "rutilea not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Rutilea Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Rutilea Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Rutilea Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so rutilea works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.rutilea directory structure (config at top level, code in subdir)
    mkdir -p "$RUTILEA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.rutilea/.env (top level, easy to find)
    if [ ! -f "$RUTILEA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$RUTILEA_HOME/.env"
            log_success "Created ~/.rutilea/.env from template"
        else
            touch "$RUTILEA_HOME/.env"
            log_success "Created ~/.rutilea/.env"
        fi
    else
        log_info "~/.rutilea/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$RUTILEA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.rutilea/config.yaml (top level, easy to find)
    if [ ! -f "$RUTILEA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$RUTILEA_HOME/config.yaml"
            log_success "Created ~/.rutilea/config.yaml from template"
        fi
    else
        log_info "~/.rutilea/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in rutilea_cli/default_soul.py — the
    ...
    if [ ! -f "$RUTILEA_HOME/SOUL.md" ]; then
        cat > "$RUTILEA_HOME/SOUL.md" << 'SOUL_EOF'
You are Rutilea Agent, an intelligent AI assistant created by Rutilea Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.rutilea/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.rutilea/"

    # Seed bundled skills into ~/.rutilea/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `rutilea update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'rutilea update'." \
            > "$RUTILEA_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $RUTILEA_HOME/.no-bundled-skills"
        log_info "  Future 'rutilea update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.rutilea/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.rutilea/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$RUTILEA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$RUTILEA_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.rutilea/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$RUTILEA_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Rutilea Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Rutilea will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Rutilea Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `rutilea update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `rutilea tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$RUTILEA_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'rutilea tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `rutilea tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `rutilea computer-use install` on their own).
    ...
    # Same upstream installer `rutilea computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Rutilea install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Rutilea'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'rutilea tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: rutilea computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'rutilea setup' after install."
        return 0
    fi
    ...
    # Run rutilea setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m rutilea_cli.main setup < /dev/tty
    else
        python -m rutilea_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$RUTILEA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Rutilea to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$RUTILEA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'rutilea whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                RUTILEA_CMD="$(get_rutilea_command_path)"
                $RUTILEA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'rutilea whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'rutilea gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $RUTILEA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $RUTILEA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: rutilea gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: rutilea gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $RUTILEA_CMD gateway > "$RUTILEA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.rutilea/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: rutilea gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: rutilea gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.rutilea-bootstrap-complete, which tells the Rutilea
    ...
    local marker_path="$INSTALL_DIR/.rutilea-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $RUTILEA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $RUTILEA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $RUTILEA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}rutilea${NC}              Start chatting"
    echo -e "   ${GREEN}rutilea setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}rutilea config${NC}       View/edit configuration"
    echo -e "   ${GREEN}rutilea config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}rutilea gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}rutilea update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'rutilea' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'rutilea' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'rutilea' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$RUTILEA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$RUTILEA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$RUTILEA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `rutilea update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$RUTILEA_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Rutilea` rename
...
# _purge_electron_build_cache() used by `rutilea desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Rutilea-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `rutilea desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Rutilea" ]; then
            app="$desktop_dir/release/linux-unpacked/Rutilea"
        elif [ -x "$desktop_dir/release/linux-unpacked/rutilea" ]; then
            app="$desktop_dir/release/linux-unpacked/rutilea"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Rutilea.app" \
            "$desktop_dir/release/mac/Rutilea.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `rutilea desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading RUTILEA_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if RUTILEA_HOME="$RUTILEA_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from rutilea_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `rutilea update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Rutilea-managed Node
            # provisioned during prerequisites/node-deps (at $RUTILEA_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $RUTILEA_HOME.
    # $RUTILEA_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'rutilea update' on this host install.
