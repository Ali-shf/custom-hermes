...
Rutilea Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``RUTILEA_NIX_BUILD=1`` on the
Rutilea package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("RUTILEA_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for rutilea-agent is not supported.\n"
    "Rutilea is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the Rutilea Nix derivation sets RUTILEA_NIX_BUILD=1. If it does, file a bug."
)
