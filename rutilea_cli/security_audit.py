"""On-demand supply-chain audit for Rutilea Agent installs.

Scans three surfaces a Rutilea user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Rutilea venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.rutilea/plugins``
...
def _discover_plugins(rutilea_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.rutilea/plugins``.
    ...
    """
    plugins_dir = rutilea_home / "plugins"
...
def _discover_components(
    *,
    ...
    home = rutilea_home or Path(get_rutilea_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `rutilea security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, rutilea_home=home
    )
