"""
Backup and import commands for rutilea CLI.

`rutilea backup` creates a zip archive of the entire ~/.rutilea/ directory
(excluding the rutilea-agent repo and transient files).

`rutilea import` restores from a backup zip, overlaying onto the current
RUTILEA_HOME root.
"""
...
from rutilea_constants import get_default_rutilea_root, get_rutilea_home, display_rutilea_home
from utils import (
    _preserve_file_mode,
...
from rutilea_cli.sizefmt import format_bytes as _format_size
...
# ``rutilea-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/rutilea-agent/``
...
# RUTILEA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "rutilea-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under RUTILEA_HOME) —
...
# File names that ``rutilea import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE RUTILEA_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Rutilea backup slot."""
...
@contextmanager
def _backup_operation_lock(rutilea_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = rutilea_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Rutilea backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside RUTILEA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to rutilea root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``rutilea-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/rutilea-agent/`` — must be preserved.
        if part == "rutilea-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside RUTILEA_HOME.
...
# to the user as a hung `rutilea update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # rutilea_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Rutilea home directory."""
    rutilea_root = get_default_rutilea_root()

    if not rutilea_root.is_dir():
        print(f"Error: Rutilea home directory not found at {rutilea_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(rutilea_root):
            _run_backup_locked(args, rutilea_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, rutilea_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"rutilea-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"rutilea-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_rutilea_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(rutilea_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(rutilea_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``rutilea-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "rutilea-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(rutilea_root)
    ...
    # outside RUTILEA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_rutilea_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: rutilea import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Rutilea backup.
    ...
    # Look for telltale files that a rutilea home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Rutilea backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.rutilea/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a rutilea dir name
        if prefix in {".rutilea", "rutilea"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``rutilea import``, which is the
    ...
    so without the chown a ``sudo rutilea import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``rutilea import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Rutilea' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_rutilea_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (rutilea_root / "config.yaml").exists()
        has_env = (rutilea_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Rutilea configuration.")
        ...
        rutilea_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under RUTILEA_HOME.
            ...
            target = rutilea_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(rutilea_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_rutilea_home()}."
            )
        ...
        profiles_dir = rutilea_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # rutilea_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: rutilea profile list  (after installing rutilea)")

        # Guidance
        print()
        if not (rutilea_root / "rutilea-agent").is_dir():
            print("Note: The rutilea-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: rutilea update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  rutilea -p {pname} gateway install")
        ...
        try:
            from rutilea_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Rutilea configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and rutilea backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to RUTILEA_HOME).
...
# — `rutilea update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $RUTILEA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside RUTILEA_HOME and the entry is silently
...
def _quick_snapshot_root(rutilea_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: rutilea snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(rutilea_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside RUTILEA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``rutilea update``.
    ...
    Args:
        ...
        rutilea_home: Override for the Rutilea home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(rutilea_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for rutilea backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_rutilea_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, rutilea_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(rutilea_root):
            return _write_full_zip_backup_locked(out_path, rutilea_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, rutilea_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``rutilea_root`` to ``out_path``.
...
def _pre_update_backup_dir(rutilea_home: Optional[Path] = None) -> Path:
    home = rutilea_home or get_rutilea_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of RUTILEA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<RUTILEA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``rutilea update``) should continue even if the backup fails.
    """
    rutilea_root = rutilea_home or get_default_rutilea_root()
    if not rutilea_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(rutilea_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, rutilea_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `rutilea claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of RUTILEA_HOME under ``backups/`` before a
    ``rutilea claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``rutilea import <archive>``.  Writes to
    ``<RUTILEA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `rutilea import` and the
