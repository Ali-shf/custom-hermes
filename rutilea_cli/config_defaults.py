"""Default configuration data for Rutilea Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from rutilea_cli/config.py. Must not import from rutilea_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["rutilea-cli"],
    # SQLite journal mode used by every Rutilea database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Rutilea server processes.
    ...
    "session": {
        # Per-terminal `rutilea -c`: each CLI session drops a breadcrumb file
        # under $RUTILEA_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # an interactive `rutilea gateway restart` must never block for hours
        ...
        # the Rutilea-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Rutilea
        ...
        # identity slot (SOUL.md). Empty by default. The RUTILEA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Rutilea adds a coding operating brief
        ...
        # `rutilea status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``RUTILEA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             RUTILEA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force RUTILEA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Rutilea auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Rutilea sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Rutilea
        ...
        #  "/home/user/.rutilea/cache/documents:/output"]
        ...
        # bundled Rutilea image, which drops to the `rutilea` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Rutilea sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Rutilea should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``rutilea chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.rutilea/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: rutilea sweeps the checkpoint base at startup
        ...
        # ``rutilea checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .rutilea.md, or .cursorrules before Rutilea applies
    ...
    # Single-query (``rutilea -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Rutilea truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Rutilea' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); rutilea =
                                      # Rutilea' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Rutilea' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Rutilea does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``rutilea kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``rutilea kanban decompose`` and the kanban
        ...
        # ``rutilea profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `rutilea model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `rutilea` (and `rutilea chat`) launches by default:
        ...
        # REPL and `--tui` (or RUTILEA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `rutilea --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `rutilea -c` muscle memory.  Default off so existing
        # users aren't surprised.  RUTILEA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `rutilea --tui` drops a one-time hint
        ...
        # model — see rutilea_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to RUTILEA_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/rutilea`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `rutilea pets`. Disabled until
        ...
        "pet": {
            ...
            # get_rutilea_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``RUTILEA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``RUTILEA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``RUTILEA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``RUTILEA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``RUTILEA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/rutilea``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Rutilea appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.rutilea/cache/piper-voices/
    ...
    # "Hey Rutilea" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey rutilea",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_rutilea" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.rutilea/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.rutilea/cache/delegation/
    ...
    # response. If not, Rutilea feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Rutilea auto-pauses the goal and
    ...
    "moa": {
        ...
        # <rutilea_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.rutilea/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.rutilea/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``rutilea skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``rutilea skills trust`` / ``rutilea skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${RUTILEA_SKILL_DIR} and ${RUTILEA_SESSION_ID} in SKILL.md
        ...
        # (`rutilea skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.rutilea/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.rutilea/.curator_backups/blobs/. Enables `rutilea curator ledger`
        # and single-mutation `rutilea curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `rutilea curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `rutilea curator run --consolidate` overrides
        ...
        # normally restored on every `rutilea update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `rutilea curator purge` deletes
        ...
        # skipped), snapshot ~/.rutilea/skills/ into
        # ~/.rutilea/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `rutilea curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for rutilea-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Rutilea Agent*" header.
    ...
    # dangerous command. -q runs export RUTILEA_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; RUTILEA_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Rutilea' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.rutilea/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or RUTILEA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .rutilea config) always require human
        ...
        # longer trigger the startup banner. Add via `rutilea doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``rutilea_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Rutilea to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $RUTILEA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via RUTILEA_CRON_MAX_PARALLEL env var.
        ...
        # RUTILEA_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # RUTILEA_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via RUTILEA_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `rutilea -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `rutilea` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `rutilea kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     rutilea-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Rutilea tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.rutilea/logs/.
    ...
    # update model picker lists without shipping a rutilea-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `rutilea model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Rutilea provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal RUTILEA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.rutilea/sessions/sessions.json entirely.
        ...
        # the RUTILEA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.rutilea.gateway` that defenses 1-2 don't
        ...
        # `rutilea update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``RUTILEA_GATEWAY_MAX_STARTS`` / ``RUTILEA_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.rutilea/.env,
        ...
        # behavior: files must live under the Rutilea cache, under
        ...
        # user. Bridged to RUTILEA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Rutilea
        # cache (~/.rutilea/cache/{documents,images,audio,video,screenshots})
        ...
        # to RUTILEA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # RUTILEA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to RUTILEA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.rutilea/state.db.
    ...
    "sessions": {
        ...
        # the default of ``rutilea sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.rutilea/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `rutilea sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `rutilea update` prints a one-line notice with
        ...
        # ~/.rutilea/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to RUTILEA_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # RUTILEA_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `rutilea sessions export`. Checked
    ...
    # ``rutilea doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `rutilea doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``rutilea update`` behaviour.
    "updates": {
        ...
        #     DBs) into <RUTILEA_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``rutilea backup``-style zip
        #     of RUTILEA_HOME into <RUTILEA_HOME>/backups/, restorable with
        #     ``rutilea import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `rutilea update` does with uncommitted local changes to the
        ...
        # When `rutilea update` finds the source checkout parked on a feature
    ...
    "lsp": {
        ...
        #              ``<RUTILEA_HOME>/lsp/bin/`` on first use.
    ...
    # `rutilea tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.rutilea/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.rutilea/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.rutilea/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Rutilea may use this encrypted
            ...
            # ~/.rutilea/bin/ on first use.  When False you must install
            ...
            # `rutilea secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.rutilea/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Rutilea disables it for our users unless they
        # explicitly opt in here. When false (default), Rutilea sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Rutilea also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Rutilea computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Rutilea passes the
    ...
    #
    # Configure with `rutilea egress setup`.  Disabled by default — the rest of
    # Rutilea works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.rutilea/bin/ on
    ...
    # Rutilea Desktop (Electron app) launch options. These only affect
    # `rutilea desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the RUTILEA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the RUTILEA_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Rutilea mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'rutilea model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Rutilea auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Rutilea' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Rutilea, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @rutilea:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. RUTILEA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #rutilea)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: rutilea-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'rutilea-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Rutilea API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Rutilea API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Rutilea API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # RUTILEA_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to RUTILEA_TOOL_PROGRESS_MODE for backward
    ...
    # RUTILEA_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "RUTILEA_PREFILL_MESSAGES_FILE": {
    ...
    "RUTILEA_EPHEMERAL_SYSTEM_PROMPT": {
