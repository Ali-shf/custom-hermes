"""
Interactive setup wizard for Rutilea Agent.
...
from rutilea_constants import get_optional_skills_dir
...
# display_rutilea_home imported lazily at call sites (stale-module safety during rutilea update)
...
from rutilea_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from rutilea_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Rutilea Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Rutilea using environment variables or config commands:")
    print_info("  rutilea config set model.provider custom")
    print_info("  rutilea config set model.base_url http://localhost:8080/v1")
    print_info("  rutilea config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'rutilea setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``RUTILEA_NONINTERACTIVE=1`` (see ``rutilea_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("RUTILEA_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``RUTILEA_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from rutilea_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'rutilea setup')")


def _print_setup_summary(config: dict, rutilea_home):
    ...
    try:
        from rutilea_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Rutilea cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    rutilea model            (pick any provider/model)")
        print_info("    rutilea setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'rutilea setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from rutilea_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `rutilea tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from rutilea_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'rutilea setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'rutilea setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'rutilea tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'rutilea setup terminal'"))
    ...
    # Spotify (OAuth via rutilea auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'rutilea setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {rutilea_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('rutilea setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('rutilea setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('rutilea setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('rutilea setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('rutilea setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('rutilea config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('rutilea config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('rutilea config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('rutilea', Colors.GREEN)}              Start chatting")
    print(f"   {color('rutilea gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('rutilea doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `rutilea tools` and `rutilea setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``rutilea model``)
    ...
    provider added to ``rutilea model`` is automatically available here.
    ...
    # Delegate to the shared rutilea model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from rutilea_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: rutilea model")
    ...
    # on demand via `rutilea auth add`, `rutilea setup` vision, and
    # `rutilea setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from rutilea_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``rutilea model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from rutilea_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.rutilea/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Rutilea auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via rutilea setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'rutilea setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Rutilea runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `rutilea setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `rutilea setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `rutilea egress setup` then `rutilea egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `rutilea egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'rutilea-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $RUTILEA_HOME/bin is never on PATH, so a bare
            # which() misses the uv Rutilea installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'rutilea-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into RUTILEA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("RUTILEA_MAX_ITERATIONS")
    ...
    print_info("  Run `rutilea setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # RUTILEA_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.rutilea/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from rutilea_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_rutilea_home(Path(get_rutilea_home()))
    except Exception:
        pass
...
def _profile_name_from_rutilea_home(rutilea_home) -> str | None:
    """Return the active profile name when RUTILEA_HOME is a profile dir."""
    if rutilea_home.parent.name == "profiles":
        return rutilea_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Rutilea delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Rutilea to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from rutilea_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  rutilea config edit")
    print_info("   Open config in your editor:  rutilea config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from rutilea_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Rutilea from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'rutilea setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     rutilea config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `rutilea import` or `rutilea setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `rutilea setup tools` and `rutilea tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Rutilea profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``rutilea_cli.auth`` — lists every supported
    ...
    """
    try:
        from rutilea_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from rutilea_cli.config
    ...
    if section_key == "model":
        ...
        from rutilea_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_rutilea.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_rutilea migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_rutilea", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Rutilea.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Rutilea to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Rutilea at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Rutilea at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Rutilea at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Rutilea at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Rutilea equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Rutilea config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Rutilea.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Rutilea's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(rutilea_home: Path) -> bool:
    ...
    print_info("Rutilea can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: rutilea claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=rutilea_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: rutilea claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Rutilea configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Rutilea config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Rutilea (use rutilea claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``rutilea setup --portal`` and ``rutilea portal``. This is the
    ...
    Rutilea session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``rutilea setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``rutilea model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``rutilea portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Rutilea Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `rutilea portal` == quick setup's Nous step.
    try:
        from rutilea_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `rutilea portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `rutilea portal`: %s", exc)
        print()
    ...
    print_info("  Run `rutilea portal info` to inspect routing.")
    print_info("  Run `rutilea` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      rutilea setup           — full or quick (auto-detected)
      rutilea setup model     — just model/provider
      rutilea setup tts       — just text-to-speech
      rutilea setup terminal  — just terminal backend
      rutilea setup gateway   — just messaging platforms
      rutilea setup tools     — just tool configuration
      rutilea setup telemetry — just local shared metrics
      rutilea setup agent     — just agent settings
    """
    from rutilea_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Rutilea Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Rutilea Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Rutilea Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, rutilea_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Rutilea configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'rutilea setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(rutilea_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Rutilea?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, rutilea_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, rutilea_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {rutilea_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'rutilea config edit'")
    ...
    # Tune later with `rutilea setup agent`.
    if not is_existing:
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from rutilea_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    _print_setup_summary(config, rutilea_home)


def _run_first_time_quick_setup(config: dict, rutilea_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``rutilea setup <section>``
    or switch providers with ``rutilea model``.
    """
    from rutilea_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'rutilea setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `rutilea import` from another machine).
        from rutilea_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    rutilea setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  rutilea setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``rutilea tools`` (which
    ...
    try:
        ...
        from rutilea_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``rutilea setup agent`` /
    ``rutilea config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, rutilea_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `rutilea update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        rutilea tools")
        print_info("  Seed skills:         rutilea skills opt-in --sync")
        print_info("  Add MCP servers:     rutilea mcp add")
        print_info("  Enable plugins:      rutilea plugins")
        print_info("  Tune agent settings: rutilea setup agent")
        print()
        _print_setup_summary(config, rutilea_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, rutilea_home)


def _blank_slate_walkthrough(config: dict, rutilea_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`rutilea update` runs from re-injecting them. Opt back in any")
            print_info("time with `rutilea skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `rutilea tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `rutilea plugins list` / `rutilea plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `rutilea plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `rutilea mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `rutilea mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   rutilea tools")
    print_info("  Seed skills:         rutilea skills opt-in --sync")
    print_info("  Add MCP servers:     rutilea mcp add")
    print_info("  Tune agent settings: rutilea setup agent")
    print()
...
def _run_quick_setup(config: dict, rutilea_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'rutilea setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Rutilea to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'rutilea setup gateway'.")
