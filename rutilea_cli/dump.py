"""
Dump command for rutilea CLI.

Outputs a compact, plain-text summary of the user's Rutilea setup
...
from rutilea_cli.config import get_rutilea_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.rutilea/.env.
    ...
    exports. ``rutilea debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.rutilea_build_sha`` by the Dockerfile's
    ``RUTILEA_GIT_SHA`` build-arg (see ``rutilea_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``rutilea dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from rutilea_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(rutilea_home: Path) -> int:
...
def _cron_summary(rutilea_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = rutilea_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"rutilea_home:      {display_rutilea_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.rutilea/.env: a managed
        ...
        # A credential added via `rutilea auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `rutilea auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["rutilea-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(rutilea_home)}")
    lines.append(f"  skills:             {_count_skills(rutilea_home)}")
