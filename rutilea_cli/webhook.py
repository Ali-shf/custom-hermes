"""rutilea webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    rutilea webhook subscribe <name> [options]
    rutilea webhook list
    rutilea webhook remove <name>
    rutilea webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.rutilea/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _rutilea_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: rutilea gateway run
"""
...
def webhook_command(args):
    """Entry point for 'rutilea webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: rutilea webhook {subscribe|list|remove|test}")
        print("Run 'rutilea webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (rutilea gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: rutilea webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from rutilea webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (rutilea gateway run)")
