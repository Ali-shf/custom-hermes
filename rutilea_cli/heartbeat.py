...
remains ``rutilea cron`` / the ``cronjob`` tool, which runs in isolated
...
def _get_session_db() -> Optional[Any]:
    # Reuse the goals module's per-RUTILEA_HOME cached SessionDB so both
    # features share one connection instead of thrashing the file.
    try:
        from rutilea_cli.goals import _get_session_db as _goals_db
