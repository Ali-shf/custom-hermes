"""Slash command definitions and autocomplete for the Rutilea CLI.
...
from rutilea_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``rutilea_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``rutilea_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Rutilea config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Rutilea works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Rutilea' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Rutilea is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.rutilea/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Rutilea Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Rutilea Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`rutilea_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/rutilea`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Rutilea ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Rutilea
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``rutilea skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``rutilea skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``rutilea skills list`` and
...
# unrelated command landed. These claim their slots right after /rutilea,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /rutilea <command>).
...
# native slot, the alias spelling stays reachable via /rutilea reset).
...
# ``/rutilea <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/rutilea" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /rutilea topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /rutilea moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /rutilea debug on Slack.
#   - egress: Docker-only proxy status; reachable as /rutilea egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /rutilea init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /rutilea version on
...
#   - diff: git working-tree diff; reached via /rutilea diff on Slack so it
...
#     /rutilea update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /rutilea heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /rutilea refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /rutilea pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /rutilea whoami on Slack.
...
#     /rutilea platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_RUTILEA_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/rutilea <verb>`` subcommand.
    ...
    can still reach them via ``/rutilea <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/rutilea`` is always reserved as the first entry so the
    legacy ``/rutilea <subcommand>`` form keeps working for anything that
    ...
    # Reserve /rutilea as the catch-all top-level command.
    entries.append(("rutilea", "Talk to Rutilea or run a subcommand", "[subcommand] [args]"))
    seen.add("rutilea")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_RUTILEA_ONLY:
            # Intentionally Slack-via-/rutilea only (see _SLACK_VIA_RUTILEA_ONLY).
            return
    ...
    # everything except /rutilea, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://rutilea-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /rutilea handler.

    Maps both canonical names and aliases so /rutilea bg do stuff works
    the same as /rutilea background do stuff.

    Plugin-registered slash commands are included so ``/rutilea <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from rutilea_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via rutilea_cli.personality."""
        try:
