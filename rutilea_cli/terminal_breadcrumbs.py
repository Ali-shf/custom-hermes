"""Per-terminal session breadcrumbs for ``rutilea -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$RUTILEA_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``rutilea -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_rutilea_home() / "terminal-sessions"
