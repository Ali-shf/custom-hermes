"""
Rutilea Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.rutilea/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Rutilea PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing rutilea-agent or rutilea PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Rutilea Agent" comment and following line
                if '# Rutilea Agent' in line or '# rutilea-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('rutilea' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing rutilea
                if 'rutilea' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Rutilea-owned file, and
...
def remove_wrapper_script():
    """Remove the rutilea wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "rutilea",
        Path.home() / ".local" / "bin" / "rutilea-acp",
        Path.home() / ".local" / "bin" / "rutilea-agent",
        Path("/usr/local/bin/rutilea"),
        Path("/usr/local/bin/rutilea-acp"),
        Path("/usr/local/bin/rutilea-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains rutilea_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'rutilea_cli' in content or 'rutilea-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(rutilea_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``rutilea`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Rutilea home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (rutilea_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``rutilea gateway run`` processes
    ...
    try:
        from rutilea_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from rutilea_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``RUTILEA_HOME`` and ``RUTILEA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\rutilea\git\cmd``, ``%LOCALAPPDATA%\rutilea\git\bin``,
#      ``%LOCALAPPDATA%\rutilea\git\usr\bin``, ``%LOCALAPPDATA%\rutilea\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\rutilea\git\`` and Node to
#      ``%LOCALAPPDATA%\rutilea\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``rutilea dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\rutilea\gateway-service\`` and sometimes
...
def _rutilea_path_markers(rutilea_home: Path) -> list[str]:
    """Path-entry substrings that identify Rutilea-owned User-PATH entries."""
    root = str(rutilea_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare rutilea-agent install dir.
    markers = [root + "\\rutilea-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if RUTILEA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "rutilea".  We don't want to catch
    # unrelated entries like "crutilea-foo" or "ephermeral", so we look for
    # backslash-rutilea as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(rutilea_home: Path) -> list[str]:
    """Strip Rutilea-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _rutilea_path_markers(rutilea_home)
            kept: list[str] = []
...
def remove_rutilea_env_vars_windows() -> list[str]:
    """Delete RUTILEA_HOME and RUTILEA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("RUTILEA_HOME", "RUTILEA_GIT_BASH_PATH"):
                try:
...
def _is_default_rutilea_home(rutilea_home: Path) -> bool:
    """Return True when ``rutilea_home`` points at the default (non-profile) root."""
    try:
        from rutilea_constants import get_default_rutilea_root
        return rutilea_home.resolve() == get_default_rutilea_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its RUTILEA_HOME directory.

    We shell out to ``rutilea -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current RUTILEA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m rutilea_cli.main` so we don't depend on a `rutilea`
    #    wrapper that may be half-removed mid-uninstall.
    rutilea_invocation = [_sys.executable, "-m", "rutilea_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                rutilea_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's RUTILEA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``rutilea uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$RUTILEA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from rutilea_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(rutilea_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Rutilea Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Rutilea Chat GUI installation was found.")
        print(f"  Checked: {rutilea_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Rutilea agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(rutilea_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Rutilea agent at {rutilea_home / 'rutilea-agent'}")
        print(f"  • Your config, sessions, and secrets under {rutilea_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Rutilea agent is still installed. Run 'rutilea' to use the CLI,")
    print("or 'rutilea uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.rutilea/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.rutilea/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie RUTILEA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_rutilea_home(rutilea_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.rutilea data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Rutilea Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {rutilea_home / 'config.yaml'}")
    print(f"  Secrets: {rutilea_home / '.env'}")
    print(f"  Data:    {rutilea_home / 'cron/'}, {rutilea_home / 'sessions/'}, {rutilea_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their RUTILEA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Rutilea data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Rutilea code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, rutilea_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Rutilea PATH entries from shell configs / Windows User PATH")
    print("  • Rutilea wrapper scripts and Rutilea-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Rutilea config/data: {rutilea_home}")
        if _is_default_rutilea_home(rutilea_home):
        ...
        print(f"  • Keep Rutilea config/data: {rutilea_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``rutilea`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$RUTILEA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in rutilea_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\rutilea\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(rutilea_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Rutilea-owned PATH entries in User environment")

        log_info("Removing RUTILEA_HOME / RUTILEA_GIT_BASH_PATH User env vars...")
        removed_env = remove_rutilea_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Rutilea-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing rutilea command...")
    ...
    #     (only when they still point into this Rutilea home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Rutilea-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(rutilea_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Rutilea-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(rutilea_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE RUTILEA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from rutilea_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(rutilea_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.rutilea/, just remove the rutilea-agent subdir
            if rutilea_home in project_root.parents or project_root.parent == rutilea_home:
    ...
    #     under RUTILEA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(rutilea_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(rutilea_home)
    ...
    # 5. Optionally remove ~/.rutilea/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile RUTILEA_HOME dirs live under
        ...
        try:
            if rutilea_home.exists():
                shutil.rmtree(rutilea_home)
                log_success(f"Removed {rutilea_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {rutilea_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {rutilea_home}")
    ...
    print()
    print("Thank you for using Rutilea Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m rutilea_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import rutilea_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``rutilea_constants`` + ``rutilea_cli.colors``
    (and lazily ``rutilea_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m rutilea_cli.uninstall")
    parser.add_argument(
        "--mode",
