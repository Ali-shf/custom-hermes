"""Shell completion script generation for rutilea CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_rutilea_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Rutilea Agent bash completion
# Add to ~/.bashrc:
#   eval "$(rutilea completion bash)"

_rutilea_profiles() {{
    local profiles_dir="$HOME/.rutilea/profiles"
...
_rutilea_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_rutilea_profiles)" -- "$cur"))
        return
    fi
...
complete -F _rutilea_completion rutilea
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef rutilea
# Rutilea Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(rutilea completion zsh)"

_rutilea_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.rutilea/profiles" ]]; then
        profiles+=($HOME/.rutilea/profiles/*(N/:t))
    fi
...
_rutilea() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_rutilea_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'rutilea command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Rutilea Agent fish completion",
        "# Add to your config:",
        "#   rutilea completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __rutilea_profiles",
        "    echo default",
        "    if test -d $HOME/.rutilea/profiles",
        "        for d in $HOME/.rutilea/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c rutilea -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c rutilea -f -s p -l profile"
        " -d 'Profile name' -xa '(__rutilea_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__rutilea_profiles)' -d 'Profile name'"
                )
