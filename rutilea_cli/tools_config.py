"""
Unified tool configuration for Rutilea Agent.

`rutilea tools` and `rutilea setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.rutilea/config.yaml under
...
from rutilea_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from rutilea_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``rutilea tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`rutilea_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from rutilea_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _RUTILEA_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `rutilea tools` → Video Generation, which walks
...
# `rutilea tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `rutilea tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from rutilea_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `rutilea tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``rutilea tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from rutilea_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from rutilea_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``rutilea tools`` checklist actually offers
    for ``platform``.
    ...
    so ``rutilea tools`` never claims to add or remove a toolset the user was
...
from rutilea_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Rutilea Subscription" — managed Firecrawl billed via Nous
    ...
    "image_gen": {
        ...
        #   - "Rutilea Subscription" — managed FAL billed via the Nous
        #     subscription (requires_nous_auth + override_env_vars).
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Rutilea Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Rutilea Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                # "Rutilea Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Rutilea routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Rutilea Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Rutilea Subscription (Browser Use cloud)" — managed Browser Use
        ...
        "providers": [
            ...
            {
                "name": "Rutilea Subscription (Browser Use cloud)",
                "badge": "subscription",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set RUTILEA_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "RUTILEA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "RUTILEA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "RUTILEA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "RUTILEA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "RUTILEA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `rutilea model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("RUTILEA_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``RUTILEA_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Rutilea telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Rutilea' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $RUTILEA_HOME/bin is never on PATH, so a bare which()
    # misses the uv Rutilea installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``rutilea update`` and
      by ``rutilea computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``rutilea update`` sets
    ...
    a further ~600s wait on Windows). ``rutilea computer-use install
    ...
    ``rutilea update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `rutilea update` calls this
    ...
    override = os.environ.get("RUTILEA_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    RUTILEA_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Rutilea' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Rutilea' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `rutilea update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Rutilea cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("RUTILEA_CUA_DRIVER_CMD", "").strip():
            _print_info(
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `rutilea computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `rutilea update` (require_confirmed_update=True) keeps the
    ...
    # `rutilea computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: rutilea computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: rutilea computer-use doctor")
            return False
    if ok and before:
        try:
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `rutilea update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `rutilea update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.rutilea/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Rutilea process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Rutilea-managed ``$RUTILEA_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Rutilea installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Rutilea-managed install)
        ...
        # (PATH -> Homebrew/Rutilea-managed node -> npx) rather than a bare
        # shutil.which — Rutilea-managed-Node-only setups resolve agent-browser
        ...
        # Rutilea-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Rutilea-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.rutilea/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.rutilea/config.yaml")
        ...
        # Run the full `rutilea auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.rutilea/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from rutilea_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: rutilea auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `rutilea auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: rutilea auth spotify")
        except Exception as exc:
        ...
        try:
            from rutilea_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: rutilea plugins enable observability/langfuse")
        _print_info("    Restart Rutilea for tracing to take effect.")
        _print_info("    Verify: rutilea plugins list")
        ...
        try:
            ...
            _print_info("    Run later: rutilea auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `rutilea auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: rutilea auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: rutilea auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``rutilea tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``rutilea tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: rutilea tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``rutilea-discord``, which contains those
...
#:
#: Saving ``rutilea tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[rutilea-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``rutilea tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"rutilea-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (rutilea-acp, rutilea-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``rutilea-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[rutilea-cli, a2a]`` must survive filtering just
    ...
    # "rutilea-cli" (which include all _RUTILEA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[rutilea-cli, spotify]`` after enabling Spotify via ``rutilea
        ...
        # (e.g. "rutilea-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``rutilea tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `rutilea tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"rutilea-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("rutilea-")}
    ...
    # shipped as a bundled plugin but user must opt in via `rutilea tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `rutilea tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by rutilea tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `rutilea config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `rutilea` instead of
    # `rutilea-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `rutilea update`/`rutilea doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `rutilea tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (rutilea-cli, rutilea-telegram, etc.)
    ...
    # Opening `rutilea tools` is the user's opt-in to reconfigure tools, so treat
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from rutilea_constants import rutilea_home_key

    scope = rutilea_home_key()
...
# ("Rutilea Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Rutilea Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    that the capability exists.  Selecting one drives an inline Rutilea Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    # later) so the picker lists them alongside FAL / Rutilea Subscription.
    ...
    # remaining hardcoded rows ("Rutilea Subscription", "Firecrawl
    ...
    # the hardcoded "Rutilea Subscription" / "Local Browser" / "Camofox" rows
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `rutilea tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``rutilea tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``rutilea tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from rutilea_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``rutilea tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from rutilea_cli.nous_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Rutilea Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Rutilea Subscription rows).
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_nous_message = _hidden_nous_gateway_message(
        cat,
        config,
        f"the Rutilea Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        for p in providers:
            ...
            # "included" star; everyone else gets a "via Rutilea Portal" hint so
            ...
            if p.get("managed_nous_feature"):
                if _nous_logged_in:
                    ...
                    sub_marker = "  ★ via Rutilea Portal (login on select)"
...
# Kept in sync with the dashboard selects (rutilea_cli/web_server.py
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Rutilea Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `rutilea model`).
    if managed_feature:
        from rutilea_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
        ...
        if not ensure_nous_portal_access(
            ...
            _print_warning(
                "  Not enabled — Rutilea Portal access is required for this backend."
            )
            return
    ...
    if provider.get("requires_nous_auth") and not managed_feature:
        ...
        if not features.nous_auth_present or not entitled:
            message = format_nous_portal_entitlement_message(
                ...
                capability=f"{provider.get('name', 'Rutilea Subscription')}",
            )
            _print_warning(
                f"  {message or 'Rutilea Subscription is only available after logging into Rutilea Portal.'}"
            )
            return
    ...
    if _show_portal_hint:
        _print_info("  Available through Rutilea Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``rutilea model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``rutilea model`` aux-task
    ...
    """
    try:
        from rutilea_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `rutilea model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Rutilea Portal login + entitlement gate as _configure_provider:
        ...
        if backend:
            ...
            if backend == "fal":
                ...
                if isinstance(img_cfg, dict):
                    img_cfg["provider"] = "fal"
                    # A managed (Rutilea Subscription) row also carries
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `rutilea model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `rutilea tools` and `rutilea setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Rutilea Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print(color(f"  Tool configuration saved to {display_rutilea_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'rutilea' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, rutilea mcp
