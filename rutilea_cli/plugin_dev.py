"""Runtime-backed validation behind ``rutilea plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary RUTILEA_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="rutilea-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                ...
                "RUTILEA_BUNDLED_PLUGINS": str(bundled),
                "RUTILEA_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    from rutilea_cli.plugins import PluginManager
    ...
    modules_before = {
        ...
        if name == "rutilea_plugins" or name.startswith("rutilea_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Rutilea discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "rutilea_plugins" or name.startswith("rutilea_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_rutilea_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".rutilea" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from rutilea_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Rutilea supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Rutilea never auto-installs plugin dependencies; "
    ...
    if schema:
        from rutilea_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Rutilea' real scanner and registration path."""
    try:
