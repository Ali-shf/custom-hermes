"""
Top-level argparse construction for the rutilea CLI.
...
# argparse runs (it sets ``RUTILEA_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``rutilea_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    rutilea                        Start interactive chat
    rutilea chat -q "Hello"        Single query mode
    rutilea --tui                  Launch the modern TUI (or set display.interface: tui)
    rutilea --cli                  Force the classic REPL (overrides display.interface: tui)
    rutilea -c                     Resume the most recent session
    rutilea -c "my project"        Resume a session by name (latest in lineage)
    rutilea --resume <session_id>  Resume a specific session by ID
    rutilea --resume latest        Resume the most recent session (same as -c)
    rutilea --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    rutilea setup                  Run setup wizard
    rutilea logout                 Clear stored authentication
    rutilea auth add <provider>    Add a pooled credential
    rutilea auth list              List pooled credentials
    rutilea auth remove <p> <t>    Remove pooled credential by index, id, or label
    rutilea auth reset <provider>  Clear exhaustion status for a provider
    rutilea model                  Select default model
    rutilea fallback [list]        Show fallback provider chain
    rutilea fallback add           Add a fallback provider (same picker as `rutilea model`)
    rutilea fallback remove        Remove a fallback provider from the chain
    rutilea config                 View configuration
    rutilea config edit            Edit config in $EDITOR
    rutilea config set model gpt-4 Set a config value
    rutilea gateway                Run messaging gateway
    rutilea -s rutilea-agent-dev,github-auth
    rutilea -w                     Start in isolated git worktree
    rutilea gateway install        Install gateway background service
    rutilea sessions list          List past sessions
    rutilea sessions browse        Interactive session picker
    rutilea sessions rename ID T   Rename/title a session
    rutilea logs                   View agent.log (last 50 lines)
    rutilea logs -f                Follow agent.log in real time
    rutilea logs errors            View errors.log
    rutilea logs --since 1h        Lines from the last hour
    rutilea debug share             Upload debug report for support
    rutilea console                Open the safe Rutilea command console
    rutilea update                 Update to latest version
    rutilea dashboard              Start web UI dashboard (port 9119)
    rutilea dashboard --stop       Stop running dashboard processes
    rutilea dashboard --status     List running dashboard processes

For more help on a command:
    rutilea <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="rutilea",
        description="Rutilea Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `rutilea chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via RUTILEA_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `rutilea setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to RUTILEA_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.rutilea/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Rutilea Agent",
    )
    ...
    # top-level parser: when the user writes `rutilea -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/rutilea_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also RUTILEA_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.rutilea/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Rutilea itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
