"""
Rutilea Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``rutilea desktop`` / ``rutilea gui``)
     Built inside the agent checkout under ``$RUTILEA_HOME/rutilea-agent/``:
       ...
       - ``$RUTILEA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Rutilea.app`` or ``~/Applications/Rutilea.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Rutilea`` (NSIS per-user)
...
the app name ("Rutilea"), separate from ``$RUTILEA_HOME``:
  - macOS:   ``~/Library/Application Support/Rutilea``
  - Windows: ``%APPDATA%\\Rutilea``
  - Linux:   ``$XDG_CONFIG_HOME/Rutilea`` (default ``~/.config/Rutilea``)
...
``rutilea uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(rutilea_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return rutilea_home / "rutilea-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Rutilea"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Rutilea"
...
def source_built_gui_artifacts(rutilea_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``rutilea desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``rutilea-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(rutilea_home)
    ...
    return [
        ...
        rutilea_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Rutilea" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Rutilea.app"),
            home / "Applications" / "Rutilea.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Rutilea).
            local_base / "Programs" / "Rutilea",
            # Older / alternate layout some builds used.
            local_base / "rutilea-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Rutilea")
    else:
        ...
        # ``uninstall_gui``.
        from rutilea_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `rutilea desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Rutilea.desktop",
        ]
    return paths


def agent_is_installed(rutilea_home: Path) -> bool:
    """Return True when a usable Python agent install exists under RUTILEA_HOME.
    ...
    if (agent_root / "rutilea_cli").is_dir():
        return True
...
def gui_is_installed(rutilea_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(rutilea_home):
        if p.exists():
            return True
...
def gui_install_summary(rutilea_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = rutilea_home if rutilea_home is not None else get_rutilea_home()
...
def uninstall_gui(rutilea_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``rutilea-agent/rutilea_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$RUTILEA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Rutilea disappears from the launcher.
        try:
            from rutilea_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove rutilea' or "
            "'sudo dnf remove rutilea'). AppImage builds are a single file you "
