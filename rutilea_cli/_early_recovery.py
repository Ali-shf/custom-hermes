"""Dependency-light venv recovery that runs BEFORE rutilea_cli.main's imports.

The ``rutilea`` console entry point is ``rutilea_cli.main:main``.  Importing
``rutilea_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``rutilea_cli.env_loader``, ``yaml`` via ``rutilea_cli.config``, ...).  In
...
a corrupted venv.  ``rutilea_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``rutilea_cli.main`` to become
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Rutilea already vendors
    (``~/.rutilea/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".rutilea" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``rutilea acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``rutilea_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``rutilea_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``rutilea update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `rutilea update`: CLI and Desktop retries preserve
...
# every `rutilea` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``rutilea update`` (or the self-lock preflight, #83569) left the
    ...
    ``rutilea_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``rutilea acp`` included).
    ...
    """
    try:
        from rutilea_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; rutilea will keep working from "
