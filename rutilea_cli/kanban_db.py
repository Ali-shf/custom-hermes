...
``<root>`` is the **shared Rutilea root** (the parent of any active
...
``rutilea -p <profile>`` joins the same board as the dispatcher that
...
the profile's Rutilea home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``RUTILEA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``RUTILEA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``rutilea kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.rutilea``. In Docker / custom
deployments where ``RUTILEA_HOME`` points outside ``~/.rutilea`` (e.g.
``/opt/rutilea``), ``<root>`` is ``RUTILEA_HOME``. Legacy env-var
overrides still work:

* ``RUTILEA_KANBAN_DB`` — pin the database file path directly.
* ``RUTILEA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``RUTILEA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``RUTILEA_KANBAN_DB``,
``RUTILEA_KANBAN_WORKSPACES_ROOT``, and ``RUTILEA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/rutilea-kanban-v1-spec.pdf`` for the full
...
from rutilea_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``rutilea_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from rutilea_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("RUTILEA_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active RUTILEA_HOME so dispatcher- and
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from rutilea_cli.lifecycle import has_hook
...
# ``RUTILEA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``RUTILEA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("RUTILEA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``RUTILEA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("RUTILEA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``RUTILEA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "RUTILEA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "rutilea_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``rutilea-agent``
...
def kanban_home() -> Path:
    """Return the shared Rutilea root that anchors the kanban board.

    Resolution order:

    1. ``RUTILEA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_rutilea_root()``, which already returns ``<root>``
       when ``RUTILEA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``RUTILEA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``RUTILEA_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("RUTILEA_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from rutilea_constants import get_default_rutilea_root
    return get_default_rutilea_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``rutilea kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``RUTILEA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``rutilea kanban boards
    ...
    env = os.environ.get("RUTILEA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``rutilea kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``RUTILEA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("RUTILEA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``RUTILEA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("RUTILEA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``RUTILEA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("RUTILEA_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``rutilea kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``rutilea_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``RUTILEA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (rutilea_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``RUTILEA_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("RUTILEA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``rutilea_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from rutilea_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``rutilea gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from rutilea_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see rutilea_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from rutilea_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Rutilea treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``rutilea kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``RUTILEA_KANBAN_DB`` env → ``RUTILEA_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `rutilea kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from rutilea_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        from rutilea_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See rutilea_state._WAL_INCOMPAT_MARKERS for detection logic.
                from rutilea_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``rutilea kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``RUTILEA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from rutilea_cli.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"rutilea_nested_{secrets.token_hex(8)}"
...
def create_task(
    ...
    *,
    ...
    each name to ``rutilea --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from rutilea_cli import projects_db as _pdb
    ...
    # `rutilea --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``rutilea kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `rutilea kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``rutilea kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``rutilea kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``rutilea kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``rutilea kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``rutilea kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``RUTILEA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Rutilea' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Rutilea-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``rutilea kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Rutilea checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Rutilea treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Rutilea creates/reuses that linked worktree. With
      no ``workspace_path``, Rutilea anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Rutilea uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``RUTILEA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Rutilea
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``rutilea kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``rutilea kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Rutilea profile.
    ...
    if not rows:
        return False
    try:
        from rutilea_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Rutilea ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. Rutilea
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``rutilea kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``RUTILEA_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from rutilea_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from rutilea_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Rutilea profile.
        # `_default_spawn` invokes ``rutilea -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Rutilea
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `rutilea kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Rutilea bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from rutilea_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_rutilea_argv() -> list[str]:
    """Return the interpreter-bound Rutilea CLI invocation."""
    # ``rutilea_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``rutilea`` package — there is no
    # ``rutilea`` package to import.
    return [sys.executable, "-m", "rutilea_cli.main"]


def _absolute_rutilea_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Rutilea shim."""
...
def _rutilea_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Rutilea executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_rutilea_argv()
    return [_absolute_rutilea_path(path)]


def _resolve_rutilea_argv() -> list[str]:
    """Resolve the ``rutilea`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$RUTILEA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("rutilea")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\rutilea.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m rutilea_cli.main`` — fallback for setups where
       Rutilea is launched from a venv and the ``rutilea`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_rutilea_bin`` for the same reason. Kept
    local (not imported from gateway) because ``rutilea_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("RUTILEA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _rutilea_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _rutilea_path_argv(resolved_env_bin)
        return _module_rutilea_argv()

    rutilea_bin = _safe_which_no_cwd("rutilea") if _IS_WINDOWS else shutil.which("rutilea")
    if rutilea_bin:
        return _rutilea_path_argv(rutilea_bin)
    return _module_rutilea_argv()
...
def _resolve_worker_cli_toolsets(rutilea_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``RUTILEA_KANBAN_TASK`` is set.
    """
    if not rutilea_home:
        return None
    try:
        ...
        token = set_rutilea_home_override(rutilea_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for RUTILEA_HOME=%r (%s)",
            rutilea_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``rutilea -p <profile> chat -q ...`` subprocess.
    ...
    ``RUTILEA_KANBAN_DB`` / ``RUTILEA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject RUTILEA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `rutilea -p <name>` the
    # _apply_profile_override() runs *before* rutilea_constants is imported.
    # If RUTILEA_HOME is absent from the child's env, get_rutilea_home() falls
    # back to Path.home() / ".rutilea" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from rutilea_cli.profiles import resolve_profile_env
    try:
        env["RUTILEA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via RUTILEA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # RUTILEA_HOME never had profiles created.
        pass
    if task.tenant:
        env["RUTILEA_TENANT"] = task.tenant
    env["RUTILEA_KANBAN_TASK"] = task.id
    env["RUTILEA_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `rutilea kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["RUTILEA_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["RUTILEA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["RUTILEA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["RUTILEA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["RUTILEA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["RUTILEA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`rutilea -p <name>`
    # rewrites RUTILEA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_rutilea_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["RUTILEA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["RUTILEA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["RUTILEA_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["RUTILEA_KANBAN_BOARD"] = resolved_board
    # RUTILEA_PROFILE is the author the kanban_comment tool defaults to.
    # `rutilea -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["RUTILEA_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited RUTILEA_TUI=1
    ...
    # older rutilea builds on PATH that predate the flag's precedence.
    env.pop("RUTILEA_TUI", None)

    cmd = [
        *_resolve_rutilea_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped RUTILEA_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("RUTILEA_HOME"))
    if worker_toolsets:
    ...
    # `rutilea kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`rutilea` executable not found on PATH. "
            "Install Rutilea Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``rutilea kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``rutilea kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `rutilea kanban dispatch` do (OOF-30): explicit
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled RUTILEA_PROFILE values like "rutilea-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Rutilea root exists

    Reads profile paths directly so this module has no import dependency on
    ``rutilea_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from rutilea_constants import get_default_rutilea_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``rutilea kanban assignees`` for the terminal.
