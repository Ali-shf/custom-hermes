...
Usage:
    rutilea pairing list              # Show all pending + approved users
    rutilea pairing approve <platform> <request-id|code>  # Approve a pairing request
    rutilea pairing revoke <platform> <user_id> # Revoke user access
    rutilea pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle rutilea pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: rutilea pairing {list|approve|revoke|clear-pending}")
        print("Run 'rutilea pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: rutilea pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.rutilea/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'rutilea pairing list' to see pending requests.\n")
