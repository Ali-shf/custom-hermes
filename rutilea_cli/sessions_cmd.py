"""``rutilea sessions`` command — extracted from ``rutilea_cli/main.py``.
...
Helpers that stay in ``rutilea_cli.main`` (``get_rutilea_home``,
...
``rutilea_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def get_rutilea_home():
    return _m().get_rutilea_home()
...
#: Default age floor for `rutilea sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`rutilea sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from rutilea_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    rutilea sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from rutilea_cli.session_recovery import (
    ...
    if action == "import":
        from rutilea_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from rutilea_state import workspace_key as _ws_key
        ...
        from rutilea_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from rutilea_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_rutilea_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from rutilea_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_rutilea_home() / "session-exports"
        ...
        # `rutilea sessions prune`: no time window and no filters at all
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `rutilea sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: rutilea sessions pin <session_id>"
            )
            return
        ...
        # Launch rutilea --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`rutilea sessions optimize` later to reclaim freed space.)")
