"""Profile distributions — shareable, packaged Rutilea profiles via git.

A distribution is a Rutilea profile published as a git repository (or
...
* ``rutilea profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``rutilea skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``rutilea profile``, not a parallel tree):

    rutilea profile install <source> [--name N] [--alias] [--force] [--yes]
    rutilea profile update  <name>  [--force-config] [--yes]
    rutilea profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "rutilea_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".rutilea_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "rutilea-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            rutilea_requires=str(data.get("rutilea_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.rutilea_requires:
            out["rutilea_requires"] = self.rutilea_requires
        if self.author:
...
def check_rutilea_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Rutilea {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Rutilea distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``rutilea profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Rutilea profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Rutilea distribution."
        )

    # Version check up-front so we fail fast
    check_rutilea_requires(manifest.rutilea_requires, rutilea_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.rutilea).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="rutilea_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `rutilea profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`rutilea profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="rutilea_dist_update_") as tmp:
        plan = plan_install(
