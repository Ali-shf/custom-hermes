"""``rutilea verify`` — detect a project's run recipe and smoke-test it.
...
``.rutilea/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``rutilea verify`` satisfies
