...
Scope of the repair is deliberately narrow. Rutilea only upgrades an npm that
lives inside its **own** managed Node tree (``$RUTILEA_HOME/node``), installing
...
and their other projects; Rutilea never modifies those. When the failing npm is
one of those foreign installs, Rutilea instead provisions its own managed Node
...
from rutilea_constants import (
    bootstrap_rutilea_managed_node,
    get_rutilea_home,
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Rutilea-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$RUTILEA_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$RUTILEA_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Rutilea-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Rutilea-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="rutilea-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Rutilea could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Rutilea-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$RUTILEA_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Rutilea-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_rutilea_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Rutilea-managed install, or
    ...
    if prefix is not None:
        # Rutilea owns this npm — upgrade it in place. Only an npm-range
