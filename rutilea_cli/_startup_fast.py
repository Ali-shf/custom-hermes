...
This module is imported by ``rutilea_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no rutilea_cli.config,
...
``rutilea_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(rutilea_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect RUTILEA_HOME?"""
    active_profile = os.path.join(rutilea_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    rutilea_home = os.environ.get("RUTILEA_HOME", "").strip()
    if rutilea_home:
        return rutilea_home
    return os.path.join(os.path.expanduser("~"), ".rutilea")
...
def container_mode_may_be_active() -> bool:
    ...
    rutilea_home = os.environ.get("RUTILEA_HOME", "").strip()
    if rutilea_home:
        if os.path.exists(os.path.join(rutilea_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(rutilea_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(rutilea_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".rutilea")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect RUTILEA_HOME.
    """
...
def print_fast_version_info() -> None:
    from rutilea_cli import __release_date__, __version__

    print(f"Rutilea Agent v{__version__} ({__release_date__})")
    ...
    print("Run 'rutilea version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``rutilea --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the RUTILEA_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("RUTILEA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
