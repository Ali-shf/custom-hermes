"""``rutilea debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Rutilea Agent. Use 'rutilea debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    rutilea debug share              Upload debug report (asks for confirmation)
    rutilea debug share --yes        Skip confirmation (for scripts/CI)
    rutilea debug share --lines 500  Include more log lines
    rutilea debug share --expire 30  Keep paste for 30 days
    rutilea debug share --local      Print report locally (no upload)
    rutilea debug share --no-redact  Disable upload-time secret redaction
    rutilea debug share --nous       Upload to Nous-internal storage (private)
    rutilea debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'rutilea debug share'",
    )
