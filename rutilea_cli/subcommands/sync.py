"""``rutilea sync`` subcommand parser — Skill Sync.

Cloned from ``rutilea_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    rutilea sync status                 show gate/opt-in/head state
    rutilea sync pull                   pull and materialize opted-in skills
    rutilea sync push                   push opted-in skills
    rutilea sync now                    reconcile: pull then push
    rutilea sync enable <skill>         opt a skill into sync
    rutilea sync disable <skill>        opt a skill out of sync
    rutilea sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    rutilea sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  rutilea sync status            what is synced, and from where\n"
            "  rutilea sync enable my-skill   include a skill in your sync\n"
            "  rutilea sync now               pull, then push\n"
            "  rutilea sync propose my-skill  share a skill with your team\n"
        ),
