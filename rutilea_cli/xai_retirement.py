...
Pure logic: walks a Rutilea config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `rutilea doctor` and a future `rutilea migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Rutilea config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Rutilea config and return retirement issues.
...
def apply_migration(
    ...
    from rutilea_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `rutilea migrate xai` reports nothing to migrate
