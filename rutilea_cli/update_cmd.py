"""Rutilea update pipeline — extracted from ``rutilea_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``rutilea_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``rutilea_cli.main`` reference — so existing call sites and test monkeypatches
that target ``rutilea_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``rutilea_cli.main``.
...
Imports are one-way: ``rutilea_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``rutilea_cli.main`` reference.

    Lets callers keep patching ``rutilea_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``rutilea_cli.main`` -> ``rutilea_cli.update_cmd`` stays one-way
    at import time.
    """
    from rutilea_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``rutilea update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``rutilea update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``rutilea_cli.config_defaults``,
    ``rutilea_cli.config``, and ``rutilea_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``rutilea_cli._subprocess_compat`` and
    ``rutilea_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "rutilea_cli.config_defaults",
        "rutilea_cli.config",
        "rutilea_cli.config_migrations",
        "rutilea_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from rutilea_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from rutilea_cli.config import migrate_config

    return migrate_config(interactive=interactive, quiet=quiet)


# Critical files that Rutilea must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "rutilea_cli/main.py",
    "rutilea_cli/config.py",
    "rutilea_cli/__init__.py",
    "rutilea_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "rutilea_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``rutilea`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="rutilea-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``rutilea update`` can be driven by a
    ...
    """
    from rutilea_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from rutilea_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('rutilea_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``rutilea update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `rutilea update`.
    ...
    print("  Preview now:  rutilea curator run --dry-run")
    print("  Pause it:     rutilea curator pause")
    print(
        "  Docs:         https://hermes-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `rutilea update`.
    ...
    try:
        ...
        return
    db_path = get_rutilea_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `rutilea sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    rutilea sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  rutilea sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``rutilea update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``rutilea update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: rutilea curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from rutilea_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    rutilea dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``rutilea_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.rutilea-update-staging"
    backup = f"{dst}.rutilea-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `rutilea update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``rutilea_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``rutilea update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && rutilea update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("RUTILEA_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== rutilea-update completed {action_id} ===")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    """Update Rutilea Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `rutilea update "
            f"--branch {branch}`, or update against main with `rutilea update`."
        )
        _m().sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="rutilea-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"rutilea-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for rutilea-agent itself
        ...
        # GitHub ZIPs extract to rutilea-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"rutilea-agent-{branch}")
        ...
        print(
            "  Re-run `rutilea update` to retry; if the agent won't start, "
    ...
    from rutilea_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `rutilea update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: rutilea skills list-modified  "
    ...
    try:
        from rutilea_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from rutilea_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_rutilea_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_rutilea_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "rutilea-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Rutilea behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes rutilea completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Rutilea couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Rutilea couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Rutilea behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Rutilea couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Rutilea couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/Ali-shf/custom-hermes.git",
    "git@github.com:Ali-shf/custom-hermes.git",
    "https://github.com/Ali-shf/custom-hermes",
    "git@github.com:Ali-shf/custom-hermes",
}

OFFICIAL_REPO_URL = "https://github.com/Ali-shf/custom-hermes.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_rutilea_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_rutilea_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Rutilea repository.")
...
def _invalidate_update_cache():
    ...
    ``rutilea update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from rutilea_constants import get_default_rutilea_root

    default_home = get_default_rutilea_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "rutilea.exe"
    lines = ["✗ Another rutilea.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Rutilea Desktop, exit any open `rutilea` REPLs, and")
    lines.append("  stop the gateway (`rutilea gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `rutilea update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``rutilea tools``."""
    try:
        from rutilea_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Rutilea Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``rutilea tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Rutilea Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Rutilea Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``rutilea update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `rutilea update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `rutilea update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Rutilea' editable-install extras or
    ...
    try:
        from rutilea_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from rutilea_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$RUTILEA_HOME/bin/uv``, but on Termux the official
    ...
    """
    from rutilea_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `rutilea update` is exactly the step
...
def _npm_lockfile_changed(rutilea_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `rutilea
    ...
    try:
        ...
        cache_file = rutilea_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(rutilea_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    rutilea update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `rutilea update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from rutilea_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `rutilea update`.")
            failed = []
            if any(
    ...
    # Rutilea profile using this checkout. Keep one per-checkout cache under the
    # shared Rutilea root rather than rerunning npm once per named profile.
    shared_rutilea_root = get_default_rutilea_root()
    ...
    # `rutilea update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_rutilea_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `rutilea update`.")
        return list(labels)
    ...
    nixos_env = with_rutilea_node_path(_m()._nixos_build_env())
    ...
    # `rutilea update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_rutilea_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.rutilea/logs/update.log`` only, never the terminal.

    During ``rutilea update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``rutilea update --check``: fetch and report without installing.
    ...
    """
    from rutilea_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``rutilea update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from rutilea_cli.config import format_docker_update_message
    ...
    from rutilea_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from rutilea_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``rutilea update`` without requiring a reinstall.  The
    ...
    ``rutilea`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/rutilea.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v rutilea'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/rutilea, code at /usr/local/lib/rutilea-agent).
    fhs_link = Path("/usr/local/bin/rutilea")
    ...
    path_comment = (
        "# Rutilea Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``rutilea-acp`` launcher next to the ``rutilea`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``rutilea update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``rutilea-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Rutilea as not installed even when it is.

    The shim simply delegates to the sibling ``rutilea`` launcher with the
    ...
    No-op on Windows (install.ps1 copies ``rutilea.exe`` + ``rutilea-acp.exe``
    ...
    so ``rutilea-acp.exe`` already resolves) and wherever a ``rutilea-acp`` is
    already present next to the ``rutilea`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        rutilea_cmd = bin_dir / "rutilea"
        acp_cmd = bin_dir / "rutilea-acp"
        try:
            if not (rutilea_cmd.is_file() or rutilea_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of RUTILEA_HOME under
      ``backups/`` (restorable via ``rutilea import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from rutilea_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from rutilea_constants import
        # get_rutilea_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from rutilea_cli.config import get_rutilea_home as _get_home
    ...
    try:
        from rutilea_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from rutilea_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_rutilea_home so the user sees ~/.rutilea/...
    try:
        from rutilea_constants import get_rutilea_home, display_rutilea_home

        home = get_rutilea_home()
        try:
            display_path = f"{display_rutilea_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  rutilea import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``rutilea update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``rutilea update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run rutilea from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.rutilea-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".rutilea-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The rutilea.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m rutilea_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``rutilea
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m rutilea_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "rutilea_cli.main" in cmdline_low:
        ...
        # the `-m rutilea_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``rutilea_cli.main`` used to import
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `rutilea` launch will complete it")
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Rutilea processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Rutilea Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Rutilea desktop app / other Rutilea terminals, then re-run:"
    )
    lines.append("    rutilea update")
    lines.append("  (or use `rutilea update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``rutilea update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from rutilea_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning rutilea-setup, and the update-in-progress marker
    ...
    the update with "Rutilea is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Rutilea backend (``rutilea_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.rutilea-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "rutilea_cli.main" in argv_low and (
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.rutilea-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    rutilea.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from rutilea_cli.gateway import (
    ...
    print("→ Stopping Windows gateway process(es) before updating Rutilea...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m rutilea_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: rutilea gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``rutilea gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from rutilea_cli import gateway_windows
        from rutilea_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``rutilea-gateway*.service``/``rutilea-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("rutilea-serve")`` alone would also accept the
        # unrelated ``rutilea-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "rutilea-gateway.service"
            or unit.startswith("rutilea-gateway-")
            or unit == "rutilea-serve.service"
            or unit.startswith("rutilea-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``rutilea-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``rutilea-serve*`` units (#83438) don't, so sending them
    ...
    (``rutilea-gateway-helper`` is fine — profile units are
    ``rutilea-gateway-<profile>`` — but ``rutilea-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "rutilea-gateway" or svc_name.startswith("rutilea-gateway-")
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``rutilea_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``rutilea_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``rutilea update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``rutilea-setup.exe``) executes
    ``$RUTILEA_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_rutilea_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``rutilea update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``rutilea update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `rutilea desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "rutilea_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Rutilea-managed Node on PATH: when
    # `rutilea update` runs inside the desktop updater chain (Desktop →
    # rutilea-setup → rutilea update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from rutilea_constants import with_rutilea_node_path

    build_env = with_rutilea_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (non-fatal; run `rutilea desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Rutilea Agent...")
    print()

    # On Windows, abort early if another rutilea.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_rutilea_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `rutilea serve` backend) will keep .pyd
    ...
    # the rutilea.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .rutilea-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Rutilea is still
    ...
    # process by design (a CLI `rutilea update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Rutilea windows/gateways and re-run: rutilea update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Rutilea gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in rutilea_cli/config.py bricked
        # every user who ran ``rutilea update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``rutilea
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``rutilea update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``rutilea update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``rutilea doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && rutilea update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_rutilea_home added to rutilea_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``rutilea`` launch finishes the install
        ...
        # symbols in rutilea_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `rutilea` again "
        ...
        if not import_ok:
            ...
            print("    Run `rutilea update` again — if it persists, reinstall:")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_rutilea_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.rutilea/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit RUTILEA_HOME so
        # it is not affected by sync_skills()'s module-level RUTILEA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's RUTILEA_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from rutilea_cli.profiles import backfill_profile_envs
        ...
        # ``rutilea update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``rutilea_cli.config`` and
        # ``rutilea_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from rutilea_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'rutilea config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'rutilea config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: rutilea config migrate")
            else:
                print()
                print("Skipped. Run 'rutilea config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from rutilea_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `rutilea curator status`. Self-stamps after printing so it
        ...
        # Self-heal the rutilea-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Rutilea on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("rutilea-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``rutilea update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from rutilea_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `rutilea update`
                # must stay fast; `rutilea computer-use install --upgrade`
        ...
        # When running as ``rutilea update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_rutilea_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... rutilea-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... rutilea-gateway*)
            ...
            try:
                from rutilea_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all rutilea-gateway* units (default + profiles) plus
            # rutilea-serve* units (the Desktop app's backend, #83438).
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: rutilea gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # rutilea-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `rutilea update` restart it automatically, allow\n"
                        ...
                        # path in `rutilea gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: rutilea gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: rutilea -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Rutilea gateway unit files are still installed.
        # When both rutilea.service (from a pre-rename install) and the
        # current rutilea-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `rutilea update` surfaces the issue until the user migrates.
        try:
            from rutilea_cli.gateway import (
                has_legacy_rutilea_units,
                _find_legacy_rutilea_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_rutilea_units():
                print()
                print("⚠ Legacy Rutilea gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_rutilea_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (rutilea.service) fight the current")
                print("  rutilea-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    rutilea gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes rutilea-serve*,
        ...
        print("  rutilea model              # Select provider and model")
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``rutilea_cli.gateway`` no longer imports,
