"""Dashboard process-hygiene helpers — extracted from ``rutilea_cli/main.py``.
...
``_detect_concurrent_rutilea_instances``) are lifted verbatim. References to
helpers that STAY in ``rutilea_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``rutilea_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``rutilea_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``rutilea_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``rutilea dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``rutilea update`` replaces files on disk, the running
    ...
    ``rutilea-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Rutilea Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``rutilea serve`` as
    ...
    environment variable ``RUTILEA_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`rutilea serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "rutilea.*dashboard"`) keeps us consistent
            # with `rutilea_cli.gateway._scan_gateway_pids` and avoids the
...
def _rutilea_home_for_pid(pid: int) -> str | None:
    """Best-effort ``RUTILEA_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("RUTILEA_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"RUTILEA_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Rutilea Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``rutilea update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], rutilea_home: str | None = None
) -> str:
    """Stable owner key: ``RUTILEA_HOME`` when known, else ``--profile`` / ``-p``.

    ``RUTILEA_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.rutilea`` roots) keep a resolved ``home:`` key so
    ...
    if rutilea_home:
        try:
            home_path = Path(rutilea_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(rutilea_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(rutilea_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``rutilea update``.

    Each candidate is ``(pid, argv, rutilea_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``RUTILEA_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``RUTILEA_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``rutilea update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, rutilea_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, rutilea_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``rutilea dashboard`` / ``rutilea serve`` processes.

    Called at the end of ``rutilea update`` (default ``reason``) and also
    from ``rutilea dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``rutilea update`` path), a detected
    ``rutilea-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``rutilea-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``rutilea update``'s systemd
    fleet-restart loop, which restarts ``rutilea-serve*`` units before this
    ...
    # When the Rutilea Desktop Electron app spawns this dashboard as a
    # backend child, it sets RUTILEA_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("RUTILEA_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `rutilea update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot RUTILEA_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _rutilea_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. rutilea update's
    ...
    #    remote backend (rutilea serve) under Restart=on-failure never comes
...
def _detect_concurrent_rutilea_instances(
    ...
    The Rutilea Desktop Electron app spawns ``rutilea.EXE`` as a backend child,
    so during ``rutilea update`` the user-invoked process and the desktop's
    ...
    shims (``rutilea.exe`` / ``rutilea-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``rutilea update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``rutilea.exe``) is a separate process from the Python
    ...
    for shim in _m()._rutilea_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated rutilea.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``rutilea update``
    ...
    #      rutilea.exe sitting *under* a non-Rutilea parent (e.g. a Rutilea
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        rutilea serve --host 127.0.0.1 --port 0
        rutilea serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "rutilea" not in cmd and "rutilea_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (RUTILEA_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("RUTILEA_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``rutilea serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``rutilea serve`` on a Mac
...
def _rutilea_home_dir() -> Path:
    """Resolved Rutilea home (RUTILEA_HOME override or ~/.rutilea)."""
    override = os.environ.get("RUTILEA_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "rutileaPath", "rutileaHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated RUTILEA_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{rutilea_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _rutilea_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local rutilea serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``rutilea serve`` backends with no parent.
    ...
    running under RUTILEA_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never RUTILEA_DESKTOP_CHILD_PID entries
