"""Windows ConPTY bridge for the `rutilea dashboard` chat tab.

Drop-in counterpart to ``rutilea_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``rutilea_cli.web_server`` consumes: ``spawn``, ``read``,
