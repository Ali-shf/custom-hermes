...
``CommandDef.execute`` (rutilea_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/rutilea_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``rutilea_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_version(ctx: CommandContext) -> CommandReply:
    ...
    from rutilea_cli.banner import format_banner_version_label
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_rutilea_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: rutilea bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from rutilea_cli.commands import gateway_help_lines
