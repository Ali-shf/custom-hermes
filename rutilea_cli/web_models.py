"""Pydantic request/response models for the Rutilea dashboard web server.

Extracted verbatim from ``rutilea_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from rutilea_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``rutilea model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `rutilea import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`rutilea -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the RUTILEA_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under RUTILEA_HOME.
    output: str = ""
