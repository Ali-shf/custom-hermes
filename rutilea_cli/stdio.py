...
on Japanese locales, etc.).  Rutilea's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``rutilea_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Rutilea is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``RUTILEA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("RUTILEA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Rutilea-managed Git install directories so
    ...
    # already-running shells don't see the change — which means rutilea
    ...
    # install without Rutilea-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Rutilea's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Rutilea's
    ``rutilea config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Rutilea (User env var in Windows Settings, or
    export in a PowerShell profile) and Rutilea picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Rutilea-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\rutilea\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``rutilea`` from right after install) retain their old PATH.

    Any subprocess Rutilea spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Rutilea-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "rutilea", "git", "cmd"),
        os.path.join(local_appdata, "rutilea", "git", "bin"),
        os.path.join(local_appdata, "rutilea", "git", "usr", "bin"),
        # Rutilea venv Scripts directory — host of the rutilea.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes rutilea, but harmless to include.
        os.path.join(local_appdata, "rutilea", "rutilea-agent", "venv", "Scripts"),
