"""``rutilea logs`` — view and filter Rutilea log files.
...
Usage examples::
    ...
    rutilea logs                    # last 50 lines of agent.log
    rutilea logs -f                 # follow agent.log in real time
    rutilea logs errors             # last 50 lines of errors.log
    rutilea logs gateway -n 100    # last 100 lines of gateway.log
    rutilea logs gui -f            # follow gui.log (dashboard/pty/ws)
    rutilea logs desktop -f        # follow desktop.log (Electron app boot/backend)
    rutilea logs --level WARNING    # only WARNING+ lines
    rutilea logs --session abc123   # filter by session ID substring
    rutilea logs --component tools  # only tool-related lines
    rutilea logs --since 1h         # lines from the last hour
    rutilea logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_rutilea_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Rutilea runs — try 'rutilea chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_rutilea_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_rutilea_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_rutilea_home()}/logs/")
        return

    print(f"Log files in {display_rutilea_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'rutilea chat' to generate logs)")
