...
the tool registry or launching Rutilea (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``rutilea-cli`` to the non-existent ``rutilea``. ``resolve_toolset('rutilea')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``rutilea-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``rutilea`` instead of ``rutilea-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `rutilea tools` to reconfigure."
        )
    return warnings
