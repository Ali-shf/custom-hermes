"""Install and remove the Linux desktop entry (``rutilea.desktop``).

``rutilea desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "rutilea.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``rutilea.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``rutilea desktop``.

    Prefer the real ``rutilea`` executable (argv[0] or PATH). When Rutilea
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from rutilea_cli.relaunch import resolve_rutilea_bin

    bin_path = resolve_rutilea_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "rutilea_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Rutilea\n"
        "GenericName=Rutilea Desktop\n"
        "Comment=Launch Rutilea Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Rutilea\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Rutilea desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "rutilea"
