"""Bounded product contract for the first Rutilea shared-metrics slice."""
...
SCHEMA_KEY = "rutilea.metrics.schema_version"
SCHEMA_VERSION = "rutilea.metrics.event.v2"
MODEL_CALL_SCOPE = "rutilea.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "rutilea.task_run"
TOOL_CALL_SCOPE = "rutilea.tool_call"
CLIENT_ACTIVE_MARK = "rutilea.client.active"
TOOL_APPROVAL_MARK = "rutilea.tool_approval"
SKILL_LIFECYCLE_MARK = "rutilea.skill.lifecycle"
SKILL_LOAD_MARK = "rutilea.skill.load"
SUBSCRIBER_NAME = "rutilea.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "rutilea.client.active"
LEGACY_MODEL_CALL_METRIC = "rutilea.model_call.count"
MODEL_ROUTE_METRIC = "rutilea.model_route.count"
TASK_STARTED_METRIC = "rutilea.task_run.started"
TASK_FINISHED_METRIC = "rutilea.task_run.finished"
TOOL_CALL_METRIC = "rutilea.tool_call.count"
TOOL_APPROVAL_METRIC = "rutilea.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "rutilea.skill.lifecycle.count"
SKILL_LOAD_METRIC = "rutilea.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Rutilea installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(rutilea_version or "").strip()
    ...
    return {
        ...
        "rutilea_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("rutilea_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Rutilea logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("rutilea.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from rutilea_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Rutilea terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Rutilea registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "rutilea-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Rutilea tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Rutilea."""
