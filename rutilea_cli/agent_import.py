"""rutilea import-agent — import Claude Code / Codex CLI setups into Rutilea.

Usage:
    rutilea import-agent                       # auto-detect ~/.claude or ~/.codex
    rutilea import-agent claude-code           # import from ~/.claude
    rutilea import-agent codex                 # import from ~/.codex
    rutilea import-agent claude-code --dry-run # preview only, no changes
    rutilea import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``rutilea claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_rutilea.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in RUTILEA_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → RUTILEA_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in RUTILEA_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in RUTILEA_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → RUTILEA_HOME/skills/codex-imports/<name>/
...
deliberately via ``rutilea setup`` or config.yaml.
"""
...
# Same entry delimiter as the Rutilea memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `rutilea config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`rutilea config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_rutilea.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Rutilea command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Rutilea glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Rutilea equivalent — "
    ...
    def import_mcp_servers(self, servers: Dict[str, Any], kind: str) -> None:
        ...
        for name, srv in servers.items():
            ...
            rutilea_srv: Dict[str, Any] = {}
            if srv.get("command"):
                rutilea_srv["command"] = srv["command"]
                if srv.get("args"):
                    rutilea_srv["args"] = srv["args"]
                env_kept, env_stripped = sanitize_mcp_env(srv.get("env"))
                if env_kept:
                    rutilea_srv["env"] = env_kept
                if env_stripped:
                    self.stripped_secrets.extend(
                        f"mcp_servers.{name}.env.{k}" for k in env_stripped
                    )
                if srv.get("cwd"):
                    rutilea_srv["cwd"] = srv["cwd"]
            if srv.get("url"):
                rutilea_srv["url"] = srv["url"]
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        rutilea_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → RUTILEA_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``rutilea import-agent`` (invoked from rutilea_cli.main)."""
    from rutilea_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: rutilea import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: rutilea import-agent claude-code   or   rutilea import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Rutilea — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: rutilea import-agent "
    ...
    print_info(f"Target:      {rutilea_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'rutilea setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: rutilea import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'rutilea setup' "
               "to configure providers, or add them to ~/.rutilea/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from rutilea_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'rutilea setup' or ~/.rutilea/.env.")
        print()
