"""Rutilea-managed uv and Python runtime repair.

Rutilea owns its own uv binary at ``$RUTILEA_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$RUTILEA_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Rutilea
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.rutilea-runtime/python``. A
...
from rutilea_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".rutilea-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Rutilea keeps *its* uv binary.

    ``$RUTILEA_HOME/bin/uv`` on POSIX, ``$RUTILEA_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Rutilea-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Rutilea keeps databases "
            "out of WAL mode on this SQLite build. The next `rutilea update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``rutilea update`` runs
    the call site from the *old*, already-imported ``rutilea_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``rutilea update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under RUTILEA_HOME
    caches the last successful self-update time.
    """
    try:
        from rutilea_constants import get_rutilea_home

        stamp = get_rutilea_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``rutilea update`` so the managed copy stays current.
...
def _reload_rutilea_constants():
    """Re-execute ``rutilea_constants`` from disk and return the fresh module.

    ``rutilea update`` imports ``rutilea_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        cannot import name 'venv_python_path' from 'rutilea_constants'
        (~/.rutilea/rutilea-agent/rutilea_constants.py)
    ...
    """
    import rutilea_constants

    return importlib.reload(rutilea_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Rutilea generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `rutilea update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("rutilea_cli.main")
    ...
    if holders:
        ...
        return True, f"other Rutilea processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Rutilea-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``rutilea update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``rutilea update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$RUTILEA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
