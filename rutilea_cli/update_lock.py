"""Cross-process mutual exclusion for in-flight Rutilea updates.

Three different surfaces can start an update of the same install tree:

* ``rutilea update`` from a terminal,
* the dashboard's Update button (``POST /api/rutilea/update`` →
  ``_spawn_rutilea_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``rutilea-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``rutilea update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <RUTILEA_HOME>/.rutilea-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``rutilea update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Rutilea is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``rutilea-setup`` binary under ``~/.rutilea`` is only refreshed by a full
  installer run (``copy_self_to_rutilea_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Rutilea is still running") on every
  GUI update forever, with no Rutilea process actually running.
"""
...
MARKER_NAME = ".rutilea-update-in-progress"

# Set by an orchestrating updater (the Tauri `rutilea-setup --update` flow) to
# its own pid before spawning `rutilea update` as a child stage. The parent
...
HANDOFF_PID_ENV = "RUTILEA_UPDATE_HANDOFF_PID"
...
# to show "Rutilea is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Rutilea home (never the context-local profile override):
    the Rust updater resolves ``$RUTILEA_HOME`` or the platform default, and the
    ...
    return get_process_rutilea_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``rutilea update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``rutilea-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Rutilea update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `rutilea update` as a stage): we run under ITS claim
