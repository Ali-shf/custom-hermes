...
Profile-aware location: ``$RUTILEA_HOME/logs/dashboard-auth.log``.
...
from ``rutilea_constants`` or other rutilea_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$RUTILEA_HOME/logs/dashboard-auth.log``.

    Uses ``rutilea_constants.get_rutilea_home()`` (a leaf module — no import
    ...
    return get_rutilea_home() / "logs" / "dashboard-auth.log"
