"""`rutilea checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.rutilea/checkpoints/``.  Actions:

    rutilea checkpoints               # same as `status`
    rutilea checkpoints status        # total size, project count, breakdown
    rutilea checkpoints list          # per-project checkpoint counts + workdir
    rutilea checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    rutilea checkpoints clear [-f]    # nuke the entire base (asks first)
    rutilea checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    rutilea checkpoints
    rutilea checkpoints prune --retention-days 3 --max-size-mb 200
    rutilea checkpoints clear -f
...
from rutilea_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: rutilea checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``rutilea checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `rutilea checkpoints` → status
