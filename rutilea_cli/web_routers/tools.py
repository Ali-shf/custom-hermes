...
late-binding seam in :mod:`rutilea_cli.web_deps` (``late`` for callables,
...
from rutilea_cli.web_deps import late, LateState
from rutilea_cli.web_models import (
...
_log = logging.getLogger("rutilea_cli.web_server")
...
_spawn_rutilea_action = late("_spawn_rutilea_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from rutilea_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `rutilea tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `rutilea computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``rutilea tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `rutilea tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``rutilea tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.rutilea/.env`` via ``save_env_value`` —
    the same store ``rutilea tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``rutilea tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``rutilea -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same RUTILEA_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``rutilea computer-use permissions grant`` as a background action.
