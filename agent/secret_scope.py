...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the RUTILEA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Rutilea runtime / deployment
    "RUTILEA_HOME", "RUTILEA_PROFILE", "RUTILEA_GATEWAY_LOCK_DIR",
    "RUTILEA_MAX_ITERATIONS", "RUTILEA_MAX_TOKENS", "RUTILEA_API_TIMEOUT",
    "RUTILEA_REDACT_SECRETS", "RUTILEA_NOUS_TIMEOUT_SECONDS",
    "_RUTILEA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "RUTILEA_KANBAN_DB", "RUTILEA_KANBAN_WORKSPACES_ROOT", "RUTILEA_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "RUTILEA_KANBAN_",
    "RUTILEA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Rutilea writes
    ...
    ``rutilea_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Rutilea parser: save_env_value
    ...
    from rutilea_cli.config import _parse_env_value
...
def build_profile_secret_scope(rutilea_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(rutilea_home)
    secrets = load_env_file(home / ".env")

    try:
        from rutilea_cli.env_loader import get_secret_source_values
