...
row taxonomy is inferred from the atlas shape so Rutilea can render both legacy
...
# rows (1536x1872), while older Rutilea/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Rutilea' activity state names. They are not always identical to the
...
# Legacy Rutilea/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Rutilea activity names -> accepted row-name aliases in descending
