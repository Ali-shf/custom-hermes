...
``rutilea pause`` writes a sentinel file at ``$RUTILEA_HOME/ESTOP``;
``rutilea resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Rutilea is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.rutilea/ESTOP``.
...
def _rutilea_home() -> Path:
    """Resolve the active RUTILEA_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active RUTILEA_HOME."""
    return _rutilea_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on RUTILEA_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Rutilea is paused ({reason}). New work is on hold; "
            "run `rutilea resume` to pick things back up."
        )
    return (
        "⏸️ Rutilea is paused. New work is on hold; "
        "run `rutilea resume` to pick things back up."
    )
