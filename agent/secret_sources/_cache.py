...
cache problem must never block Rutilea startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Rutilea home used for cache paths.

    ``home_path`` is whatever ``load_rutilea_dotenv()`` already resolved;
    falling back to ``$RUTILEA_HOME`` / ``~/.rutilea`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from rutilea_constants import get_rutilea_home

        home_path = get_rutilea_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<rutilea_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if RUTILEA_HOME is ever made traversable.
            try:
