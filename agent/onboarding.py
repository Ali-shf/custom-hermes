...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Rutilea starts and finds ``~/.openclaw/``.

    Points users at ``rutilea claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``rutilea claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Rutilea, run "
        "`rutilea claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `rutilea claw cleanup` (renames it to ~/.openclaw.pre-migration — "
