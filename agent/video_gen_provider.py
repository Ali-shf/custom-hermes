...
as ``kind: backend``) or ``~/.rutilea/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``rutilea tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$RUTILEA_HOME/cache/videos/``, creating parents as needed."""
    from rutilea_constants import get_rutilea_home

    path = get_rutilea_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$RUTILEA_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$RUTILEA_HOME/cache/videos/``.
