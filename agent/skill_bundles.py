...
Storage
-------
Bundles live in ``~/.rutilea/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``RUTILEA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``rutilea bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under RUTILEA_HOME.

    Honors ``RUTILEA_BUNDLES_DIR`` for tests; falls back to
    ``<RUTILEA_HOME>/skill-bundles``.
    """
    override = os.environ.get("RUTILEA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_rutilea_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `rutilea bundles` CLI subcommand.
# ---------------------------------------------------------------------------
