"""Anthropic Messages API adapter for Rutilea Agent.

Translates between Rutilea's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in rutilea_cli/runtime_provider.py.
    """
...
# Rutilea effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to rutilea's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to rutilea's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + RutileaAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("X-Title", "Rutilea Agent")
        headers.setdefault("User-Agent", f"RutileaAgent/{_RUTILEA_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Rutilea loads that into the process env from
    # ``~/.rutilea/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Rutilea notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Rutilea historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``rutilea models``) never mutates ``~/.rutilea/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Rutilea)
      ...
      5. Anthropic credential_pool OAuth entry (~/.rutilea/auth.json)
    ...
    # 1. Rutilea-managed OAuth/setup token env var
    ...
    # 5. Rutilea credential_pool OAuth entry.
...
# ── Rutilea-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.rutilea/.anthropic_oauth.json (our own file).
...
def _get_rutilea_oauth_file() -> Path:
    return get_rutilea_home() / ".anthropic_oauth.json"
...
def run_rutilea_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Rutilea-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Rutilea with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from rutilea_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_rutilea_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Rutilea-managed OAuth credentials from ~/.rutilea/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Rutilea OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See rutilea-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See rutilea-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See rutilea-agent#13848 (Kimi) and
    rutilea-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Rutilea Agent", "Claude Code")
                text = text.replace("Rutilea agent", "Claude Code")
                text = text.replace("rutilea-agent", "claude-code")
        ...
        #      a) bare Rutilea-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Rutilea surfaces in its CLI. We
