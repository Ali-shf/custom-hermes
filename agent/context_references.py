...
from rutilea_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
_SENSITIVE_RUTILEA_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    rutilea_home = get_rutilea_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(rutilea_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(rutilea_home / rel for rel in _SENSITIVE_RUTILEA_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Rutilea path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.rutilea/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Rutilea path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
