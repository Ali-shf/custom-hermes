...
configured under ``monitoring.export.otlp``. Lets an operator stream Rutilea
...
def _require_sdk(*, auto_install: bool = True, prompt: bool = True):
    ...
    try:
        ...
        raise OTLPUnavailable(
            ...
            "    pip install 'rutilea-agent[otlp]'\n"
            f"(import error: {e})"
        )
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"rutilea.event": kind or "unknown"}
    keep_by_kind = {
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'rutilea-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
