"""Upload a Rutilea session transcript to Hugging Face as an agent trace.

Rutilea stores sessions in its own SQLite store (``rutilea_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``rutilea trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Rutilea' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "rutilea-traces"
_RUTILEA_VERSION = "rutilea-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Rutilea OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Rutilea' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Rutilea conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.rutilea/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `rutilea trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/rutilea-traces`` dataset. Returns a
