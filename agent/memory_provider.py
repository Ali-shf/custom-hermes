...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `rutilea backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - rutilea_home (str): The active RUTILEA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.rutilea``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "rutilea").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'rutilea memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], rutilea_home: str) -> None:
        ...
        Called by 'rutilea memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``rutilea_home`` is the active RUTILEA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE RUTILEA_HOME.

        ``rutilea backup`` only walks RUTILEA_HOME, so any provider state kept
        ...
        of the archive, and ``rutilea import`` restores them to their original
