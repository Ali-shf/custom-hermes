...
Every Rutilea request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Rutilea Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=rutilea-agent",
        "client=rutilea-client-v<__version__>",
    ]

The version is sourced live from ``rutilea_cli.__version__`` so it auto-aligns
...
``rutilea_cli.__version__`` is the canonical source of truth.
"""
...
def _rutilea_version() -> str:
    """Return the current Rutilea release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``rutilea_cli`` cannot be imported (should
...
def rutilea_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=rutilea-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=rutilea-client-v{_rutilea_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Rutilea session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Rutilea conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.rutilea/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Rutilea
    ...
    """
    tags = ["product=rutilea-agent", rutilea_client_tag()]
