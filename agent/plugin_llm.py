...
Plugins built on Rutilea Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Rutilea
...
:class:`~rutilea_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Rutilea' main config
...
handles every provider, fallback chain, and per-task override Rutilea
supports.
"""
...
def _resolve_task_ownership(plugin_id: str) -> tuple[frozenset, frozenset]:
    ...
    try:
        ...
        pass
    try:
        from rutilea_cli.main import _AUX_TASKS
...
class PluginLlm:
    ...
    Instances are constructed by :class:`rutilea_cli.plugins.PluginContext`
