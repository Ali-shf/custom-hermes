...
The parent Rutilea process may itself be a Kanban dispatcher worker with
RUTILEA_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's RUTILEA_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "rutilea_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "RUTILEA_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "RUTILEA_KANBAN_TASK",
    "RUTILEA_KANBAN_RUN_ID",
    "RUTILEA_KANBAN_WORKSPACE",
    "RUTILEA_KANBAN_WORKSPACES_ROOT",
    "RUTILEA_KANBAN_CLAIM_LOCK",
    "RUTILEA_KANBAN_BOARD",
    "RUTILEA_KANBAN_DB",
)
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``RUTILEA_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$RUTILEA_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``RUTILEA_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``RUTILEA_KANBAN_*`` vars while losing the ContextVar in
