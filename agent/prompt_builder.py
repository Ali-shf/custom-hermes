...
_RUTILEA_MD_NAMES = (".rutilea.md", "RUTILEA.md")


def _find_rutilea_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.rutilea.md`` or ``RUTILEA.md``.
    ...
    # could pick up a .rutilea.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _RUTILEA_MD_NAMES:
...
RUTILEA_AGENT_HELP_GUIDANCE = (
    ...
    "information. Load the `rutilea-agent` skill with skill_view(name='rutilea-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.rutilea/kanban.db`. Your task id is in "
    "`$RUTILEA_KANBAN_TASK`; your workspace is `$RUTILEA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $RUTILEA_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $RUTILEA_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${RUTILEA_KANBAN_BRANCH:-wt/$RUTILEA_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$RUTILEA_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`rutilea profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `rutilea kanban <verb>` for board operations. Use "
...
#
# The rutilea-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to rutilea-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Rutilea YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Rutilea launches that runtime.\n\n"
        ...
        "run `rutilea computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Rutilea "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Rutilea floating over another application, so the
    user is rarely asking about Rutilea — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Rutilea "
        ...
        "rather than anything inside Rutilea. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Rutilea terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Rutilea desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.rutilea.send(\"prompt\") — or a "
        "data-rutilea-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Rutilea WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Rutilea itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Rutilea runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Rutilea itself is running. The host OS, home, and cwd "
                f"of the Rutilea process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Rutilea "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Rutilea
    ...
    extra = (os.getenv("RUTILEA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_rutilea_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("RUTILEA_PLATFORM") or os.environ.get("RUTILEA_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.rutilea/skills/`` directory.  External dirs
    ...
    # RUTILEA_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_rutilea_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.rutilea/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_rutilea_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Rutilea Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `rutilea-agent` skill "
            "first. It has the actual commands (e.g. `rutilea config set …`, `rutilea tools`, "
            "`rutilea setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: rutilea setup, rutilea setup tools, rutilea setup terminal, rutilea status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from RUTILEA_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the RUTILEA_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from rutilea_cli.config import ensure_rutilea_home
        ensure_rutilea_home()
    except Exception as e:
        logger.debug("Could not ensure RUTILEA_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_rutilea_home()
...
def _load_rutilea_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.rutilea.md / RUTILEA.md — walk to git root."""
    rutilea_md_path = _find_rutilea_md(cwd_path)
    if not rutilea_md_path:
        return ""
    try:
        content = rutilea_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = rutilea_md_path.name
        try:
            rel = str(rutilea_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".rutilea.md", context_length=context_length,
            read_path=str(rutilea_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", rutilea_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .rutilea.md / RUTILEA.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from RUTILEA_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Rutilea install/source
    ...
    # explicitly configured cwd is honored verbatim — the Rutilea tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Rutilea in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Rutilea install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_rutilea_md(cwd_path, context_length)
    ...
    # SOUL.md from RUTILEA_HOME only — skip when already loaded as identity
    if not skip_soul:
