...
Owns one Codex thread per Rutilea session. Drives `turn/start`, consumes
...
# Rutilea' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_RUTILEA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Rutilea/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Rutilea session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _RUTILEA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("RUTILEA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="rutilea",
            client_title="Rutilea Agent",
            client_version=_get_rutilea_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Rutilea' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Rutilea' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own rutilea-tools callback we
            # auto-accept — the user already approved Rutilea' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Rutilea
        approval-mode/timeout logic. The Rutilea-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Rutilea approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Rutilea approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Rutilea returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Rutilea approval mode/timeout resolution
    ...
    # "prompt expired"; the Rutilea-side messaging already distinguishes them.
    return "decline"
...
def _get_rutilea_version() -> str:
    """Best-effort Rutilea version string for codex's userAgent line."""
    try:
