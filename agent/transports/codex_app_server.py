...
projection into Rutilea' display, approval bridging, transcript projection into
...
"codex_app_server"`. Rutilea' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Rutilea secret — gateway
        ...
        spawn_env = rutilea_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("RUTILEA_KANBAN_TASK"):
            kanban_db = spawn_env.get("RUTILEA_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "RUTILEA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("RUTILEA_HOME", os.path.expanduser("~/.rutilea")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "rutilea",
        client_title: str = "Rutilea Agent",
