...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``rutilea logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Rutilea only initiates compaction in "rutilea" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Rutilea preflight compression for codex app-server "
                "(mode=%s); Rutilea will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Rutilea (via _emit_status), so it always
