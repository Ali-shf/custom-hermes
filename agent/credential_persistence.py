...
``auth.json``.  They intentionally have no dependency on ``rutilea_cli.auth`` so
...
# Sources Rutilea owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Rutilea-owned OAuth/device-code state)
