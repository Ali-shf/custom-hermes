...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in rutilea_state.py — kept local
# to avoid importing rutilea_state at module load time (its module-level
# DEFAULT_DB_PATH = get_rutilea_home() / "state.db" breaks tests that
# monkeypatch get_rutilea_home to return a str).
...
from rutilea_constants import PARTIAL_STREAM_STUB_ID
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.rutilea/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Rutilea tool use: "
    ...
    return (
        ...
        f"context, but Rutilea needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Rutilea config, set `model.ollama_num_ctx: 65536` "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `rutilea auth reset anthropic`. Until "
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Rutilea' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `rutilea fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from rutilea_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.rutilea/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see rutilea_state.classify_persistence_error).
    ...
    # all run inside Codex). Default Rutilea path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Rutilea internals.
        #
        # Rutilea invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Rutilea tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from rutilea_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from rutilea_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("RUTILEA_DUMP_REQUESTS"):
                ...
                from rutilea_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Rutilea/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: rutilea auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `rutilea doctor` for credential-chain diagnostics, or")
                    ...
                    from rutilea_constants import display_rutilea_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Rutilea-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: rutilea config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: rutilea config set ANTHROPIC_API_KEY \"\"")
                ...
                # and omits the field, and Rutilea' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from rutilea_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `rutilea model`."
                        )
                ...
                # providers (DeepSeek, Kimi, MiMo, Rutilea).  A 429 can
                ...
                # which Rutilea' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Rutilea' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`rutilea",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Rutilea may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See rutilea-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `rutilea auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `rutilea model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: rutilea portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: rutilea setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        rutilea fallback add   (interactive picker — same as `rutilea model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.rutilea/config.yaml` to extend the per-call "
                            f"timeout. (Rutilea's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Rutilea mid-turn, resume logic still sees the
                    ...
                    from rutilea_state import classify_persistence_error
                ...
                # timeout (RUTILEA_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from rutilea_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("RUTILEA_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
