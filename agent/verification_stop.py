...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``RUTILEA_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("RUTILEA_VERIFY_ON_STOP")
    if env is not None:
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether the workspace has a runtime verify recipe ``rutilea verify`` can run.

    True when a saved ``.rutilea/environment.json`` manifest exists, or when
    ...
    if verify_commands:
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction += (
                ...
                "start + readiness), prefer `rutilea verify --json` — a passing "
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction = (
                ...
                "project has a runnable verification recipe. Run `rutilea verify "
            ...
            command_instruction = (
                ...
                "`tempfile` path with a `rutilea-verify-` filename prefix, run it "
